...
namespace Krista.FM.Client.MDXExpert.Data
{
    ...
    public class CellSetInfo
    {
        ...
        /// <summary>
        /// Инициализируем коллекции по сел сету
        /// </summary>
        /// <param name="cls">сам сел сет</param>
        ...
        /// </summary>
        /// <param name="cls">сам сел сет</param>
        /// <param name="isMeasurePartAxes">создавать ли для мер отдельную секцию</param>
            ...
            bool isCreateMeasureSections)
        {
            ...
            {
                //искуственно прерывать инициализацию на последнем мембере будем только если 
                //требуется создание коллекции мер, которая как раз на нем и находиться
                if (isCreateMeasureSections)
                {
                    if (dimensionCount == 1)
                        return result;
    ...
    /// <summary>
    /// Информация о элементе измерения
    /// </summary>

...
namespace Krista.FM.Client.MDXExpert
{
    public partial class OffsetPointControl : UserControl
    {
        ...
        /*
        private void CalculateOffsetPercent(MapPoint offsetPoint)
        {
            float intervalX = (maxExtent.X - minExtent.X) / 2;
            float intervalY = (maxExtent.Y - minExtent.Y) / 2;
            float centrX = minExtent.X + intervalX;
            float centrY = minExtent.Y + intervalY;

            offsetX = (offsetPoint.X - centrX) / intervalX;
            offsetY = (offsetPoint.Y - centrY) / intervalY; 

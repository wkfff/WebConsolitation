...
namespace Krista.FM.Client.MDXExpert
{
    public static class MapHelper
    {
        ...
        {
            double minValue = Consts.maxRangeValue;
        ...
        {
            double maxValue = Consts.minRangeValue;
        ...
        public static double GetMinimumRangeLimit(DataSet ds, List<string> excludedMeasures, SerieRule excludedSerieRule)
        {
            double minValue = Consts.maxRangeValue;
            if (ds == null)
            {
            ...
            foreach (DataTable table in ds.Tables)
            {
                for (int i = 3; i < table.Columns.Count; i++)
                {
                    ...
                    if (excludedSerieRule != null)
                    {    
                        if ((excludedSerieRule.Serie.Name == table.TableName) &&
                            (excludedSerieRule.Name == table.Columns[i].ColumnName))
                        {    
                            continue;
                        }
                    }
        ...
        public static double GetMaximumRangeLimit(DataSet ds, List<string> excludedMeasures, SerieRule excludedSerieRule)
        {
            double maxValue = Consts.minRangeValue;
            if (ds == null)
            {
            ...
            foreach (DataTable table in ds.Tables)
            {
                for (int i = 3; i < table.Columns.Count; i++)
                {
                    ...
                    if(excludedSerieRule != null)
                    {
                        if((excludedSerieRule.Serie.Name == table.TableName)&&(excludedSerieRule.Name == table.Columns[i].ColumnName))
                        {
                            continue;
                        }
                    }

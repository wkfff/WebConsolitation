...
{
    class BKKUIndicatorsList : List<Indicator>
    {
        ...
        private void MakeResultRow(DataRow row, Indicator indicator)
        {
            ...
            for (int year = 0; year < BKKUIndicatorsService.yearsCount; year++)
            {
                ...
                row[valueColumnName] = indicator.Handler.assessionType != AssessionType.NonAssessed ?
                    FormatedValue(indicator.ResultData[year].value) : "не определено";
                row[formulaValueColumnName] = indicator.FormulaValues[year];
                row[assessRowName] = indicator.ResultData[year].assession;
            }
        }

        private string FormatedValue(double value)
        {
            if (value == 0)
            {

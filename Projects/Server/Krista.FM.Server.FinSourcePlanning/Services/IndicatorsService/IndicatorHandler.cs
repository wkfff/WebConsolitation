...
namespace Krista.FM.Server.FinSourcePlanning.Services.IndicatorsService
{
    public class IndicatorHandler
    {
        ...
        {
            ResultData[] resultData = new ResultData[IndicatorsService.yearsCount];
        ...
        private string CompositionHandlerText()
        {
            ...
            string validationResultText = "if (result.value == double.PositiveInfinity || " +
                                         "result.value == double.NegativeInfinity || " +
                                         "result.value.ToString() == \"NaN\")" +
                                         "{result.value = 0;}";
            string calculateMethodContent = string.Format("ResultData result = new ResultData();{0}{1}result.assession = Assess(result.value, input.assessCritValues);return result;",
                handlerFormulaText, validationResultText);
            return 

...
{
    public class Service2016OKVEDController : SchemeBoundController
    {
        private readonly INewRestService newRestService;

        private readonly Service2016OKVEDViewModel model = new Service2016OKVEDViewModel();

        public Service2016OKVEDController()
        {
        ...
        public RestResult Read(int masterId)
        {
            ...
            {
                Success = true,
                Data = newRestService.GetItems<F_F_ServiceOKVED>()
                        .Where(v => v.RefService.ID == masterId)
                        .Select(
                            v => new Service2016OKVEDViewModel
                            {
                                ID = v.ID,
                                Code = v.Code,
                                Name = v.Name
        ...
        public RestResult Create(string data, int masterId)
        {
            try
            {
                ...
                var formData = JavaScriptDomainConverter<F_F_ServiceOKVED>.DeserializeSingle(data);
        ...
        private string ValidateData(JsonObject record, int masterId)
        {
            ...
            var okved = newRestService.GetItems<F_F_ServiceOKVED>().FirstOrDefault(
            ...
            if (okved != null)
            {
                message.Append(Msg2.FormatWith(okved.Name, okved.Code));
            }

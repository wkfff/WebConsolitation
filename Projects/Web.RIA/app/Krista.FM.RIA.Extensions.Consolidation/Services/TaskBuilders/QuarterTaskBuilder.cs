...
{
    public class QuarterTaskBuilder : ITaskBuilder
    {
        ...
        public IList<D_CD_Task> Build(D_CD_Reglaments reglament, DateTime start, DateTime end)
        {
            ...
            for (int period = start.Year; period <= end.Year; period++)
            {
                ...
                foreach (var subject in subjects)
                {
                    for (int i = 0; i < 4; i++)
                    {
                        var beginDate = new DateTime(period, 1, 1).AddMonths(i * 3);
                        var endDate = new DateTime(period, 1, 1).AddMonths((i + 1) * 3);
                        ...
                        if (beginDate >= start && endDate <= end)
                        {
                                    ...
                                    && x.RefYear == year))
                            {
                                ...
                                {
                                    Name = "{0} за {1} квартал".FormatWith(reglament.RefTemplate.Name, i + 1),

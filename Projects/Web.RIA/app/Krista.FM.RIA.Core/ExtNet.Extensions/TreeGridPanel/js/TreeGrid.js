...
Ext.ux.maximgb.tg.AbstractTreeStore = Ext.extend(Ext.data.Store,
{
    ...
    */
    load: function (options) {
        ...
        if (this.beforeLoadParams) {
            this.beforeLoadParams(this, options);
        }
    ...
    */
    getLoadedNodeIdFromOptions: function (options) {
        var result = null;
        if (options && options.params && options.params[this.paramNames.active_node] > -1) {
...
Ext.ux.maximgb.tg.AdjacencyListStore = Ext.extend(Ext.ux.maximgb.tg.AbstractTreeStore,
{
    ...
    */
    ...
    getNodeChildren: function (rc) {
        ...
        for (i = 0, len = records.length; i < len; i++) {
            //console.log(records[i].get(this.parent_id_field_name), rc.id, records[i].get(this.parent_id_field_name) == rc.id);
            if (records[i].get(this.parent_id_field_name) && records[i].get(this.parent_id_field_name) == rc.id) {
                result.push(records[i]);
            }
        }
...
Ext.ux.maximgb.tg.GridView = Ext.extend(Ext.grid.GridView,
{
    ...
    collapsed_icon_class_line: 'ux-maximgb-tg-elbow-plus_line',
    ...
    initTemplates: function () {
            ...
            ts.mastercell = new Ext.Template(
                '<td class="x-grid3-col x-grid3-cell x-grid3-td-{id} {css}" style="{style}" tabIndex="0" {cellAttr}>',
                    '<div class="ux-maximgb-tg-mastercell-wrap">', // This is for editor to place itself right
                        '{treeui}',
                        '<div style="left: {left}px; position: relative; " class="x-grid3-cell-inner x-grid3-col-{id}" unselectable="on" {attr}>{value}</div>',
                    '</div>',
                '</td>'
            );
        }
        ...
        //        '<div class="ux-maximgb-tg-uiwrap" style="width: {wrap_width}px; height: 100%">',
        //            '{elbow_line}',
        //            '<div style="left: {left}px" class="{cls}">&#160;</div>',
        //            '<div style="left: {left}px; height: 33px; width: {width_line}px;" class="ux-maximgb-tg-elbow-plus_line">&#160;</div>',
        //        '</div>'
        //    );
        //}
        if (!ts.treeui) {
        ...
        if (store.isLeafNode(record)) {
            ...
            if (store.isExpandedNode(record)) {
                ...
                if (store.hasNextSiblingNode(record)) {
                    ...
                    tpl_data.width_line = 15;
                }
                else {
                    tpl_data.cls += this.last_collapsed_icon_class;
                    tpl_data.width_line = 0;
                }
            }
        }
...
Ext.ux.maximgb.tg.EditorGridPanel = Ext.extend(Ext.net.GridPanel,
{
    /**
    * @cfg {String|Integer} master_column_id Master column id. Master column cells are nested.
    * Master column cell values are used to build breadcrumbs.
...
Ext.ux.maximgb.tg.PagingToolbar = Ext.extend(Ext.PagingToolbar,
{
    ...
    getPageData: function () {
        ...
        if (this.store) {
            cursor = this.store.getPageOffset();
    ...
    updateInfo: function () {
        ...
        if (this.displayItem) {
            if (this.store) {
                cursor = this.store.getPageOffset();
                count = this.store.getRootNodes().length;
                total = this.store.getTotalCount();
            }
            msg = count == 0 ?
                this.emptyMsg
    ...
    */
    movePrevious: function () {
        var store = this.store,
            cursor = store ? store.getPageOffset() : 0;
    ...
    */
    moveLast: function () {
        var store = this.store,
            cursor = store ? store.getPageOffset() : 0,
            total = store ? store.getTotalCount() : 0,

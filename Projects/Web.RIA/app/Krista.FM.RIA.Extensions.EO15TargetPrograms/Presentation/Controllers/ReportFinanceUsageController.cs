...
{
    public class ReportFinanceUsageController : SchemeBoundController
    {
        private readonly IProgramService programService;
        private readonly IActionService actionService;
        private readonly IFinanceService financeService;

        public ReportFinanceUsageController(
        ...
        public ActionResult GetReportTable(int programId, int year)
        {
            ...
            var data = this.financeService.GetReportTable(program, year);
        ...
        public AjaxFormResult SaveReportTable(int programId, int year, string storeChangedData)
        {
            ...
            try
            {
                ...
                foreach (var rowData in data.Updated)
                {
                    ...
                    {
                        throw new Exception("Не указано id мероприятия/источника_финансирования");
                    }

                    int financePlanId = Convert.ToInt32(rowData["ID"]);

                    Dictionary<int, decimal?> monthFactList = GetMonthFactList(rowData);

                    financeService.SaveReportFactData(programId, financePlanId, monthFactList);
                }

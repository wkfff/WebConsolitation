...
{
    public partial class Food_0001_0002_Chart : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            Label1.Text = GetDescription(6, 11);
            Label2.Text = GetDescription(7, 12);
            Label3.Text = GetDescription(8, 13);
            Label4.Text = GetDescription(9, 14);
            Label5.Text = GetDescription(10, 15);
        }

        private string GetDescription(int grownIndex, int absoluteGrownIndex)
        {
            string img;
            if (source[grownIndex].ToString().Contains("-"))
            {
            ...
            return String.Format("<span style='Color: White'>{0}</span><br/>{1} руб.<br/>{2}&nbsp;{3:N2}%", source.Table.Columns[absoluteGrownIndex].ColumnName.Split(';')[1].Replace("Город ", "").Replace("город ", "").Replace("Великий", "В.").Replace("муниципальный район", "МР"), absoluteGrown, img, source[grownIndex]);
        }

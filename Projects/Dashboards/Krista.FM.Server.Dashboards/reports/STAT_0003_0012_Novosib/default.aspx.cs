 ...
 */
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0012_Novosib
{
    public partial class Default : CustomReportPage
    {
        ...
        class CoolDate
        {
            ...
            public string ConvertToMDXName(bool mounth)
            {
                if (mounth)
                {
        ...
        private void SetQueryParam()
        {
            ...
            ChosenPrevPeriod.Value = new CoolDate(CurDate.dt.AddMonths(-1)).ConvertToMDXName(true) + ".[Заключительные обороты]";
        ...
        class InfoRow
        {
            ...
            public InfoRow(string BaseName)
            {
                switch (BaseName)
                {
                    ...
                    case "Среднесписочная численность работников крупных и средних организаций, в % к численности трудовых ресурсов":
                        {
                            ReportDisplay = "Среднесписочная численность работников крупных и средних организаций, в % к численности трудовых ресурсов";

                            Caption = "Среднесписочная численность работников крупных и средних организаций";
                            EdIzm = "в % к численности трудовых ресурсов";
        ...
        {
            PageTitle.Text = "Востребованность трудовых ресурсов (рейтинг территорий)";
            PageSubTitle.Text = string.Format("Данные мониторинга численности трудовых ресурсов муниципальных районов и городских округов Новосибирской области по состоянию на {0}", CurDate.ReportDate());
            Page.Title = PageTitle.Text;

            LabelMap2.Text = string.Format("«{0}», {1} на {2}", 
                GlobalNavigationInfoRow.Caption, 
                GlobalNavigationInfoRow.EdIzm, 
                CurDate.ReportDate());

            LabelChart1.Text = string.Format("Распределение территорий по показателю «{0}», {1} на {2}", 
                GlobalNavigationInfoRow.Caption,
                GlobalNavigationInfoRow.EdIzm,
                CurDate.ReportDate());  
        }
        ...
        private void FillNaviagatorPanel()
        {
            ...
            TableListForChart.AddOrGetRow("Численность занятых индивидуальным трудом и по найму у отдельных граждан, в % к численности трудовых ресурсов")["Uname"]
                = "[Показатели__Рынок труда].[Показатели__Рынок труда].[Все].[Численность занятых индивидуальным трудом и по найму у отдельных граждан, в % к численности трудовых ресурсов]";
            TableListForChart.AddOrGetRow("Численность граждан, занятых в домашнем хозяйстве производством товаров и услуг для реализации, в % к численности трудовых ресурсов")["Uname"]
                = "[Показатели__Рынок труда].[Показатели__Рынок труда].[Все].[Численность граждан, занятых в домашнем хозяйстве производством товаров и услуг для реализации, в % к численности трудовых ресурсов]";
            TableListForChart.AddOrGetRow("Трудоспособное население в трудоспособном возрасте, не занятое в экономике и обучением, в % к численности трудовых ресурсов")["Uname"]
                = "[Показатели__Рынок труда].[Показатели__Рынок труда].[Все].[Трудоспособное население в трудоспособном возрасте, не занятое в экономике и обучением, в % к численности трудовых ресурсов]";

            TableListForChart.AddOrGetRow("Среднесписочная численность работников крупных и средних организаций, в % к численности трудовых ресурсов")["Uname"]
                = "[Показатели__Рынок труда].[Показатели__Рынок труда].[Все].[Среднесписочная численность работников крупных и средних организаций, в % к численности трудовых ресурсов]";
        ...
        private void CustomaizeMapko()
        {
            ...
            legend.Title = SetBr(NaviagatorChart.ActiveRow[0].ToString(), 20);
        ...
        {   
            ...
            try
            {
                if (TableFOValue.Rows.Count > 0)
                {
                    decimal FoValue = (decimal)TableFOValue.Rows[0][1];

                    LastValuyFromChart = FoValue > LastValuyFromChart ? FoValue : LastValuyFromChart;
                }
            }
            catch { }
            ...
            UltraChart1.Tooltips.FormatString = string.Format("<SERIES_LABEL><br><b><DATA_VALUE:N2></b>, {0}", GlobalNavigationInfoRow.EdIzm);
        ...
        protected void GenHorizontalLineAndLabel(int startX, int StartY, int EndX, int EndY, Color ColorLine, string Label, Infragistics.UltraChart.Shared.Events.FillSceneGraphEventArgs e, bool TextUP)
        {
            ...
            if (TextUP)
            {
                textLabel.bounds = new System.Drawing.Rectangle(startX + 50, StartY - 17, 800, 15);
            }
            else
            {
        ...
        void Chart_FillSceneGraph(object sender, Infragistics.UltraChart.Shared.Events.FillSceneGraphEventArgs e)
        {
            ...
            try
            {
                ...
                try
                {
                    ...
                    {
                        RFheader = string.Format("{0}: {1:N2}, {2}", "Российская федерация", TableRFValue.Rows[0][1], GlobalNavigationInfoRow.EdIzm);
        ...
        private void ExcelExportButton_Click(object sender, EventArgs e)
        {
            ...
            sheet2.Columns[0].Width = 100 * 1000;            

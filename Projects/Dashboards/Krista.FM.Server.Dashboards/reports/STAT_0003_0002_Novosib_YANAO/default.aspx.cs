 ...
 */
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0002_Novosib_YANAO
{
	public partial class Default : CustomReportPage
	{
        ...
        private const string PageSubTitleCaption = "Данные еженедельного мониторинга ситуации на рынке труда в Ямало-Ненецком автономном округе по выбранному муниципальному образованию";
        private const string Chart1TitleCaption = "Динамика уровня регистрируемой безработицы, {0}";
        private const string Chart2TitleCaption = "Динамика уровня напряжённости на рынке труда, {0}";
		...
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			UltraWebGrid.InitializeRow += 
                new Infragistics.WebUI.UltraWebGrid.InitializeRowEventHandler(UltraWebGrid_InitializeRow);
			...
			#region Ссылки
            CrossLink1.Text = "Мониторинг ситуации на рынке<br>труда Ямало-Ненецкого автономного округа".Replace(" ", "&nbsp;");
			CrossLink1.NavigateUrl = "~/reports/STAT_0003_0001_Novosib_YANAO/Default.aspx";
			#endregion
		...
		protected void MakeDynamicText()
		{
            try
            {
                ...
                string text1 = "Численность официально зарегистрированных безработных, по состоянию на <b>{0}</b> составила <b>{1:N0}</b> человек. {2} числа безработных граждан за период с <b>{3}</b> по <b>{0}</b> в целом по выбранному муниципальному образванию составил{4} <b>{5:N0}</b> чел. (темп {6}<b>{7:P2}</b>).";
                ...
                try
                {
                    ...
                    values1[1] = Math.Abs(Convert.ToDouble(row0[1].ToString()));
                ...
                try
                {
                    ...
                    values2[1] = Math.Abs(Convert.ToDouble(row1[3-1].ToString()));
                    ...
                    values3[2] = Math.Abs(Convert.ToDouble(row0[1].ToString()) / Convert.ToDouble(dtText.Rows[0][1]) / 1000);
		...
		{//Grid_Novosib
			...
			foreach (DataRow row in dtGridDescription.Rows)
			{
                ...
                try
                {
                    row[1] = MDXDateToShortDateString(PrevRow[1].ToString());
                }
                catch
                {
                    row[1] = DBNull.Value;
			...
			for (int i = 1; i < e.Row.Cells.Count; ++i)
			{
				...
				if (e.Row.Index % 3 == 1 && i != 6)
				{
					...
					if (cell.Value != null && Convert.ToDouble(cell.Value) != 0)
					{
                        ...
                        cell.Text = string.Format("{0:P2}", value);
						if (value < 0)
						{
                            if (i == 2 || i == 3 || i == 4)//|| i == 6
							{
				...
				if ((i == 2 || i == 5) && (e.Row.Index % 3 == 2))
				{
					...
					if (Convert.ToInt32(cell.Value) == 1)
					{
                        ...
                        if (i == 2)
                        {
                            cell.Title = String.Format("Ранг по Ямало-Ненецкому автономному округу на {1}: {0} \nСамый низкий уровень регистрируемой безработицы", cell.Value, date);
                        }
                        else
                        {
					...
					{
                        cell.Title = String.Format("Ранг по Ямало-Ненецкому автономному округу на {1}: {0}", cell.Value, date);
					}
				}
		...
		private void ExcelExportButton_Click(object sender, EventArgs e)
		{
            ...
            headerLayout.childCells.RemoveAt(6);
            headerLayout.childCells.RemoveAt(3);
            ...
            try
            {
                sheet1.MergedCellsRegions.Add(1, 0, 1, 8);
                sheet1.MergedCellsRegions.Add(2, 0, 2, 8);
            }
            catch { }
			...
			UltraChart2.Height = CRHelper.GetChartHeight(CustomReportConst.minScreenHeight * 0.7);
			ReportExcelExporter1.Export(UltraChart2, LabelChart2.Text, sheet3, 1);
            sheet2.Columns[0].Width = 30 * 2000;
            sheet3.Columns[0].Width = 30 * 2000;
		}

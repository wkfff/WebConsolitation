...
namespace Krista.FM.Server.Dashboards.reports.FK_0004_0006
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            UserParams.Filter.Value = ShowAll.Checked ? "SELF_AND_AFTER" : "SELF";


            if (!Page.IsPostBack)
            {
                CustomCalendar1.WebCalendar.SelectedDate = CubeInfo.GetLastDate(DataProvidersFactory.SecondaryMASDataProvider, "FK_0004_0006_lastDate");
            }

            currentDate = CustomCalendar1.WebCalendar.SelectedDate;

            Page.Title = String.Format("Анализ показателей исполнения федеральных целевых программ ");
        ...
        private void BindInfoText()
        {
            ...
            lbInfo.Text = String.Format(@"<div>В Российской Федерации действуют <b>59</b> целевых программ на общую сумму по уточненной бюджетной росписи <b>{7:N2}</b> млн.руб.<br>
По состоянию на <b>{8:dd.MM.yyyy}</b> года:<br/>
•	<b>доведено бюджетных ассигнований</b> на {0:yyyy} год по федеральным целевым программам <b>{1:N2}</b> млн. руб. (<b>{2:N2}%</b>  от уточненной бюджетной росписи);<br/>
•	<b>распределено бюджетных ассигнований</b> на {0:yyyy} год по федеральным целевым программам <b>{3:N2}</b> (<b>{4:N2}%</b>   от уточненной бюджетной росписи);<br/>
•	<b>фактически исполнено</b> по федеральным целевым программам <b>{5:N2}</b> млн.руб. (<b>{6:N2}%</b>  от уточненной бюджетной росписи).</div>",
                currentDate,
                dtInfo.Rows[0][0],
                dtInfo.Rows[0][1],
                dtInfo.Rows[0][2],
...
<img src='../../images/StarGrayBB.png'>&nbsp;<b>Минимальный</b> процент исполнения:", lbInfo.Text);
            ...
            for (int i = 0; i < 5; i++)
            {
                lbInfo.Text = String.Format(@"{0}<br/>
 <b>{1:N2}%</b> {2}", lbInfo.Text, dtInfo.Rows[i][2], dtInfo.Rows[i][1].ToString().Replace("Федеральная целевая программа", "ФЦП"));
            }
 ...
 <b>{1:N2}%</b> {2}", lbInfo.Text, dtInfo.Rows[i][2], dtInfo.Rows[i][1].ToString().Replace("Федеральная целевая программа", "ФЦП"));
        ...
        private void GridDataBind()
        {
            ...
            foreach (DataRow row in gridDt.Rows)
            {
                ...
                newRow[0] = row[0];
                newRow[1] = row[1].ToString().Replace("Федеральная целевая программа", "ФЦП");

                newRow[2] = row[2];
                double.TryParse(row[4].ToString(), out value);
                newRow[3] = String.Format("<span style='font-size: 10pt'>{0:N0}</span><br/>{2}<br/>{1:N2}%", row[3], row[4], GetGaugeUrl(value, "Процент доведенных БА от уточненной росписи, %"));
                double.TryParse(row[6].ToString(), out value);
                newRow[4] = String.Format("<span style='font-size: 10pt'>{0:N0}</span><br/>{2}<br/>{1:N2}%", row[5], row[6], GetGaugeUrl(value, "Хинт к линейке - Процент распределенных БА от доведенных БА, %"));
        ...
        void WebDataGrid1_InitializeRow(object sender, Infragistics.Web.UI.GridControls.RowEventArgs e)
        {
            if (ShowAll.Checked)
            {
                CRHelper.SaveToErrorLog("ShowAll.Checked");
                if (e.Row.Items[e.Row.Items.Count - 1].Value != DBNull.Value &&
                e.Row.Items[e.Row.Items.Count - 1].Value.ToString() == "Программа")
                {
                    e.Row.CssClass = "levelOne";
                }

                if (e.Row.Items[e.Row.Items.Count - 1].Value != DBNull.Value &&
                e.Row.Items[e.Row.Items.Count - 1].Value.ToString() == "Подпрограмма")
                {
                    e.Row.CssClass = "levelTwo";
                }
            }
        ...
        protected void UltraWebGrid_InitializeLayout(WebDataGrid e, DataTable dtGrid)
        {
            ...
            SetHeaderCaption(e, 0, "Код КЦСР");
            SetHeaderCaption(e, 1, "Наименование программы");
            SetHeaderCaption(e, 2, String.Format("Уточненная роспись на {0:yyyy}г., млн.руб", currentDate));
            SetHeaderCaption(e, 3, String.Format("Доведенные бюджетные ассигнования на {0:yyyy}г., млн.руб", currentDate));
            SetHeaderCaption(e, 4, String.Format("Распределенные бюджетные ассигнования  на {0:yyyy}г., млн.руб", currentDate));

 ...
 */
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0001_Novosib
{
	public partial class Default : CustomReportPage
	{
		...
		private const string mapFolderName = "Субъекты\\Новособл";
        ...
        private const string PageSubTitleCaption = "Данные ежемесячного мониторинга ситуации на рынке труда в Новосибирской области по состоянию на {0}";
        ...
        private const string Chart1TitleCaption = "Соотношение числа заявленных вакансий и численности незанятых граждан, состоящих на учете, по Новосибирской области";
		...
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			#region Ссылки
            CrossLink1.Text = "Мониторинг&nbsp;ситуации&nbsp;на&nbsp;рынке&nbsp;труда&nbsp;<br>(по&nbsp;муниципальным&nbsp;образованиям&nbsp;Новосибирской&nbsp;области)";
			CrossLink1.NavigateUrl = "~/reports/STAT_0003_0002_Novosib/Default.aspx";
			#endregion
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
            ...
            item.Text = "Численность незанятых граждан, состоящих на учете, человек";
		...
		protected void MakeDynamicText()
		{
            ...
            string text1 = "Численность официально зарегистрированных безработных, по состоянию на <b>{0}</b> составила <b>{1:N2}</b> тыс.человек. {2} числа безработных граждан за период с <b>{3}</b> по <b>{0}</b> в целом по Новосибирской области составил{4} <b>{5:N0}</b> чел. (темп {6}<b>{7:P2}</b>).";
            ...
            try
            {
                ...
                if (row1[3] != DBNull.Value)
                {
                    ...
                    values2[1] = //Convert.ToDouble(row0[3].ToString()) +
                         Math.Abs(Convert.ToDouble(row0[3].ToString())*
                         Convert.ToDouble(row1[3].ToString())); 
                    //Math.Abs(Convert.ToDouble(row2[3].ToString()))*100;
        ...
        class RankingField
        {
            ...
            {                
                ...
                foreach (KeyVal kv in Fields)
                {
                    ...
                    {                        
                        return !Reverce?i:Fields.Count - i+1;
                    }
                }
                return DBNull.Value;
            }
        ...
        {   
            ...
            for (int i = 1; i < Table.Columns.Count; i+=2)
            {
                ...
                {                    
                    DataRow RowValue = ReportTable.AddOrFindRow(BaseRow[0].ToString() + ";Значение");
                    DataRow RowDeviation = ReportTable.AddOrFindRow(BaseRow[0].ToString() + ";Отклонение");
                    DataRow RowSpeedDeviation = ReportTable.AddOrFindRow(BaseRow[0].ToString() + ";Прирост");
            ...
            foreach (DataRow Row in ReportTable.Rows)
            {
                if (j != 3)
                {
                    if (j % 3 == 0)
                    {
                        Row[3] = Ranked.GetRang(Row[0].ToString(),true);
                        Row[6] = Ranked1.GetRang(Row[0].ToString(), true);
        ...
        private void ClearDataMember(DataTable dtGrid)
        {
            foreach (DataRow Row in dtGrid.Rows)
            {
                if (Row[0].ToString().Contains("ДАННЫЕ)"))
                {
                    Row[0] = Row[0].ToString().Replace("(", "").Replace("ДАННЫЕ)", "");
                }
            }
        }
		...
		protected void UltraWebGrid_InitializeLayout(object sender, Infragistics.WebUI.UltraWebGrid.LayoutEventArgs e)
		{
            ...
            if (CustomizerSize is CustomizerSize_800x600)
            {
                //
            //    columnWidth = 140;
            //        //Convert.ToInt32((Convert.ToInt32(e.Layout.Grid.Width.Value) - CRHelper.GetColumnWidth(00)) / 8);
		...
		protected void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
		{
			...
			for (int i = 2; i < e.Row.Cells.Count+1; ++i)
            {
                try
                {
                    ...
                    {
                        if (Convert.ToInt32(cell.Value) == ((dtGrid.Rows.Count / 3) - 1))
                        ...
                        else if (Convert.ToInt32(cell.Value) == 1)
                        {
                        ...
                        {
                            cell.Title = String.Format("Ранг по Новосибирской области: {0}", cell.Value);
                        }
                    }
		...
		protected void UltraChart1_DataBinding(object sender, EventArgs e)
		{
			...
			if (dtChart != null)
			{
                ...
                dtChart.Rows[0][0] = "Численность незанятых граждан, состоящих на учете, человек";

...
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0001
{
	public partial class Default : CustomReportPage
	{
		...
		private const string PageSubTitleCaption = "Данные еженедельного мониторинга ситуации на рынке труда в ХМАО-Югре по состоянию на {0}";
		private const string Map1TitleCaption = "Уровень зарегистрированной безработицы по муниципальным образованиям на {0}";
		private const string Map2TitleCaption = "Численность незанятых граждан, состоящих на учёте в органах службы занятости населения, на {0}, человек";
		private const string Chart1TitleCaption = "Соотношение числа вакансий и численности безработных, зарегистрированных в органах службы занятости населения, по ХМАО-Югра";
		...
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			#region Ссылки
			CrossLink1.Text = "Мониторинг&nbspситуации&nbspна&nbspрынке&nbspтруда&nbsp(по&nbspмуниципальным&nbspобразованиям ХМАО-Югры)";
			CrossLink1.NavigateUrl = "~/reports/STAT_0003_0002/Default.aspx";
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			legend1.Title = "Численность зарегистрированных безработных в расчете на 1 вакансию";
			...
			legend2.Font = new Font("MS Sans Serif", 7, FontStyle.Regular);
			legend2.AutoFitText = true;
			legend2.Title = "Соотношение числа зарегистрированных безработных и числа вакансий";
			legend2.AutoFitMinFontSize = 7;
			...
			item.Text = "Численность безработных граждан, зарегистрированных в органах службы занятости населения";
			...
			AddMapLayer(DundasMap2, mapFolderName, "Территор", CRHelper.MapShapeType.Areas);
		...
		private void Years()
		{
			...
			DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "Годы", dtYears);
		...
		protected void MakeDynamicText()
		{
			...
			string text1 = "Численность безработных граждан, зарегистрированных в органах службы занятости, по состоянию на <b>{0}</b> составила <b>{1:N3}</b> тыс.человек. {2} числа безработных граждан за период с <b>{3}</b> по <b>{0}</b> в целом по ХМАО-Югре составил{4} <b>{5:N0}</b> чел. (темп {6}<b>{7:P2}</b>).";
            ...
            string text3 = "<br/>Численность населения на начало года составила <b>{0:N3}</b> тыс.чел., численность экономически активного населения составила <b>{1:N3}</b> тыс.чел. (<b>{2:P3}</b> от численности населения).";
			...
			if (row1[4] != DBNull.Value)
			{
				...
				values2[1] = Convert.ToDouble(row1[4].ToString());
			...
			foreach (DataRow row in dtMap1.Rows)
			{
				...
				foreach (Shape shape in shapeList)
				{
					...
					if (IsCalloutTownShape(shape))
					{
						...
						shape.TextVisibility = TextVisibility.Shown;
						shape.Text = String.Format("{0}\n{1:N0} чел.\n{2:N2}%", row[1].ToString(), Convert.ToDouble(row[3].ToString()), Convert.ToDouble(row[4].ToString()));
						shape.TextVisibility = TextVisibility.Shown;
		...
		public void FillMapData2(MapControl map)
		{
			...
			DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "Показатель", dtMap2);
					...
					row[2] != DBNull.Value && row[2].ToString() != string.Empty)
				{
					...
					foreach (Shape shape in shapeList)
					{
						...
						else
						{
							...
							if (!IsCalloutTownShape(shape))
							{
								shape.Text = string.Format("{0}\nвакансий: {2:N0}\n{1:N1}", row[4].ToString(), tensionKoeff, vacancyCount);
								Symbol symbol = new Symbol();
		...
		protected void UltraWebGrid_InitializeLayout(object sender, Infragistics.WebUI.UltraWebGrid.LayoutEventArgs e)
		{
			...
			for (int i = 1; i < e.Layout.Bands[0].Columns.Count; ++i)
			{
                ...
                string HeaderCaptionField = e.Layout.Bands[0].Columns[i].Key;
				GridHeaderCell headerCell = headerLayout.AddCell(HeaderCaptionField);

                if (HeaderCaptionField.Contains("Численность незанятых граждан, состоящих на учёте в органах службы занятости населения, человек"))
                {
                    headerCell.AddCell("01"+MDXDateToShortDateString(selectedDate.Value).Remove(0,2));
                }
                else
				if (i != 2)
				{
			...
			for (int i = 1; i < e.Row.Cells.Count; ++i)
			{
				...
				if ((i == 4 || i == 7) && (e.Row.Index != 2) && (e.Row.Index % 3 == 2))
				{
					if (Convert.ToInt32(cell.Value) == 1)
					{
                        ...
                        if (i == 4)
                        {
                            cell.Title = String.Format("Ранг по ХМАО-Югре: {0}\nСамый низкий уровень безработицы", cell.Value);
                        }
                        else
                        {
					...
					{
						cell.Title = String.Format("Ранг по ХМАО-Югре: {0}", cell.Value);
					}
				}
		...
		protected void UltraChart1_DataBinding(object sender, EventArgs e)
		{
			...
			DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "Параметры", dtChart);
			if (dtChart != null)
			{
				query = DataProvider.GetQueryText("STAT_0003_0001_chart_description");
				DataTable dtChartDescription = new DataTable();
				DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "Описание столбцов", dtChartDescription);
		...
		private void ExcelExportButton_Click(object sender, EventArgs e)
		{
			...
			DundasMap2.Width = Unit.Pixel((int)(CustomReportConst.minScreenWidth * 0.6));

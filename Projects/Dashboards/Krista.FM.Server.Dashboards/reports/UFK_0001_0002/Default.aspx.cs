...
namespace Krista.FM.Server.Dashboards.reports.UFK_0001_0002
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string query = DataProvider.GetQueryText("UFK_0001_0002_date");
            ...
            Page.Title = "Фактическое поступление доходов в бюджеты всех уровней бюджетной системы РФ";
            Label1.Text = Page.Title;
            if (ComboRegion.SelectedValue == "Все территории")
            {
                selectedRegion.Value = string.Format("{0}.[Все районы].[Консолидированный бюджет Республики Карелия].[Районы]", RegionSettingsHelper.Instance.RegionDimension);
            }
            else
            {
            ...
            Label2.Text = String.Format("{1}, данные по состоянию на {0} года, тыс.руб.", date, ComboRegion.SelectedValue);
        ...
        protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
        {
                ...
                string query = DataProvider.GetQueryText("UFK_0001_0002_compare_grid");
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            string date = String.Format("{0:dd.MM.yyyy}", currDateTime);
              ...
              for (int i = 2; i < e.Layout.Bands[0].Columns.Count - 3; i += 3)
              {
                  ...
                  e.Layout.Bands[0].Columns[i].Header.Caption = string.Format("На {0} года, тыс. руб.", date);
                  currDateTime = currDateTime.AddYears(1);
                  date = String.Format("{0:dd.MM.yyyy}", currDateTime);
                  e.Layout.Bands[0].Columns[i + 1].Header.Caption = string.Format("На {0} года, тыс. руб.", date);
                  e.Layout.Bands[0].Columns[i + 2].Header.Caption = string.Format("Темп роста к прошлому году,%");
              }
              try
              {
                  e.Layout.Bands[0].Columns[0].Header.Title = "Наименование доходов";
                  e.Layout.Bands[0].Columns[1].Header.Title = "Код бюджетной классификации доходов";
                  e.Layout.Bands[0].Columns[2].Header.Title = "Поступило доходов в прошлом году в бюджеты всех уровней";
                  e.Layout.Bands[0].Columns[3].Header.Title = "Поступило доходов в текущем году в бюджеты всех уровней";
                  e.Layout.Bands[0].Columns[4].Header.Title = "Темп роста поступлений к аналогичному периоду прошлого года";
                  e.Layout.Bands[0].Columns[5].Header.Title = "Поступило доходов в прошлом году в бюджет Российской Федерации";
                  e.Layout.Bands[0].Columns[6].Header.Title = "Поступило доходов в текущем году в бюджет Российской Федерации";
                  e.Layout.Bands[0].Columns[7].Header.Title = "Темп роста поступлений к аналогичному периоду предыдущего года";
                  e.Layout.Bands[0].Columns[8].Header.Title = "Поступило доходов в бюджет Республики Карелия в прошлом году";
                  e.Layout.Bands[0].Columns[9].Header.Title = "Поступило доходов в бюджет Республики Карелия в текущем году";
                  e.Layout.Bands[0].Columns[10].Header.Title = "Темп роста поступлений к аналогичному периоду предыдущего года";
                  e.Layout.Bands[0].Columns[11].Header.Title = "Поступило доходов в бюджеты муниципальных образований в прошлом году";
                  e.Layout.Bands[0].Columns[12].Header.Title = "Поступило доходов в бюджеты муниципальных образований в текущем году";
                  e.Layout.Bands[0].Columns[13].Header.Title = "Темп роста поступлений к аналогичному периоду предыдущего года";
                  e.Layout.Bands[0].Columns[14].Header.Title = "Поступило доходов в государственные внебюджетные фонды в прошлом году";
                  e.Layout.Bands[0].Columns[15].Header.Title = "Поступило доходов в государственные внебюджетные фонды в текущем году";
                  e.Layout.Bands[0].Columns[16].Header.Title = "Темп роста поступлений к аналогичному периоду предыдущего года";
                  e.Layout.Bands[0].Columns[17].Header.Title = "Поступило доходов в бюджеты других уровней в прошлом году";
                  e.Layout.Bands[0].Columns[18].Header.Title = "Поступило доходов в бюджеты других уровней в текущем году";
                  e.Layout.Bands[0].Columns[19].Header.Title = "Темп роста поступлений к аналогичному периоду предыдущего года";

                  int spanOX = 2;
                  int spanSX = 3;
                  ColumnHeader ch = new ColumnHeader(true);
                  ch.Caption = "Поступило всего нарастающим итогом с начала года";
                  ch.Title = "";
                  ...
                  ch.Caption = "в бюджет Российской Федерации";
                  ch.Title = "";
                  ch.RowLayoutColumnInfo.OriginY = 0;
                  ch.RowLayoutColumnInfo.OriginX = spanOX;
                  ch.RowLayoutColumnInfo.SpanX = spanSX;
                  ...
                  spanOX = spanSX + spanOX;
                  ch = new ColumnHeader(true);
                  ch.Caption = "в бюджет Республики Карелия";
                  ch.Title = "";
                  ch.RowLayoutColumnInfo.OriginY = 0;
                  ch.RowLayoutColumnInfo.OriginX = spanOX;
                  ch.RowLayoutColumnInfo.SpanX = spanSX;
                  ...
                  spanOX = spanSX + spanOX;
                  ch = new ColumnHeader(true);
                  ch.Caption = "в бюджеты муниципальных образований";
                  ch.Title = "";
                  ch.RowLayoutColumnInfo.OriginY = 0;
                  ch.RowLayoutColumnInfo.OriginX = spanOX;
                  ch.RowLayoutColumnInfo.SpanX = spanSX;
                  ...
                  spanOX = spanSX + spanOX;
                  ch = new ColumnHeader(true);
                  ch.Caption = "в государственные внебюджетные фонды";
                  ch.Title = "";
                  ch.RowLayoutColumnInfo.OriginY = 0;
                  ch.RowLayoutColumnInfo.OriginX = spanOX;
                  ch.RowLayoutColumnInfo.SpanX = spanSX;
                  ...
                  spanOX = spanSX + spanOX;
                  ch = new ColumnHeader(true);
                  ch.Caption = "в бюджеты других уровней";
                  ch.Title = "";
                  ch.RowLayoutColumnInfo.OriginY = 0;
                  ch.RowLayoutColumnInfo.OriginX = spanOX;
                  ch.RowLayoutColumnInfo.SpanX = spanSX;
        ...
        private void ExcelExporter_HeaderCellExporting(object sender, HeaderCellExportingEventArgs e)
        {
                ...
                {
                    e.HeaderText = "Поступило всего нарастающим итогом с начала года";
                }
                else
                    if ((e.CurrentColumnIndex > 4) && (e.CurrentColumnIndex < 8))
                    {
                        e.HeaderText = "в бюджет Российской Федерации";
                    }
                    else
                        if ((e.CurrentColumnIndex > 7) && (e.CurrentColumnIndex < 11))
                        {
                            e.HeaderText = "в бюджет Республики Карелия";
                        }
                        else
                            if ((e.CurrentColumnIndex > 8) && (e.CurrentColumnIndex < 14))
                            {
                                e.HeaderText = "в бюджеты муниципальных образований";
                            }
                            else
                                if ((e.CurrentColumnIndex > 11) && (e.CurrentColumnIndex < 17))
                                {
                                    e.HeaderText = "в государственные внебюджетные фонды";
                                }
                                else
                                    if ((e.CurrentColumnIndex > 14) && (e.CurrentColumnIndex < 20))
                                    {
                                        e.HeaderText = "в бюджеты других уровней";
                                    }

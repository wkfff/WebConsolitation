...
namespace Krista.FM.Server.Dashboards.reports.FO_0039_0002_Kostroma
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            Page.Title = String.Format("Динамика результатов оценки качества организации и осуществления бюджетного процесса в МР(ГО) Костромской области");
        ...
        {
            string measureStr = ValueSelected ? "Значение по состоянию" : "Оценка качества по состоянию";
            switch (quarterIndex)
            {
                case 1:
                    {
                        return String.Format("{0} на 01.04.{1}", measureStr, year);
                    }
                case 2:
                    {
                        return String.Format("{0} на 01.07.{1}", measureStr, year);
                    }
                case 3:
                    {
                        return String.Format("{0} на 01.10.{1}", measureStr, year);
                    }
                case 4:
                    {
                        return String.Format("{0} по итогам {1} года", measureStr, year);
                    }
                default:
                    {
                        return string.Empty;
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            {
                bool intValue = e.Layout.Bands[0].Columns[i].Header.Caption.Contains("Ранг");
            ...
            for (int i = 1; i < lastIndicatorIndex; i = i + 3)
            {
                ...
                if (caption != String.Empty)
                {
                    ...
                    indicatorCell.AddCell("Отклонение", indicatorUnit);
                }
            }
            ...
            if (!ValueSelected)
            {
                ...
                for (int i = 0; i < 3; i++)
                {
                    ...
                    indicatorCell.AddCell("Отклонение", 2);
                }
            }

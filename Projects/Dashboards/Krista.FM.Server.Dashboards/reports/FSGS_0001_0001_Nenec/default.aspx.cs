 ...
 */
namespace Krista.FM.Server.Dashboards.reports.FSGS_0001_0001_Nenec
{
	public partial class Default : CustomReportPage
	{
		...
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			UltraChart1.Height = CRHelper.GetChartHeight(CustomReportConst.minScreenHeight * 0.50000001);
            ...
            firstDate = UserParams.CustomParam("first_date", true);
            regionsForChart = UserParams.CustomParam("regions_for_chart", true);
            ...
            UltraChart1.Height = pageHeight - 50;
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
            ...
            PageSubTitle.Text = String.Format(PageSubTitleCaption, ComboRegion.SelectedValue,ComboDate.SelectedValue);
		...
		protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
		{
			...
			DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование продукта", dtGrid);
		...
		{
			string prevDate = dtGrid.Columns[cell.Column.Index - 1].Caption;
		...
		protected void FillComboRegion(string queryName)
		{
			...
			if (dict.ContainsKey("Уральский федеральный округ"))
		...
		private void SetDynamicText(string string_date)
		{
			...
			if ((dictUp.Count == 0) && (dictDown.Count == 0))
			{
				if (hasData)
				{
                    head = String.Format("&nbsp;&nbsp;&nbsp;По состоянию на <b>{0}</b> не наблюдалось изменение розничных цен на основные товары (услуги)",
						string_date);
				}
				else
				{
                    head = String.Format("&nbsp;&nbsp;&nbsp;По состоянию на <b>{0}</b> данные об изменении розничных цен на товары (услуги) отсутствуют.",
						string_date);
				}
			}
			else
			{
                head = String.Format("&nbsp;&nbsp;&nbsp;По состоянию на <b>{0}</b> наблюдалось изменение розничных цен на товары (услуги):",
					string_date);
			}

...
namespace Krista.FM.Server.Dashboards.reports.FO_0039_0004
{
    public partial class Default : CustomReportPage
    {
        ...
        private int rateChartBeginIndex;
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string query = DataProvider.GetQueryText("FO_0039_0004_date");
            ...
            Page.Title = String.Format("Сравнительная характеристика максимальных и минимальных оценок показателей по МР Омской области");
        ...
        {
            string query = DataProvider.GetQueryText("FO_0039_0004_chart");
            dtChart = new DataTable();
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Dummy", dtChart);

            for (int rowIndex = 0; rowIndex < dtChart.Rows.Count; rowIndex++)
            {
                DataRow row = dtChart.Rows[rowIndex];
                for (int i = 0; i < row.ItemArray.Length; i++)
                {
                    if (i == 0 && row[i] != DBNull.Value)
                    {
                        if (rowIndex == 0)
                        {
                            rateChartBeginIndex = row[i].ToString().Contains("г.") ? 1 : 0;
                        }
        ...
        protected void UltraChart_FillSceneGraph(object sender, FillSceneGraphEventArgs e)
        {
            for (int i = 0; i < e.SceneGraph.Count; i++)
            {
                ...
                if (primitive is Box)
                {
                    ...
                    if (box.DataPoint != null && box.Value != null)
                    {
                        ...
                        {
                            indicatorList = String.Format("({0})", dtChart.Rows[rowIndex][columnIndex].ToString().Replace("br", " "));
                        }
            ...
            if (dtChart != null && dtChart.Rows.Count > 0)
            {
                ...
                double textX = rateChartBeginIndex == 0 ? xMin - 0.5 * axisStep : xMin + 2.5 * axisStep;
                ...
                for (int i = rateChartBeginIndex; i < dtChart.Rows.Count; i++)
                {
                    Text rankText = new Text();
                    rankText.bounds = new Rectangle((int)textX, (int)textY, 3 * (int)axisStep, 20);
                    rankText.SetTextString((i + 1 - rateChartBeginIndex).ToString());

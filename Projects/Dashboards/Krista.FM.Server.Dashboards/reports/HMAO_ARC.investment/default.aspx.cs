...
namespace Krista.FM.Server.Dashboards.HMAO_ARC.investment
{
 ...
 not isempty([Measures].[За период])
            and (not [ОК__ОКВЭД].[ОК__ОКВЭД].currentmember.is_datamember)
                ...
                Group.Value = "По видам экономической деятельности";
...
and
        ...
        private void DataBindGrid()
        {
            ...
            DataTable BaseTable = DataBaseHelper.ExecQueryByID("Grid", "cur-Field");
            ...
            {
                {
                    ...
                    foreach (DataColumn TableGridCol in TableGrid.Columns)
                    {
                        ...
                        {
                            //if (!(
                            //    (TableGridCol.ColumnName == "out-subject-invest-sum"
                            //    ||
                            //    TableGridCol.ColumnName == "in-subject-invest-sum")))
                            {
                    ...
                    int LevelNumber = int.Parse(string.Format("{0:N0}", BaseRow["Levelnumber"]));
                    if ((BaseRow["Razdel"] != DBNull.Value)
                       &&
                       LevelNumber == 2)
                    {
                        if (BaseRow["Razdel"].ToString().Contains("C"))
                        {
        ...
        private string GetPrevDateHint()
        {
            ...
            if (ComboTypePeriod.SelectedValue == "к предыдущему периоду")
            {
                ...
                {
                    Year--;
                    return string.Format(GetFirstMounthOfNumberQuart(4), Year.ToString());
                }
        ...
        private void SetHeaderReport()
        {
            ...
            PageSubTitle.Text = string.Format("Ежеквартальный мониторинг иностранных инвестиций в Ханты-Мансийском автономном округе за {0} по {1}",
                curdate.DisplayText(),

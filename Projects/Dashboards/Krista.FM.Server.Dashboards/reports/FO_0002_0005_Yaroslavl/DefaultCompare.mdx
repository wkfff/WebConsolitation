...
<query id="FO_0002_0005_HMAO_date">
    <![CDATA[
-- FO_0002_0005_HMAO_compare_date
with
...
from  [ФО_МесОтч_Доходы]
  ...
  <query id="FO_0002_0005_HMAO_compare_Grid">
    <![CDATA[
-- FO_0002_0005_HMAO_compare_Grid 
with 
        ...
        (
            {
                ...
                (
                    ...
                    (
                        (
                            [КД].[Сопоставимый].CurrentMember.Properties("Код") = "00011105010000000120"
                        ) or
                        (
                            [КД].[Сопоставимый].CurrentMember.Properties("Код") = "00011105020000000120"
                        ) 
                    ) 
                )
            }
        ...
        (
            {
              ...
              (
               ...
               (
                (<%incomes_kd_dimension%>.CurrentMember.Properties("Код") = "00011105050010000120") or
                (<%incomes_kd_dimension%>.CurrentMember.Properties("Код") = "00011105050010000120") or
                (<%incomes_kd_dimension%>.CurrentMember.Properties("Код") = "00011105050010000120")
			   )
			 )
           }
        )'        
...
----------------------------
    ...
    as 'IIF    
        (
            IsEmpty    
            (
                ([Measures].[% выполнения годовых назначений], <%incomes_kd_dimension%>.[<%kd_group_name%>])
    ...
    as 'IIF  
        (
            IsEmpty  ( (Measures.[Ранг процент МО], <%incomes_kd_dimension%>.[<%kd_group_name%>]) ) or not (Measures.[isMRGO]),
            null,
            max  
            (
                [Города и районы],
                (Measures.[Ранг процент МО], <%incomes_kd_dimension%>.[<%kd_group_name%>])    
            ) 
        )'  
   ...
   member Measures.[Доля в МО] 
    ...
    as 'IIF
        (
            IsEmpty
            (
                (Measures.[Среднедушевые доходы], <%incomes_kd_dimension%>.[<%kd_group_name%>])
    ...
    as 'IIF
        (
            IsEmpty
            (
                 (Measures.[Ранг среднедуш МР], <%incomes_kd_dimension%>.[<%kd_group_name%>])
            ) or not ( Measures.[isMR] ),
            null,
            max
            (
    ...
    as 'IIF  
        (
            IsEmpty  ( (Measures.[Ранг процент МР], <%incomes_kd_dimension%>.[<%kd_group_name%>]) ) or not (Measures.[isMR]),
            null,
            max  
            (
                [Районы],
                (Measures.[Ранг процент МР], <%incomes_kd_dimension%>.[<%kd_group_name%>])
            ) 
        )'          
...
where 
  ...
  <query id="FO_0002_0005_HMAO_compare_population">
    <![CDATA[
-- FO_0002_0005_HMAO_compare_population 
with   
    member Measures.[isMRGO] 
    as 'not( [Районы].[Сопоставимый].CurrentMember is [Районы].[Сопоставимый].[Консолидированный бюджет субъекта] or
         [Районы].[Сопоставимый].CurrentMember is [Районы].[Сопоставимый].[<%own_subject_budget_name%>] or
...
where  
  ...
  <query id="FO_0002_0005_HMAO_compare_avg">
    <![CDATA[
-- FO_0002_0005_HMAO_compare_avg 
with 
---------- Полный список доходов 
    member <%incomes_kd_dimension%>.[Налоговые доходы ] 
    as 'Aggregate 

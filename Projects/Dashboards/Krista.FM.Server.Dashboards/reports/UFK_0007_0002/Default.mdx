...
from  [УФК_Сводная ведомость по кассовым поступл месячн]
  ...
  <query id="UFK_0007_0002_compare_grid">
    <![CDATA[
-- UFK_0007_0002_compare_grid
with 
...
---------
    ...
    as '
            ...
            (
                <%region_dimension%>.[Все районы],
                <%region_dimension%>.[Уровень 05],
                SELF_AND_BEFORE 
           )
        '
        
        member [Measures].[Темп роста ]  
    as 'IIF([Measures].[За аналогичный период прошлого года нараст итог] = null, null,
    [Measures].[С начала года]/ [Measures].[За аналогичный период прошлого года нараст итог]) * 1000'    
    member [Measures].[Доля ]  
    ...
    member [Measures].[Доля в прошлом году]    
    as '[Measures].[За аналогичный период прошлого года нараст итог]/
        (
            [Measures].[За аналогичный период прошлого года нараст итог],
            <%region_dimension%>.[Все районы ]
        ) * 1000' 
    member <%region_dimension%>.[Общий итог] 
    as 'sum
        (
            [МО],
            <%incomes_kd_dimension%>.CurrentMember
        )'
        member <%region_dimension%>.[Все районы ] 
    as '<%region_dimension%>.[Все районы] - <%region_dimension%>.[Новосибирская область (Консолидированный)].[Новосибирская Область (Бюджет Субъекта)]'
select
    {
    ...
    Non Empty
    { 
      <%region_dimension%>.[Все районы ] + [МО]  - <%region_dimension%>.[Новосибирская область (Консолидированный)] - <%region_dimension%>.[Новосибирская область (Консолидированный)].[Новосибирская Область (Бюджет Субъекта)] - <%region_dimension%>.[Все районы] - <%region_dimension%>.[Новосибирская область (Консолидированный)].[Городские округа].[г. Новосибирск].DATAMEMBER
    }on rows 
from [УФК_Сводная ведомость по кассовым поступл месячн]
where 
    (

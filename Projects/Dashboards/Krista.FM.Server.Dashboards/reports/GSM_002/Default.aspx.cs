...
namespace Krista.FM.Server.Dashboards.HMAO_ARC.GSM_002
{
    public partial class _default : CustomReportPage
    {
        ...
        Node GetPrevNode(Node n)
        {
            ...
            {
                return GetLastChild(n.Parent.PrevNode);
            }
            if (n.Parent.Parent.PrevNode != null)
            {
                return GetLastChild(GetLastChild(n.Parent.Parent.PrevNode));
            }
            return null;
            //if (n.PrevNode == null)
            //{
        ...
        private bool IsFirstNode()
        {
            ...
            return (node.Parent.Index == 0) && (node.Index == 0) && (node.Parent.Parent.PrevNode == null);
        }
        ...
        {
            if (ComboCurDay.SelectedNode.Level <= 1) 
            {
        ...
        private void DataBindGrid()
        {
            ...
            foreach (DataRow Row in SUTABLE.Rows)
            {
                ...
                {
                    if (!((TypeField.SelectedValue.Contains("Газ сжиженный"))&(Row["Field"].ToString().Contains("Белоярский"))))
                    {
        ...
        private void SetHeader()
        {
            ...
            ConfHeader(cols.FromKey("CUR").Header, 1, 1, 1, 1, CurDay.GridHeader() + " г.");
            ConfHeader(cols.FromKey("PREV").Header, 2, 1, 1, 1, PrevDay.GridHeader() + " г.");
            ConfHeader(cols.FromKey("FY").Header, 3, 1, 1, 1, FyDay.GridHeader() + " г.");
        ...
        private void DataBindColChart()
        {
            ...
            for (int i = 1; i < ReportTable.Rows.Count; i++)
            {
                try
                {
                    if ((TypeField.SelectedValue.Contains("Газ сжиженный")&&(ReportTable.Rows[i]["Field"].ToString().Contains("Белоярский"))))
                    {
                        continue;
                    }
        ...
        protected void DataBindMap()
        {
            ...
            {
                if ((TypeField.SelectedValue.Contains("Газ сжиженный") && (sh.Name.Contains("Белоярский"))))
                {
                    continue;
                }
    ...
    HttpResponseSubstitutionCallback callback
        ...
        private void SetHeaderReport()
        {
            ...
            PageSubTitle.Text = string.Format("Еженедельный мониторинг цен на нефтепродукты на основании данных мониторинга предприятий торговли в разрезе муниципальных образований, Ханты-Мансийский автономный округ, по состоянию на {0} года", CurDay.GridHeader());
        ...
        private void ExcelExporter_EndExport(object sender, Infragistics.WebUI.UltraWebGrid.ExcelExport.EndExportEventArgs e)
        {
            ...
            for (int i = 5; i < Grid.Rows.Count + 5; i++)
            {
                try
                {
                    if (TypeField.SelectedValue.Contains("Газ сжиженный углеводор"))
                        if (e.CurrentWorksheet.Rows[i].Cells[0].Value.ToString().Contains("Белоярский"))
                        {
                            String.Format("Белоярский");
                            e.CurrentWorksheet.Rows[i].Cells[0].Comment = new WorksheetCellComment();
                            e.CurrentWorksheet.Rows[i].Cells[0].Comment.Visible = true;
                            e.CurrentWorksheet.Rows[i].Cells[0].Comment.Text = new FormattedString("Единица измерения: куб.метр");
                        }
                }
                catch { }
            }           
        ...
        private void PdfExportButton_Click(object sender, EventArgs e)
        {
            ...
            if (TypeField.SelectedValue.Contains("Газ сжиженный"))
            {

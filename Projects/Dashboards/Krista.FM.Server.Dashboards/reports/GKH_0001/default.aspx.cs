...
namespace Krista.FM.Server.Dashboards.GKH_0001
{
    public partial class _default : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            Grid.Height = CustomizerSize is CustomizerSize_800x600? Unit.Empty:540;
        ...
        {
            ComboCurDay.Width = IsSmall ? 430 : 550;
            ComboRegion.Width = IsSmall ? 430 : 550;
            ComboComapraDate.Width = IsSmall ? 430 : 550;
            ...
            Link1.Text = "Мониторинг&nbsp;тарифов&nbsp;и&nbsp;нормативов&nbsp;на&nbsp;коммунальные&nbsp;услуги<br>в&nbsp;разрезе&nbsp;муниципальных&nbsp;образований";
        ...
        string GetCurRegion()
        {
            return DataBaseHelper.ExecQueryByID("GetRegionBuYear").Rows[0][0].ToString();
        }
        ...
        private static void fillStrRow(DataRow GridRow, string[] ParsRow)
        {
            ...
            GridRow["Yslyga"] = ParsRow[2].ToString();
        ...
        void SaveToErrorLogColDataTable(DataTable table)
        {
            foreach (DataColumn col in table.Columns)
            {
        ...
        private void FormatBaseTable(DataTable BaseTable)
        {
            //string PrevYear = (int.Parse(ComboCurDay.SelectedValue) - 1).ToString();
            ...
            string PrevMounth = UserComboBox.getLastBlock(ChosenPrevPeriod.Value);
                ...
                //GetPrevNode(ComboCurDay.SelectedNode).Text == ComboCurDay.SelectedValue?
                //"empty":
                //SetCurDay.OtherInfo[GetPrevNode(ComboCurDay.SelectedNode).Text]["mounth"];
        ...
        private void DataBindGrid()
        {
            ...
            DataTable EdIzmTable = DataBaseHelper.ExecQueryByID("EdIzm", "Yslyg");
            ...
            foreach (DataRow BaseRow in BaseTable.Rows)
            {
                ...
                }
                {
                    ...
                    GridRowD["val1"] = GetDeviation(BaseRow[1], BaseRow[1 + adededPrevColNumber]);
                    GridRowD["val2"] = GetDeviation(BaseRow[2], BaseRow[2 + adededPrevColNumber]);
                    //GridRowD["val3"] = GetDeviation(BaseRow[3], BaseRow[3 + adededPrevColNumber]);

                    GridRowSD["val1"] = GetSpeedDeviation(BaseRow[1], BaseRow[1 + adededPrevColNumber]);
                    GridRowSD["val2"] = GetSpeedDeviation(BaseRow[2], BaseRow[2 + adededPrevColNumber]);
        ...
        private void ConfHeader()
        {
            ...
            yearhe.AddCell("При наличии приборов учета").AddCell("Тариф");
        ...
        private void FormatRowYsly(UltraWebGrid Grid, string p)
        {
            foreach (UltraGridRow Row in Grid.Rows)
            {
                ...
                if (GetNextVertCell(cell).Value == null)
                {
                    ...
                    if (GetNextCell(GetNextVertCell(cell)).Value == null)
                    {
                        cell.Style.Font.Bold = true;
                    }
                    cell.ColSpan = 5;
        ...
        private void SetIndicatorSpeedDeviationcell(UltraGridCell ValueCell, bool reverce)
        {
            ...
            if (IndicatorCell.Value != null)
            {
                ...
                if (Value != 0)
                {
                    ...
                    if (Value != 100)
                        GetNextCell(ValueCell).Title = string.Format("{0} к {1} {2} года", TitleCaption, (PrevMounth).ToString(), PrevYear);
                    else
                    {
        ...
        private void FormatRow()
        {
            ...
            foreach (UltraGridRow Row in Grid.Rows)
            {
                if (Row.Cells.FromKey("Org").Value != null)
                {
        ...
        {
            PageSubTitle.Text = string.Format("Анализ тарифов для населения с учётом надбавки, НДС и прочих выплат, применяемых при начислении платежа гражданину, {1}, по состоянию на {0} по сравнению с {2}",
                ComboCurDay.SelectedValue.ToLower(), 
                ComboRegion.SelectedValue,
                GetAnalogForHeader());

            Hederglobal.Text = "Мониторинг тарифов и нормативов на коммунальные услуги в разрезе организаций, предоставляющих услуги ЖКХ";
            Page.Title = Hederglobal.Text;

            Link1.NavigateUrl = "../GKH_0002/default.aspx";
        }
        ...
        void sad(object sender, Infragistics.WebUI.UltraWebGrid.ExcelExport.EndExportEventArgs e)
        {
            ...
            e.CurrentWorksheet.Rows[1].Height = 24 * 30; 
        }

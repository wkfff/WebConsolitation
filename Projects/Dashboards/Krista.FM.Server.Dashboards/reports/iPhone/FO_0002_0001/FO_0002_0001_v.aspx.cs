...
{
    public partial class FO_0002_0001_v : CustomReportPage
    {
        ...
        {            
            ...
            DataTable dtDate = new DataTable();
            query = DataProvider.GetQueryText("FO_0002_0001_incomes_date");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForPivotTable(query, dtDate);
        ...
        private static DataTable GetBudetDataSource()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0002_0001_budget_incomes_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);

            DataTable source = new DataTable();

            query = DataProvider.GetQueryText("FO_0002_0001_budget_outcomes_all_v");
            ...
            query = DataProvider.GetQueryText("FO_0002_0001_budget_outcomes_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0002_0001_budget_deficite_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
        ...
        private void BindDeficiteIndicator(double incomesWithoutBpValue, double restsChangeValue, double sellIncomesValue, double deficiteProficiteValue)
        {
            ...
            {
                deficiteDescription = String.Format("Профицит бюджета по плану на {0} год: &nbsp;<span style=\"color: white\"><b>{1:N2}</b></span>&nbsp; тыс.руб.<br/>Предельный дефицит (с учетом изменения остатков и поступлений от продажи акций): &nbsp;<span style=\"color: white\"><b>{2:N2}</b></span>&nbsp; тыс.руб.", year, deficiteProficiteValue / 1000, deficiteLimit / 1000);
            }
            else
            {
                deficiteDescription =

...
{
    public partial class FO_0007_0001_1 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            query = DataProvider.GetQueryText("FO_0007_0001_rests_date");
            ...
            query = DataProvider.GetQueryText("FO_0007_0001_table_struct");
            ...
            query = DataProvider.GetQueryText("FO_0007_0001_table_debt");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, " ", dtDebt);

            dtSaldo = new DataTable();
            query = DataProvider.GetQueryText("FO_0007_0001_table_saldo");
            ...
            lbDescription.Text =
                String.Format(
                    "Информация по государственному внутреннему долгу Омской области<br/>по состоянию на&nbsp;<span class='DigitsValue'>{0:dd.MM.yyyy}</span>, тыс.руб.",
                    currDate.AddMonths(1));

            IPadElementHeader1.Text = String.Format("Структура государственного внутреннего долга  на {0:dd.MM.yyyy}", currDate.AddMonths(1));
            IPadElementHeader2.Text = currDate.Month == 1 ? String.Format("Операции  на внутреннем рынке заимствований<br/>за январь {1} г.", CRHelper.RusMonth(currDate.Month), currDate.Year) : String.Format("Операции  на внутреннем рынке заимствований<br/>за январь-{0} {1} г.", CRHelper.RusMonth(currDate.Month), currDate.Year);
            ...
            query = DataProvider.GetQueryText("FO_0007_0001_result");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, " ", dtResult);

            Label1.Text = String.Format(
                    "<table><tr><td>Результат исполнения областного бюджета &nbsp;<span class='DigitsValueXLarge'>{0:N1}</span>&nbsp;тыс.руб.</td><td>&nbsp;<a href='webcommand?showPinchReport=FO_0007_0002'><img src='../../../images/detail.png'></a></td></tr></table>",
                    dtResult.Rows[0][0]);
        }
        ...
        void UltraWebGrid4_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            CRHelper.FormatNumberColumn(e.Layout.Bands[0].Columns[0], "N1");
            CRHelper.FormatNumberColumn(e.Layout.Bands[0].Columns[1], "N1");

...
{
    public partial class FO_0535_0015_v : CustomReportPage
    {
        ...
        private void InitializeCreditDebts()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0535_0015_credit_debts");
        ...
        private void BindDeficiteIndicator(double incomesWithoutBpValue, double restsChangeValue, double sellIncomesValue, double deficiteProficiteValue, double budjetCreditPlanValue, double moGroupValue)
        {
            ...
            {
                deficiteDescription = String.Format("Бюджет на {0} год сбалансированный<br/>Предельный дефицит (с учетом изменения остатков и поступлений от продажи акций):&nbsp;<span style=\"color: white\"><b>{0:N2}</span>&nbsp;тыс.руб.", deficiteLimit / 1000, year);
            }
            else if (deficiteProficiteValue > 0)
            {
                deficiteDescription = String.Format("Профицит бюджета МО по плану на {0} год:&nbsp;<span style=\"color: white\"><b>{1:N2}</span>&nbsp;тыс.руб.<br/>Предельный дефицит (с учетом изменения остатков и поступлений от продажи акций):&nbsp;<span style=\"color: white\"><b>{2:N2}</span>&nbsp;тыс.руб.", year, deficiteProficiteValue, deficiteLimit / 1000);
            }
            else
            {
                deficiteDescription =
        ...
        private static DataTable GetBudetDataSource()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0535_0015_budget_incomes_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);

            DataTable source = new DataTable();

            query = DataProvider.GetQueryText("FO_0535_0015_budget_outcomes_all_v");
            ...
            query = DataProvider.GetQueryText("FO_0535_0015_budget_outcomes_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0535_0015_budget_deficite_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
        ...
        private void InitializeBudgetSettlement()
        {
            ...
            query = DataProvider.GetQueryText("FO_0535_0015_settlements_list");
        ...
        protected void UltraWebGridBudgetSettlement_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0535_0015_settlement_incomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);

            DataTable source = new DataTable();

            query = DataProvider.GetQueryText("FO_0535_0015_settlement_outcomes_all");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0535_0015_settlement_outcomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0535_0015_settlement_deficite");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);

...
{
    public partial class FST_0001_0012_Horizontal : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            UltraChartFST_0001_0001_Chart2.QueryName = "FST_0001_0011_ChartElectro";
        ...
        {
            string query = DataProvider.GetQueryText("FST_0001_0012_Horizontal_Grid");
            ...
            query = DataProvider.GetQueryText("FST_0001_0011_Grid");
            DataTable dtGrid1 = new DataTable();
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dtGrid1);
            ...
            for (int i = 1; i < dtGrid1.Rows.Count; i++)
            {
                ...
                {
                    double taxValue;
                    if (dtGrid1.Rows[i]["Среднеотпускной тариф на электрическую энергию"] != DBNull.Value &&
                        Double.TryParse(dtGrid1.Rows[i]["Среднеотпускной тариф на электрическую энергию"].ToString(), out taxValue) &&
                        taxValue != 0)
                    {
                        notZeroCountRF++;
                        foAvgRF += taxValue;
                    ...
                    double taxDemosValue;
                    if (dtGrid1.Rows[i]["Тариф на электрическую энергию для населения"] != DBNull.Value &&
                        Double.TryParse(dtGrid1.Rows[i]["Тариф на электрическую энергию для населения"].ToString(), out taxDemosValue) &&
                        taxDemosValue != 0)
                    {
                        notZeroDemosCountRF++;
                    ...
                    double grownValue;
                    if (dtGrid1.Rows[i]["Рост тарифа на электрическую энергию"] != DBNull.Value &&
                        Double.TryParse(dtGrid1.Rows[i]["Рост тарифа на электрическую энергию"].ToString(), out grownValue) &&
                        grownValue != -0)
                    {
                        notZeroGrownCountRF++;
                    ...
                    double grownDemosValue;
                    if (dtGrid1.Rows[i]["Рост тарифа на электрическую энергию для населения"] != DBNull.Value &&
                        Double.TryParse(dtGrid1.Rows[i]["Рост тарифа на электрическую энергию для населения"].ToString(), out grownDemosValue) &&
                        grownDemosValue != -0)
                    {
            ...
            UltraChartFST_0001_0001_Chart1.RfMiddleLevel = notZeroGrownCountRF == 0 ? 0 : (foGrownAvgRF / notZeroGrownCountRF) - 1;
            ...
            UltraChartFST_0001_0001_Chart2.RfMiddleLevel = notZeroGrownDemosCountRF == 0 ? 0 : (foGrownDemosAvgRF / notZeroGrownDemosCountRF) - 1;
        ...
        protected void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            if (e.Row.Cells[13].Value.ToString() == "Субъект РФ")
            {
                ...
                {
                    e.Row.Cells[1].Value = String.Format("<table style='border: 1px solid black; border-collapse: collapse; margin-right: -4px'><tr><td style='width: 76px; border: 1px solid black'>{0}</td><td style='width: 50px; border: 1px solid black'>{1:N2}</td></tr></table>", GetRankImage(e, 5, 6), e.Row.Cells[1].Value);
                }
                if (e.Row.Cells[2].Value != null)
                {
                    e.Row.Cells[2].Value = String.Format("<table style='border: 1px solid black; border-collapse: collapse; margin-right: -4px'><tr><td style='width: 76px; border: 1px solid black'>{0}</td><td style='width: 50px; border: 1px solid black'>{1:P0}</td></tr></table>", GetRankImage(e, 7, 8), e.Row.Cells[2].Value);
                }
                if (e.Row.Cells[3].Value != null)
                {
                    e.Row.Cells[3].Value = String.Format("<table style='border: 1px solid black; border-collapse: collapse; margin-right: -4px'><tr><td style='width: 76px; border: 1px solid black'>{0}</td><td style='width: 50px; border: 1px solid black'>{1:N2}</td></tr></table>", GetRankImage(e, 9, 10), e.Row.Cells[3].Value);
                }
                if (e.Row.Cells[4].Value != null)
                {
                    e.Row.Cells[4].Value = String.Format("<table style='border: 1px solid black; border-collapse: collapse; margin-right: -4px'><tr><td style='width: 76px; border: 1px solid black'>{0}</td><td style='width: 50px; border: 1px solid black'>{1:P0}</td></tr></table>", GetRankImage(e, 11, 12), e.Row.Cells[4].Value);
                }
            }
            ...
            if (e.Row.Cells[13].Value != null &&
                (e.Row.Cells[13].Value.ToString() == "РФ"))
            {

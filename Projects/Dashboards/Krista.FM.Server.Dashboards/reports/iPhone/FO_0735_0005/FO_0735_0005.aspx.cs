...
{
    public partial class FO_0735_0005 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!dtDate.Rows[0][4].ToString().Contains("Заключительные обороты"))
            {
                ...
                if (IsQuaterPlanType)
                {
                    ...
                    lbDate.Text = "&nbsp;(на&nbsp;<span style='color: white;'>" + date.ToString("dd.MM.yyyy") + "</span>) по бюджету округа, тыс.руб.";
                }
                else
                {
            ...
            {
                Label1.Text = "Остаток средств на начало года&nbsp;";
                Label2.Visible = true;
                Label3.Visible = true;
                Label4.Visible = true;
        ...
        private void BindTagCloud()
        {
            ...
            for (int j = 0; j < dtExecuteOutcome.Rows.Count - 1; j++)
            {
                DataRow row = dtExecuteOutcome.Rows[j];
                Tag tag = new Tag();
                ...
                for (int i = 1; i < 6; i++)
                {
                        ...
                        row[i].ToString() != String.Empty)
                    {
                        ...
                        double ethalon = Convert.ToDouble(dtExecuteOutcome.Rows[dtExecuteOutcome.Rows.Count - 1][i]);
        ...
        private void MakeHtmlTableIncomes()
        {
            ...
            for (int i = 0; i < dtData.Rows.Count; i++)
            {
                ...
                TableCell cell;
                if (dtData.Rows[i][0].ToString() == "Доходы бюджета округа")
                {
                    dtData.Rows[i][0] = "Доходы бюджета округа";
                ...
                {
                    dtData.Rows[i][0] = "Поступления из источников финансирования дефицита бюджета округа";
                    cell = GetNameCell(dtData.Rows[i][0].ToString(), false);
                    cell.ColumnSpan = 4;
                    cell.Style.Add("padding-top", "2px");
                    cell.Style.Add("padding-bottom", "2px");
        ...
        private void MakeHtmlTableOutcomes()
        {
            ...
            for (int i = 0; i < dtData.Rows.Count; i++)
            {
                ...
                {
                    dtData.Rows[i][0] = "Выплаты из источников финансирования дефицита бюджета округа";
                    cell = GetNameCell(dtData.Rows[i][0].ToString(), false);
                    cell.ColumnSpan = 4;
                    row.Cells.Add(cell);
                    Outcomes.Rows.Add(row);

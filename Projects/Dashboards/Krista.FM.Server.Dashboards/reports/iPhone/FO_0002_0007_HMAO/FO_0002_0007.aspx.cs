...
{
    public partial class FO_0002_0007_HMAO : CustomReportPage
    {
        ...
        protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0002_0007_HMAO");
            ...
            if (dt.Columns.Count > 0 && dt.Rows.Count > 0)
            {
                ...
                dt.Rows[0]["Коэффициент бюджетной обеспеченности населения"] = 1000 * avgIncomesTotal;
                dt.Rows[0]["Лучший по коэфф.бюдж.обесп. МО"] = avgIncomesBestItem;
                if (avgIncomesMaxValue != double.MinValue)
                {
                    dt.Rows[0]["Лучшие коэфф.бюдж.обесп. МО"] = avgIncomesMaxValue;
                }
                dt.Rows[0]["Худший по коэфф.бюдж.обесп. МО"] = avgIncomesWorseItem;
                if (avgIncomesMinValue != double.MaxValue)
                {
                    dt.Rows[0]["Худшие коэфф.бюдж.обесп. МО"] = avgIncomesMinValue;
                }
            ...
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 8:
                        {
                            k = 0.001;
                            caption = "<div style=\"line-height:15px;\">Бюджетные расходы на душу населения,<br />тыс.руб./чел.</div>";
                            break;
                        }
                    case 13:
                        {
        ...
        {
            string query = DataProvider.GetQueryText("F0_0002_0007_iphone_outcomes");
            ...
            {
                query = DataProvider.GetQueryText("FO_0002_0007_HMAO_population");
                    ...
                    foreach (DataRow populationRow in populationDT.Rows)
                    {
                        if (populationRow[0] != DBNull.Value)
                        {
                                ...
                                populationRow[populationColumnName].ToString() != String.Empty)
                            {
                                ...
                                DataRow incomesRow = outcomesDT.Rows.Find(rowName);
                                if (incomesRow != null)
                                {

...
namespace Krista.FM.Server.Dashboards.reports.iPad
{
    public partial class STAT_0001_0003 : CustomReportPage
    {
        ...
        private string GetDescritionText(DateTime date)
        {
               ...
               Double.TryParse(dtText.Rows[3][1].ToString(), out ufoMot))
            {
                ...
                description = String.Format(
                   "В&nbsp;<span class='DigitsValue'><b>{0} {1} года</b></span>&nbsp;уровень общей безработицы<br/>по методологии МОТ в ХМАО-Югре&nbsp;<span class='DigitsValueXLarge'><b>{2:P2}</b></span><br/>{3}", CRHelper.RusMonthPrepositional(date.Month), date.Year, thisMot, description);
            }
                ...
                Double.TryParse(dtText.Rows[1][2].ToString(), out prevMonth))
            {
                ...
                description += String.Format("За {1} {2} года число безработных<br/>{0}{5} на&nbsp;<span class='DigitsValue'><b>{3:N0}</b></span>&nbsp;человек (темп роста&nbsp;<b><span class='DigitsValue'>{4:P2}</span></b>)<div style='height: 7px; clear: both'></div>", grown, CRHelper.RusMonth(date.Month), date.Year, Math.Abs(grownValue), grownTemp, GetImage(grown));
            }
        ...
        private void BindTagCloud()
        {
            ...
            DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(
                query, "Наименование показателя", dtTagCloud);
            ...
            foreach (DataRow row in dtTagCloud.Rows)
            {
                ...
                {
                    Tag tag = new Tag();
                    tag.weight = Convert.ToInt32(row["Численность населения "]);
        ...
        private void CommentTextDataBind()
        {
            ...
            if (dtCommentText.Rows.Count > 0)
            {
                ...
                string str1 =
                    string.Format(
                        @"На&nbsp;<span class='DigitsValue'><b>{0}</b></span>&nbsp;уровень регистрируемой в органах службы занятости безработицы в ХМАО-Югре&nbsp;<span class='DigitsValueXLarge'><b>{1:P2}</b></span> {2} <div style='height: 7px; clear: both'></div>",
                        dateTimeStr, redundantlevelValue, redundantLevelRFGrow);

                string str2 = string.Format(@"Численность безработных&nbsp;<span class='DigitsValueXLarge'><b>{0:N0}</b></span>&nbsp;человек<br>", totalCount);
                ...
                string str5 = string.Format(@"Население без статуса безработных на&nbsp;<span class='DigitsValue'>{1:01.MM.yyyy}</span>&nbsp;составляет&nbsp;<span class='DigitsValueXLarge'><b>{0:N0}</b></span>&nbsp;человек<div style='height: 7px; clear: both'></div>", unemploued, currDateTimeHmao);

                string str6 = string.Format(@"Потребность в работниках, заявленная работодателями в органы службы занятости населения&nbsp;<span class='DigitsValueXLarge'><b>{0:N0}</b></span>&nbsp;вакансий<br/>", dtCommentTextHmao.Rows[5][1]);

                string str7 = string.Format(@"Коэффициент напряженности на рынке труда&nbsp;<span class='DigitsValueXLarge'><b>{0:N2}</b></span><br/>", dtCommentTextHmao.Rows[6][1]);

                string str8 = string.Format(@"Численность безработных на 1 вакансию&nbsp;<span class='DigitsValueXLarge'><b>{0:N1}</b></span>", dtCommentTextHmao.Rows[7][1]);
                ...
                string str9 = String.Format("<div style='height: 7px; clear: both'></div>Экономически активное население&nbsp;<span class='DigitsValue'><b>{0:N0}</b></span>&nbsp;человек<br/>Численность населения ХМАО-Югры&nbsp;<span class='DigitsValue'><b>{1:N0}</b></span>&nbsp;человек",

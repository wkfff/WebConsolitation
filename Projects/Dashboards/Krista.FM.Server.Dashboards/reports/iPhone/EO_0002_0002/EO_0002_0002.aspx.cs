...
{
    public partial class EO_0002_0002 : CustomReportPage
    {
        #region Поля
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("EO_0002_0002_percent");
...
<td align=center>Процент исполнения финансирования за&nbsp;<span class='DigitsValue'>{2:yyyy}г.</span>&nbsp;по состоянию на&nbsp;<span class='DigitsValue'>{2:dd.MM.yyyy}</span></td></tr></table>", dtPercent.Rows[0][1], GetGaugeUrl(dtPercent.Rows[0][1]), currentDate);
            ...
            UserParams.Filter.Value = "[Исполнение расходов__Задачи].[Исполнение расходов__Задачи].[Все задачи].[Содействие занятости населения]";
            DataTable dtAction = new DataTable();
            query = DataProvider.GetQueryText("EO_0002_0002_Action");
            ...
            ConfigureActionChart(UltraChart12, dtAction, 2, "1.2");
            ConfigureActionChart(UltraChart13, dtAction, 3, "1.3");
            ConfigureActionChart(UltraChart14, dtAction, 4, "1.4");

            UserParams.Filter.Value = "[Исполнение расходов__Задачи].[Исполнение расходов__Задачи].[Все задачи].[Социальная адаптация и трудовая занятость молодежи]";
            dtAction = new DataTable();
            query = DataProvider.GetQueryText("EO_0002_0002_Action");
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование показателей", dtAction);

            ConfigureActionChart(UltraChart21, dtAction, 1, "2.1");
            ConfigureActionChart(UltraChart22, dtAction, 2, "2.2");

            UserParams.Filter.Value = "[Исполнение расходов__Задачи].[Исполнение расходов__Задачи].[Все задачи].[Содействие трудоустройству инвалидов]";
            dtAction = new DataTable();
            query = DataProvider.GetQueryText("EO_0002_0002_Action");
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование показателей", dtAction);
            ...
            UserParams.Filter.Value = @"[Исполнение расходов__Задачи].[Исполнение расходов__Задачи].[Все задачи].[Повышение качества и конкурентоспособности рабочей силы на рынке труда]";
            dtAction = new DataTable();
            query = DataProvider.GetQueryText("EO_0002_0002_Action");
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование показателей", dtAction);
        ...
        {
            string query = DataProvider.GetQueryText("EO_0002_0002_grid");
        ...
        private void InitializeChart()
        {
            ...
            string query = DataProvider.GetQueryText("EO_0002_0002_Chart");
        ...
        private void ConfigureActionChart(UltraChart chart, DataTable dataSource, int dataIndex, string num)
        {
            ...
            double percent = 0;
            if (dataSource.Rows[2][dataIndex] != DBNull.Value)
            {

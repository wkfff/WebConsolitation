...
{
    public partial class FO_0002_0010 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            lbPageTitle.Text = "Исполнение бюджетов Новосибирской области";
            lbDescription.Text = String.Format("Анализ исполнения бюджетов Новосибирской области по состоянию на {0:dd.MM.yyyy} , тыс.руб.", date);
       ...
       protected void OutcomesGrid_DataBinding(object sender, EventArgs e)
            ...
            foreach (DataRow dataRow in dtGrid.Rows)
            {
                ...
                if (moId == String.Empty)
                {
                        ...
                        budgetId == "2")
                    {
                        dataRow[1] =
                              String.Format(
                                  "<div style='float: right'><a href='webcommand?showReport=FO_0002_0004_BUDGET={0}'><img src='../../../images/detail.png'></a></div>", budgetId);
                    }
                    else
                    {
                        dataRow[1] =
                              String.Format(
                                  "<div style='float: right'><a href='webcommand?showReport=FO_0002_0012'><img src='../../../images/detail.png'></a></div>", budgetId);
                    }
                }
                else
                {
                    dataRow[1] =
                        String.Format(
                            "<div style='float: right'><a href='webcommand?showPinchReport=FO_0002_0011_mo={0}'><img src='../../../images/detail.png'></a></div>", moId);
                }
        ...
        protected void OutcomesGrid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            {
                e.Row.Cells[2].Value = String.Format("{0:N2}<br/>{1:P2}", e.Row.Cells[2].Value, e.Row.Cells[6].Value); // 3
            }

            if (e.Row.Cells[7].Value != null)
            {
                e.Row.Cells[3].Value = String.Format("{0:N2}<br/>{1:P2}", e.Row.Cells[3].Value, e.Row.Cells[7].Value); // 4
            }

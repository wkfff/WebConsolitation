...
{
    public partial class FO_0002_0017 : CustomReportPage
    {
       ...
       protected void OutcomesGrid_DataBinding(object sender, EventArgs e)
        {
                ...
                if (UserParams.Mo.Value.Contains("МР")) // выбран район
                {
                    CRHelper.SaveToErrorLog("OK13");
                    string query = DataProvider.GetQueryText("FO_0002_0017_Grid2");
                ...
                else if (UserParams.Mo.Value.Contains("г.") || UserParams.Mo.Value.Contains("р.п."))
                {
                    ...
                    string query = DataProvider.GetQueryText("FO_0002_0017_Grid1");
                    dtGrid = new DataTable();
                    DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dtGrid);
                }
             /*   else if (UserParams.BudgetLevel.Value.Contains("Конс. бюджет МО"))
                {
                    string query = DataProvider.GetQueryText("FO_0002_0017_Grid3");
                    dtGrid = new DataTable();
                    DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dtGrid);
                }
           */
        ...
        protected void OutcomesGrid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            Label1.Text = string.Format("- не соблюдается условие равномерности({0:P1})", percent);
            Label2.Text = string.Format("- соблюдается условие равномерности ({0:P1})", percent);
                ...
                if (i == 2)
                {
                        ...
                        if (e.Row.Cells[i].Value != null && e.Row.Cells[i].Value.ToString() != string.Empty)
                        {
                            ...
                            if (rate >= percent)
                            {
                                ...
                                hint = string.Format("Соблюдается условие равномерности ({0:P1})", percent);
                            }
                            else
                            {
                                if (rate < percent)
                                {
                                    ...
                                    hint = string.Format("Не соблюдается условие равномерности ({0:P1})", percent);
                                }
                            }

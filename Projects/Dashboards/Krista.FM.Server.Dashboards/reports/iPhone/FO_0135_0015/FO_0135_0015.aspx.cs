...
{
    public partial class FO_0135_0015 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            {
                UserParams.Mo.Value = "Болотнинский район";
            }
            ...
            string query = DataProvider.GetQueryText("FO_0135_0015_population");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_incomes_date");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_outcomes_date");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForPivotTable(query, dtDate);
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_credit_debts");
        ...
        private void InitializeIncomes()
        {
            ...
            dt = new DataTable();
            string query = DataProvider.GetQueryText("FO_0135_0015_Incomes");
        ...
        private void InitializeOutcomes()
        {
            ...
            dt = new DataTable();
            string query = DataProvider.GetQueryText("FO_0135_0015_Outcomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, " ", dt);
        ...
        private void InitializeBkku()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0135_0015_Inner_Debt");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_Incomes_without_BP");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_inner_fin_source");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_Deficite");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_Outcomes_debt");
        ...
        private void BindServeIndicator(double bpValue, double outcomesServeValue, double outcomesAllValue)
        {
            ...
            string serveDescription = outcomesServeValue == 0 ? "Расходы на обслуживание долга&nbsp;<b>отсутствуют</b>" : String.Format("Расходы на обслуживание долга {0}:&nbsp;<b>{1:N2}</b>&nbsp;тыс.руб.", String.Format("за {0} месяцев {1} года", month, year), outcomesServeValue / 1000);

            string debtOutcomesTooltip = String.Format("<b>Соблюдение ограничения расходов на обслуживание долга</b><br/>Приведены данные по собственному бюджету района<br/>без учета бюджетов поселений<br/>Расходы на обслуживание долга МО<br/>ограничиваются 15% расходов бюджета (без учета<br/>расходов за счет финансовой помощи от других бюджетов)<br/><br/>Значение индикатора:&nbsp;<b>{0:P2}</b><br/><b>{1}</b><br/>{2}<br/>Предельный размер расходов на обслуживание долга:&nbsp;<b>{3:N2}</b> тыс.руб.",
                                                indicator, GetCrime(crime), serveDescription, limitServe / 1000);

            if (serveDescription.Contains("Расходы на обслуживание долга&nbsp;<b>отсутствуют</b>"))
            {
                lbDebtServe.Text = "&nbsp;";
                Label29.Text = "&nbsp;";
                text3 = "Нет";
        ...
        private void BindDeficiteIndicator(double incomesWithoutBpValue, double restsChangeValue, double sellIncomesValue, double deficiteProficiteValue)
        {
            ...
            {
                deficiteDescription = String.Format("Сбалансированный бюджет<br/>Предельный дефицит (с учетом изменения остатков<br/>и поступлений от продажи акций):&nbsp;<b>{0:N2}</b>&nbsp;тыс.руб.", deficiteLimit / 1000);
            }
            else if (deficiteProficiteValue > 0)
            {
                deficiteDescription = String.Format("Профицит бюджета по плану на {0} год:&nbsp;<b>{1:N2}</b>&nbsp;тыс.руб.<br/>Предельный дефицит (с учетом изменения остатков<br/>и поступлений от продажи акций):&nbsp;<b>{2:N2}</b>&nbsp;тыс.руб.", year, deficiteProficiteValue, deficiteLimit / 1000);
            }
            else
            {
                deficiteDescription =
                    String.Format(
                        "Дефицит бюджета МО (с учетом изменения остатков<br/>и поступлений от продажи акций):<br/>по плану на {0} год:&nbsp;<b>{1:N2}</b>&nbsp;тыс.руб.<br/>предельный дефицит:&nbsp;<b>{2:N2}</b>&nbsp;тыс.руб.",
            ...
            string deficiteTooltip = String.Format("<b>Соблюдение ограничения дефицита бюджета</b><br/>Приведены данные по собственному бюджету района<br/>без учета бюджетов поселений<br/>Дефицит бюджета МО (с учетом изменения остатков<br/>и поступлений от продажи акций)<br/>не может превышать 10% доходов<br/>без учета финансовой помощи от других бюджетов<br/>и средств областного фонда компенсаций<br/><br/>Значение индикатора:&nbsp;<b>{0:P2}</b><br/><b>{1}</b><br/>{2}",
        ...
        private void BindDebtIndicator(double innerDebtValue, double incomesWithoutBpValue, double budjetCreditValue)
        {
            ...
            string debtTooltip = String.Format("<b>Соблюдение ограничения на объем долга,<br/>установленного БК РФ</b><br/>Приведены данные по собственному бюджету района<br/>без учета бюджетов поселений<br/>Объем долга МО не может превышать объем доходов<br/>с учетом бюджетных кредитов и без учета<br/>финансовой помощи от других бюджетов<br/>и средств областного фонда компенсаций<br/><br/>Значение индикатора:&nbsp;<b>{0:P2}</b><br/><b>{1}</b><br/>{2}<br/>Предельное значение<br/>объема долга МО:&nbsp;<b>{3:N2}</b>&nbsp;тыс.руб.",
                                                debtIndicator, GetCrime(crime), debtDescription, debtLimit / 1000);

            if (debtDescription.Contains("Муниципальный долг&nbsp;<b>отсутствует</b>"))
            {
                lbDebts.Text = "&nbsp;";
                Label31.Text = "&nbsp;";
                text1 = "Нет";
        ...
        protected void UltraWebGridBudget_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0135_0015_budget_incomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);

            DataTable source = new DataTable();

            query = DataProvider.GetQueryText("FO_0135_0015_budget_outcomes_all");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_budget_outcomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0135_0015_budget_deficite");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
        ...
        {
            DataTable dtDate = new DataTable();
            string query = DataProvider.GetQueryText("FO_0135_0015_incomes_date");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForPivotTable(query, dtDate);
            ...
            DataTable dtIncomes = new DataTable();
             query = DataProvider.GetQueryText("FO_0135_0015_Incomes_settlement");
            ...
            query = DataProvider.GetQueryText("FO_0135_0015_Outcomes_settlement");
        ...
        private void InitializeSettlement()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0135_0015_settlements_count");

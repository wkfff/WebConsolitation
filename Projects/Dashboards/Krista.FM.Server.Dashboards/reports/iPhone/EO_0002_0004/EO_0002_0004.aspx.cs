...
{
    public partial class EO_0002_0004 : CustomReportPage
    {
        #region Поля
        ...
        private void BindInfoText()
        {
           // string queryCountText = string.Format("select count (*) from {0} {1}", "d_Org_FNSDebtor", filter);
            ...
            string query = DataProvider.GetQueryText("EO_0002_0004_info");
            DataTable dtInfo = new DataTable();
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование показателей", dtInfo);

            lbInfo.Text = String.Format(@"В Ханты-Мансийском автономном округе действуют&nbsp;<span class='DigitsValueLarge'>{0:N0}</span>&nbsp;целевые программы на общую сумму по уточненной бюджетной росписи&nbsp;<span class='DigitsValue'>{1:N2}</span>&nbsp;млн.руб.<br/>
По состоянию на&nbsp;<span class='DigitsValue'>{2:dd.MM.yyyy}</span>&nbsp;года&nbsp;<span class='DigitsValue'>фактически исполнено</span>&nbsp;за&nbsp;<span class='DigitsValue'>{5:yyyy}</span>&nbsp;год по целевым программам&nbsp;<span class='DigitsValue'>{3:N2}</span>&nbsp;млн.руб. (<span class='DigitsValue'>{4:P2}</span>&nbsp;от уточненной бюджетной росписи на&nbsp;<span class='DigitsValue'>{5:yyyy}</span>&nbsp;год)",
               dtInfo.Rows[0][1],
               dtInfo.Rows[0][2],
        ...
        {
            string query = DataProvider.GetQueryText("EO_0002_0004_grid");
            ...
            foreach (DataRow row in gridDt.Rows)
            {
                ...
                newRow = newDt.NewRow();
                newRow[0] = row[0];               
                newRow[1] = String.Format("Факт на {0:dd.MM.yyyy}г.", currentDate);
        ...
        protected string GetGaugeUrl(object oValue, string alt)
        {
            ...
            if (value > 100)
                value = 100;
            string path = "EO_0002_0004_gauge_" + value.ToString("N0") + ".png";

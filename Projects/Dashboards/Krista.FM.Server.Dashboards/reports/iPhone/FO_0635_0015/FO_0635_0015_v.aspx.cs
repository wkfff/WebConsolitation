...
{
    public partial class FO_0635_0015_v : CustomReportPage
    {
        ...
        private void InitializeCreditDebts()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0635_0015_credit_debts");
        ...
        private void BindServeIndicator(double bpValue, double outcomesServeValue, double outcomesAllValue)
        {
            ...
            string serveDescription = outcomesServeValue == 0 ? String.Format("Расходы бюджета МО на обслуживание долга по плану на {0} год по состоянию на {1:dd.MM.yyyy} г.&nbsp;<span style=\"color: white\"><b>отсутствуют</b></span>&nbsp;", year, date) : String.Format("Расходы бюджета МО на обслуживание долга по плану на {0} год по состоянию на {1:dd.MM.yyyy} г.&nbsp;<span style=\"color: white\"><b>{2:N2}</b></span>&nbsp; тыс.руб.", year, date, outcomesServeValue / 1000);
            
            string debtOutcomesTooltip = String.Format("<span style=\"color: white\"><b>Соблюдение ограничения расходов на обслуживание долга</b></span>&nbsp;<br/>Приведены данные по собственному бюджету, без учета поселений.<br/>Расходы бюджета МО на обслуживание долга не должны превышать 15% от общей суммы расходов (без учета расходов за счет финансовой помощи от других бюджетов)<br/>Значение индикатора:&nbsp;<span style=\"color: white\"><b></span>{0:P2}</b><br/><span style=\"color: white\"><b>{1}</b></span><br/>{2}<br/>Предельный размер расходов на обслуживание долга:&nbsp;<span style=\"color: white\"><b>{3:N2}</b></span>&nbsp; тыс.руб.",
        ...
        private void BindDeficiteIndicator(double incomesWithoutBpValue, double restsChangeValue, double sellIncomesValue, double deficiteProficiteValue, double budjetCreditPlanValue, double moGroupValue)
        {
            ...
            {
                deficiteDescription = String.Format("Бюджет на {0} год сбалансированный<br/>Предельный дефицит (с учетом изменения остатков и поступлений от продажи акций):&nbsp;<span style=\"color: white\"><b>{0:N2}</b></span>&nbsp;тыс.руб.", deficiteLimit / 1000, year);
            }
            else if (deficiteProficiteValue > 0)
            {
                deficiteDescription = String.Format("Профицит бюджета МО по плану на {0} год:&nbsp;<span style=\"color: white\"><b>{1:N2}</b></span>&nbsp;тыс.руб.<br/>Предельный дефицит (с учетом изменения остатков и поступлений от продажи акций):&nbsp;<span style=\"color: white\"><b>{2:N2}</b></span>&nbsp;тыс.руб.", year, deficiteProficiteValue, deficiteLimit / 1000);
            }
            else
            {
                deficiteDescription =
                    String.Format(
                        "Дефицит бюджета МО по плану на {0} год:&nbsp;<span style=\"color: white\"><b>{1:N2}</b></span>&nbsp;тыс.руб.<br/>предельный дефицит:&nbsp;<span style=\"color: white\"><b>{2:N2}</b></span>&nbsp;тыс.руб.",
            ...
            string deficiteTooltip = String.Format("<span style=\"color: white\"><b>Соблюдение ограничения дефицита бюджета</b></span><br/>Приведены данные по собственному бюджету, без учета поселений.<br/>Группа МО по доле МБТ&nbsp;<span style=\"color: white\"><b>{3:N0}</b></span><br/>Дефицит бюджета МО (с учетом изменения остатков и поступлений от продажи акций) не может превышать 10% (для МО группы 3 по доле МБТ 5%) утвержденного годового объема доходов без учета безвозмездных поступлений и доходов, полученных по дополнительному нормативу от НДФЛ<br/>Значение индикатора:&nbsp;<span style=\"color: white\"><b>{0:P2}</b></span><br/><span style=\"color: white\"><b>{1}</b></span><br/>{2}",
        ...
        private void BindDebtIndicator(double innerDebtValue, double incomesWithoutBpValue, double budjetCreditValue, double moGroupValue)
        {
            ...
            string debtTooltip = String.Format("<span style=\"color: white\"><b>Соблюдение ограничения на объем долга, установленного БК РФ</b></span><br/>Приведены данные по собственному бюджету, без учета поселений.<br/>Группа МО по доле МБТ&nbsp;<span style=\"color: white\"><b>{4:N0}</b></span><br/>Объем долга МО не может превышать утвержденный годовой объем доходов без учета безвозмездных поступлений и доходов, полученных по дополнительному нормативу от НДФЛ (для МО группы 3 по доле МБТ 50% от доходов)<br/>Значение индикатора:&nbsp;<span style=\"color: white\"><b>{0:P2}</b></span><br/><span style=\"color: white\"><b>{1}</b></span><br/>{2}<br/>Предельное значение объема долга МО:&nbsp;<span style=\"color: white\"><b>{3:N2}</b></span>&nbsp;тыс.руб.",
        ...
        {
            UserParams.BudgetLevelEnum.Value = "[Уровни бюджета].[СКИФ].[Все].[Конс.бюджет субъекта]";
        ...
        private static DataTable GetBudetDataSource()
        {
            ...
            string query = DataProvider.GetQueryText("FO_0635_0015_budget_incomes_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);

            DataTable source = new DataTable();

            query = DataProvider.GetQueryText("FO_0635_0015_budget_outcomes_all_v");
            ...
            query = DataProvider.GetQueryText("FO_0635_0015_budget_outcomes_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0635_0015_budget_deficite_v");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            foreach (DataRow row in dt.Rows)
            {
                ...
                if (Double.TryParse(row["Факт по области прошлый год "].ToString(), out valueLastYear) &&
                   ...
                   valueLastYear != 0)
                {
                    row["Темп роста по округу"] = valueCurrYear/valueLastYear;
                }
            }
            dt.AcceptChanges();
            return dt;
        ...
        protected void UltraWebGridBudget_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns[6].Header.Caption = "Темп роста по округу %";
        ...
        protected void Grid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns[6].Header.Caption = e.Layout.Bands[0].Columns[6].Header.Caption.Split(';')[0];
        ...
        private void InitializeBudgetSettlement()
        {
            ...
            query = DataProvider.GetQueryText("FO_0635_0015_settlements_list");
        ...
        protected void UltraWebGridBudgetSettlement_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0635_0015_settlement_incomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);

            DataTable source = new DataTable();

            query = DataProvider.GetQueryText("FO_0635_0015_settlement_outcomes_all");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0635_0015_settlement_outcomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);
            ...
            source = new DataTable();
            query = DataProvider.GetQueryText("FO_0635_0015_settlement_deficite");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", source);

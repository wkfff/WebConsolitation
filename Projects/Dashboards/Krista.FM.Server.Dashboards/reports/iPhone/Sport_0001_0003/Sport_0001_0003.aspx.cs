...
namespace Krista.FM.Server.Dashboards.reports.iPhone
{
	...
	/// детализация РФ - занимающиеся, сравнение территорий
	/// </summary>
    public partial class Sport_0001_0003 : CustomReportPage
    {
    	public string TEMPORARY_URL_PREFIX = "../../..";
		public string REPORT_ID = "Sport_0001_0003";
    	...
    	protected override void Page_Load(object sender, EventArgs e)
        {
			...
			HeaderDisease.Text = String.Format("Спорт и заболеваемость по территориям");
			...
			chartDisease.SporterPopulation = FillOrderedData();
		...
		/// </summary>
		private OrderedData FillOrderedData()
		{
			DataTable msTable = DataProvider.GetDataTableForChart("Sport_0001_0003_minsport", DataProvidersFactory.PrimaryMASDataProvider);
			DataTable fmTable = DataProvider.GetDataTableForChart("Sport_0001_0003_population", DataProvidersFactory.SecondaryMASDataProvider);
	...
	{
		public Sport_0001_0003 Page { set; get; }
		public SGMEasy DiseaseData { set; get; }
		...
		protected override void SetData()
		{
			...
			OrderedData dataRank1 = new OrderedData();
			OrderedData dataRank2 = new OrderedData();
			
			OrderedData sporter = FillOrderedData(); 
			foreach (OrderedValue value in sporter.Data)
			{
				IDs.Add(value.ID);
				decimal disease = 100000 * DiseaseData.GetDiseaseDecimal(value.ID) / value.ExtraValue;

				DataRow row = Table.NewRow();
				
				if (RegionsNamingHelper.IsRF(value.ID))
					row[0] = String.Format("<a href=\"webcommand?showReport=Sport_0001_0001_fo=0\">{0}</a>", value.ID);
				else if (RegionsNamingHelper.IsFO(value.ID))
					row[0] = String.Format("<a href=\"webcommand?showReport=Sport_0001_0001_fo={0}\">{1}</a>", CustomParams.GetFOIdByName(value.ID), value.ID);
				else if (RegionsNamingHelper.IsSubject(value.ID))
				...
				if (RegionsNamingHelper.IsSubject(value.ID))
				{
					dataRank1.Add(new OrderedValue(value.ID, value.Value));
					dataRank2.Add(new OrderedValue(value.ID, disease));
				}
				
			}
			
			dataRank1.Sort();
			dataRank2.Inversed = true;
			dataRank2.Sort();
			...
			for (int i = 0; i < Table.Rows.Count; i++)
			{
				...
				{
					int rank = dataRank1.GetRank(id);
					row[2] = Convert.ToString(rank == dataRank1.MaxRank ? -rank : rank);
					rank = dataRank2.GetRank(id);
					row[4] = Convert.ToString(rank == dataRank2.MaxRank ? -rank : rank);
				}
			}
		...
		protected override void SetDataStyle()
		{
			...
			Band.Columns[1].Width = 140;
			Band.Columns[2].Width = 50;
			Band.Columns[3].Width = 160;
		...
		/// доля населения, занимающегося спортом
		/// </summary>
		private static OrderedData FillOrderedData()
		{
			DataTable msTable = DataProvider.GetDataTableForChart("Sport_0001_0003_minsport_grid", DataProvidersFactory.PrimaryMASDataProvider);
			DataTable fmTable = DataProvider.GetDataTableForChart("Sport_0001_0003_population_grid", DataProvidersFactory.SecondaryMASDataProvider);

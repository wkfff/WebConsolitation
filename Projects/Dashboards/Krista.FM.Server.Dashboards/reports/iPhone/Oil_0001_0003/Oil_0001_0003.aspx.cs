...
{
    public partial class Oil_0001_0003 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            //CustomParams.MakeOilParams("4", "id");

            string multitouchIcon = String.Empty;

            multitouchIcon = "<img src='../../../images/detail.png'>";
            detalizationIconDiv.InnerHtml = String.Format("<table><tr><td><a href='webcommand?showPinchReport=Oil_0001_0005_OIL={1}'>{0}</a></td>", multitouchIcon, HttpContext.Current.Session["CurrentOilID"]);
            detalizationIconDiv.InnerHtml += String.Format("<td><a href='webcommand?showPinchReport=Oil_0001_0004_OIL={1}'><img src='../../../images/lock.png'></a></td></tr></table>", multitouchIcon, HttpContext.Current.Session["CurrentOilID"]);

            string filename = HttpContext.Current.Server.MapPath("~/TemporaryImages/Oil_0001_0003/") + "TouchElementBounds.xml";
            Directory.CreateDirectory(HttpContext.Current.Server.MapPath("~/TemporaryImages/Oil_0001_0003/"));
            File.WriteAllText(filename, String.Format("<?xml version=\"1.0\" encoding=\"UTF-8\"?><touchElements><element id=\"Oil_0001_0005_OIL={0}\" bounds=\"x=0;y=700;width=768;height=100\" openMode=\"incomes\"/><element id=\"Oil_0001_0004_OIL={0}\" bounds=\"x=0;y=800;width=768;height=230\" openMode=\"outcomes\"/></touchElements>", HttpContext.Current.Session["CurrentOilID"]));

            IPadElementHeader4.Text = String.Format("Динамика розничных цен на {0} по ХМАО-Югре", UserParams.Oil.Value.ToLowerFirstSymbol());
        ...
        private void InitializeTable1()
        {
            ...
            string query = DataProvider.GetQueryText("Oil_0001_0003_incomes_regional_date");
            ...
            elementCaption.Text = String.Format("Сравнение средней цены по МО ХМАО-Югры на {0:dd.MM.yyyy}", currentDateRegional);
            ...
            dt = new DataTable();
            query = DataProvider.GetQueryText("Oil_0001_0003_table1");
            DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, " ", dt);

            #region Средняя цена по ХМАО

            DataTable dtHMAO = new DataTable();
            query = DataProvider.GetQueryText("Oil_0001_0003_grid_hmao");
            DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, " ", dtHMAO);

            dt.Rows[0][1] = MathHelper.GeoMean(dtHMAO.Columns[1], DBNull.Value);
            object prevHMAOValue = MathHelper.GeoMean(dtHMAO.Columns[2], DBNull.Value);
            object yearHMAOValue = MathHelper.GeoMean(dtHMAO.Columns[3], DBNull.Value);

            dt.Rows[0][2] = MathHelper.Minus(dt.Rows[0][1], prevHMAOValue);
            dt.Rows[0][3] = MathHelper.Mult(MathHelper.Grown(dt.Rows[0][1], prevHMAOValue), 100);
            dt.Rows[0][4] = MathHelper.Minus(dt.Rows[0][1], yearHMAOValue);
            dt.Rows[0][5] = MathHelper.Mult(MathHelper.Grown(dt.Rows[0][1], yearHMAOValue), 100);

            #endregion

            dt.Rows[0][0] = String.Format("<table style='border-collapse: collapse; margin-top: -3px; margin-bottom: -3px'><tr><td align='left' style='border: none; text-align:left'>Ханты-Мансийский автономный округ&ndash;Югра</td></tr><tr><td align='left' style='border: none; text-align:left'><span style='DigitsValue'>(по сплошному методу учета на {0:dd.MM.yyyy})</td></tr></table>", currentDateRegional);

            InitializeChart();

            dtDate = new DataTable();
            query = DataProvider.GetQueryText("Oil_0001_0003_incomes_federal_date");
            ...
            query = DataProvider.GetQueryText("Oil_0001_0003_table2");
            ...
            dt.Rows[1][0] = String.Format("<table style='border-collapse: collapse; margin-top: -3px; margin-bottom: -3px'><tr><td align='left' style='border: none; text-align:left'>Уральский федеральный округ</td></tr><tr><td align='left' style='border: none; text-align:left'><span style='DigitsValue'>({1} по данным Росстата на {0:dd.MM.yyyy})</span></td></tr></table>", currentDateFederal, UserParams.OilFederal.Value);
            dt.Rows[2][0] = String.Format("<table style='border-collapse: collapse; margin-top: -3px; margin-bottom: -3px'><tr><td align='left' style='border: none; text-align:left'>Российская Федерация</td></tr><tr><td align='left' style='border: none; text-align:left'><span style='DigitsValue'>({1} по данным Росстата на {0:dd.MM.yyyy})</span></td></tr></table>", currentDateFederal, UserParams.OilFederal.Value);
            ...
            if (HttpContext.Current.Session["CurrentOilID"].ToString() != "5")
            {
                for (int i = 0; i < dt.Rows.Count; i++)
                {
                    ...
                    string gaugeValue = String.Empty;
                    if (MathHelper.IsDouble(dt.Rows[i][1]))

                        gaugeValue = String.Format("{0:N2}", Convert.ToDouble(dt.Rows[i][1]));
        ...
        {
            string query = DataProvider.GetQueryText(String.Format("Oil_0001_0003_chart1"));
            ...
            if (dtChart.Rows.Count > 0)
            {
                ...
                foreach (DataRow row in dtChart.Rows)
                {
                    if (row[0] != DBNull.Value)
                    {
                        ...
                        row[1] = MathHelper.GeoMean(row, 1, 1, DBNull.Value);
                    }
                }

                UltraChart1.Series.Clear();
                NumericSeries series = CRHelper.GetNumericSeries(1, dtChart);
                series.Label = dtChart.Columns[1].ColumnName;
        ...
        private void SetupDynamicChart(UltraChart chart)
        {
        ...
        private void InitializeChart()
        {
            ...
            string query = DataProvider.GetQueryText("Oil_0001_0003_chart");
            DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "Dummy", dtChart);
        ...
        protected void UltraChart_FillSceneGraph(object sender, FillSceneGraphEventArgs e)
        {
            ...
            RFheader = string.Format("{0}: {1:N2} {2}", "Средняя по ХМАО-Югре", hmaoAvg, "руб.");

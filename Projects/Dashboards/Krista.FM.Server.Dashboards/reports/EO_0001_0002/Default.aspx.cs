...
namespace Krista.FM.Server.Dashboards.reports.EO_0001_0002
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            UltraWebGrid.Height = IsSmallResolution ? CRHelper.GetGridHeight(MinScreenHeight - 270) : CRHelper.GetGridHeight(MinScreenHeight - 210);
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("EO_0001_0002_date");
            ...
            Territory = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider, "EO_0001_0002_Territory");
            if (!Page.IsPostBack)
            {
                ComboTerritory.Title = "МО";
                ComboTerritory.Width = 230;
            ...
            Period = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider, "EO_0001_0002_Period");

            ComboPeriod.Title = "Период";
            ComboPeriod.Width = 230;
            ...
            else
            {
                ...
                //ComboPeriod.SetСheckedState(SelectState,true);
            }
            SelectState = ComboPeriod.SelectedValue;
            Customers = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider,
                                                       "EO_0001_0002_Customers");

            ComboCustomers.Title = "Заказчики";
            ComboCustomers.Width = 345;
            ...
            Page.Title = PageTitle.Text = "Исполнение целевых программ";
            PageSubTitle.Text = IsSmallResolution ? string.Format("Анализ исполнение муниципальных целевых программ <br/> {0}, {1}, за {2} ",
                cust, ComboTerritory.SelectedValue, ComboPeriod.SelectedValue) : string.Format("Анализ исполнение муниципальных целевых программ {0}, {1}, за {2} ",
        ...
        protected string GetGaugeUrl(object oValue)
        {
            ...
            if (value > 100)
                value = 100;
            string path = "EO_0001_0002_gauge_" + value.ToString("N0") + ".png";
        ...
        protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("EO_0001_0002_grid");
        ...
        void FillDynamicText()
        {
            ...
            string query = DataProvider.GetQueryText("EO_0001_0002_DynamicText");
            DataTable dtDynamicTextInfo = new DataTable();
            DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, " ", dtDynamicTextInfo);
            if (dtDynamicTextInfo.Rows.Count > 1)
            {
                ...
                DynamicText.Text =
                    string.Format(
                        "<span style = \" \">На <b>{0}</b> год в <b>{6}</b> действуют <b>{1}</b> {7} на общую сумму <b>{2:N2}</b> тыс.руб. По состоянию на <b>{3}</b> года фактически исполнено по целевым программам <b>{4:N2}</b> тыс.руб. (<b>{5:P2}</b>)</br>",
                        PeriodYear.Year, countRecord, sumPlan, stateOn, sumFact, percentFact,
                        ComboTerritory.SelectedValue.Replace("Город", "г."), dataDictionary[countRecord]);
                query = DataProvider.GetQueryText("EO_0001_0002_Top5Min");
                DataTable dtTop5Min = new DataTable();
                DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, " ", dtTop5Min);
                ...
                query = DataProvider.GetQueryText("EO_0001_0002_Top5Max");
                DataTable dtTop5Max = new DataTable();
                DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, " ", dtTop5Max);
        ...
        private void CreateDictionaryWords(int countRec)
        {
            ...
            while (i <= countRec)
            {
                dataDictionary.Add(i, "целевых программ");
                dataDictionary.Add(i + 1, "целевая программа");
                dataDictionary.Add(i + 2, "целевые программы");
                dataDictionary.Add(i + 3, "целевые программы");
                dataDictionary.Add(i + 4, "целевые программы");
                dataDictionary.Add(i + 5, "целевых программ");
                dataDictionary.Add(i + 6, "целевых программ");
                dataDictionary.Add(i + 7, "целевых программ");
                dataDictionary.Add(i + 8, "целевых программ");
                dataDictionary.Add(i + 9, "целевых программ");
                i = i + 10;
            }
        }
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            headerLayout.AddCell("План на " + PeriodYear.Year + "г., тыс.рублей");
            string[] splitPeriodQuarter = ComboPeriod.SelectedValue.Split(" ");
            headerLayout.AddCell("Факт на " + splitPeriodQuarter[0] + " " + splitPeriodQuarter[1] + " " + PeriodYear.Year + "г., тыс.рублей");

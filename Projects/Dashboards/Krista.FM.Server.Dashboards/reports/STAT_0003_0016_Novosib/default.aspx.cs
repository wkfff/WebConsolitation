 ...
 */
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0016_Novosib
{
    public partial class Default : CustomReportPage 
    {
        ...
        class SetComboDay : IDataSetCombo
        {
            public override void LoadData(DataTable Table)
            {
                ...
                foreach (DataRow row in Table.Rows)
                {
                    ...
                    string DisplayNAme = this.GetAlowableAndFormatedKey("Январь - " + Mounth + " " + Year + " года", "");

                    if (Mounth.Contains("Январь"))
                    {
                        DisplayNAme = this.GetAlowableAndFormatedKey("Январь " + Year + " года", "");
                    }
        ...
        class InfoRow
        {
            ...
            public InfoRow(string BaseName)
            {
                switch (BaseName)
                {
                    case "Коэффициент рождаемости":
                        {
                            ReportDisplay = "Коэффициент рождаемости";

                            Caption = "Коэффициент рождаемости";
                            EdIzm = "";
                    ...
                    case "Коэффициент смертности":
                        {
                            Caption = "Коэффициент смертности";
                            EdIzm = "";
                            ReportDisplay = "Коэффициент смертности";
                            FormatString = "N2";
                            Rang = true;
                            Revece = true;
                            ReveceRang = true;
                    ...
                    case "Коэффициент естественного прироста (убыли)":
                        {
                            Caption = "Коэффициент естественного прироста (убыли)";
                            EdIzm = "";
                            ReportDisplay = "Коэффициент естественного прироста (убыли)";
                            FormatString = "N2";
                            Rang = true;
                            Revece = false;
                            ReveceRang = false;
                    ...
                    case "Коэффициент миграционного прироста (убыли)":
                        {
                            Caption = "Коэффициент миграционного прироста (убыли)";
                            EdIzm = "";
                            ReportDisplay = "Коэффициент миграционного прироста (убыли)";
                            FormatString = "N2";
                            Rang = true;
                            Revece = false;
                            ReveceRang = false;
        ...
        {
            PageTitle.Text = "Мониторинг демографической ситуации (рейтинг территорий)";
            PageSubTitle.Text = string.Format("Данные мониторинга демографической ситуации в муниципальных районах и городских округах {1} по состоянию на {0}", ComboCurrentPeriod.SelectedValue, "Новосибирской области");
             
            Page.Title = PageTitle.Text;            
            
            LabelMap2.Text = string.Format(
                "«{0}», {1} за {2}",
            ...
            //SubLabelMap2.Text = string.Format("Рейтинг министерства труда, занятости и трудовых ресурсов Новосибирской области");

            LabelChart1.Text = string.Format("Распределение территорий по показателю «{0}», {1} за {2}",
                GlobalNavigationInfoRow.Caption,
                GlobalNavigationInfoRow.EdIzm,
                ComboCurrentPeriod.SelectedValue);
        }
        ...
        private void FillNaviagatorPanel()
        {
            ...
            TableListForChart.AddOrGetRow("Коэффициент рождаемости")["Uname"]
                = "[Население__Естественное движение].[Население__Естественное движение].[Все показатели].[Коэффициент рождаемости]";
            TableListForChart.AddOrGetRow("Коэффициент смертности")["Uname"]
                = "[Население__Естественное движение].[Население__Естественное движение].[Все показатели].[Коэффициент смертности]";
            TableListForChart.AddOrGetRow("Коэффициент естественного прироста (убыли)")["Uname"]
                = "[Население__Естественное движение].[Население__Естественное движение].[Все показатели].[Коэффициент естественного прироста (убыли)]";
            TableListForChart.AddOrGetRow("Коэффициент миграционного прироста (убыли)")["Uname"]
                = "[Население__Естественное движение].[Население__Естественное движение].[Все показатели].[Коэффициент миграционного прироста (убыли)]";
        ...
        private void CustomaizeMapko()
        {
            ...
            rule.FromColor = GlobalNavigationInfoRow.EndColor;
            rule.MiddleColor = GlobalNavigationInfoRow.CenterColor; 
            rule.ToColor = GlobalNavigationInfoRow.BeginColor; 
        ...
        public void FillMap1Data()
        {
            ...
            legend.Visible = TableMap.Rows.Count > 0;
        ...
        private void GenChart()
        {
            ...
            FirstValuyFromChart = (decimal)TableChart.Rows[0][0];
            ...
            { 
                ...
                FirstValuyFromChart = FirstValuyFromChart < FoValue ? FirstValuyFromChart : FoValue;
            }
            catch { }

            UltraChart1.Axis.Y.RangeMax = (double)LastValuyFromChart * 1.1;
            UltraChart1.Axis.Y.RangeMin = (double)FirstValuyFromChart;
        ...
        Primitive DrawLine(int startX, int StartY, int EndX, int EndY)
        {
            Infragistics.UltraChart.Core.Primitives.Line Line = new Infragistics.UltraChart.Core.Primitives.Line(new Point(startX, StartY), new Point(EndX, EndY));

            Line.PE.Stroke =Color.Black;
        ...
        void Chart_FillSceneGraph(object sender, Infragistics.UltraChart.Shared.Events.FillSceneGraphEventArgs e)
        {
            try
            {
                ...
                if (FirstValuyFromChart < 0)
                {

                    e.SceneGraph.Add(DrawLine(

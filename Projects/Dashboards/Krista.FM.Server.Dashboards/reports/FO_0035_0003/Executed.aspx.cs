...
namespace Krista.FM.Server.Dashboards.reports.FO_0035_0003
{
    public partial class Executed : CustomReportPage
    {
        ...
        public bool IsYar
        {
            get
            {
                return RegionSettings.Instance.Id.ToLower() == "yaroslavl";
            }
        }
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            if (IsQuaterPlanType)
            {
                PopupInformer1.HelpPageUrl = "QuarterExecutedHelp.html";
            }
            else
            {
                PopupInformer1.HelpPageUrl = (IsKursk) ? "WeeklyYearExecutedHelp.html" : "DailyYearExecutedHelp.html";
            }
            ...
            Link2.NavigateUrl = "~/reports/FO_0035_0001/Default.aspx";

            Link3.Visible = true;
            Link3.Text = "Структура&nbsp;расходов&nbsp;областного&nbsp;бюджета&nbsp;по&nbsp;ведомствам";
            Link3.NavigateUrl = "~/reports/FO_0035_0003/Kinds.aspx";
        }
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                UserParams.CustomParam("period_quater").Value =
                    CRHelper.PeriodMemberUName(
                        String.Empty, CustomCalendar1.WebCalendar.SelectedDate, 3);
            }
        ...
        private void AdditionalSetupGrid()
        {
            ...
            //            }
            grid.Rows[headerRowsCount].Style.BorderDetails.ColorTop =
                    Color.FromArgb(192, 192, 192);
            grid.Rows[headerRowsCount].Style.Font.Bold = true;
            grid.Rows[grid.Rows.Count - footerRowsCount - 1].Style.BorderDetails.ColorTop =
                    Color.FromArgb(192, 192, 192);
            grid.Rows[grid.Rows.Count - 1].Style.BorderDetails.ColorTop =
                    Color.FromArgb(192, 192, 192);
        ...
        private void AddNewSourceRow(DataTable dtSource, int rowsCounter)
        {
            ...
            if (Double.TryParse(dtExecute.Rows[rowsCounter][4].ToString(), out value) && rowsCounter != 0)
            {
        ...
        protected void grid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            for (int i = 2; i < e.Layout.Bands[0].Columns.Count - 1; i = i + 3)
            {
                ...
                CRHelper.FormatNumberColumn(e.Layout.Bands[0].Columns[i], "N0");
                e.Layout.Bands[0].Columns[i].Header.Title = (IsQuaterPlanType) ? "Квартальные назначения" : "Годовые назначения";

                captions = e.Layout.Bands[0].Columns[i + 1].Header.Caption.Split(';');
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, i + 1, "Факт", "");
                CRHelper.FormatNumberColumn(e.Layout.Bands[0].Columns[i + 1], "N0");
                e.Layout.Bands[0].Columns[i + 1].Header.Title = (IsQuaterPlanType) ? "Исполнено с начала квартала" : "Исполнено с начала года";
                    ...
                    case 5:
                        {
                            assessionText = string.Format("выплаты до 6 и 21 числа каждого месяца не менее 1/{0} {1} плана", 
                                2 * Devider, IsQuaterPlanType ? "квартального" : "годового");
                            break;
                        }
                    ...
                    case 14:
                        {
                            assessionText = string.Format("на конец месяца должно быть исполнено 1/{0} {1} плана",
                                Devider, IsQuaterPlanType ? "квартального" : "годового");
                            break;
                        }
                    // Прочие
                    case 17:
                        {
                            assessionText = string.Format("равномерное расходование по числу дней {0}", IsQuaterPlanType ? "квартала" : "в году");
                            break;
                        }
                }
                if (i != 2)
        ...
        private double SalaryAssessionLimit()
        {
            ...
            double monthNum = (IsQuaterPlanType) ? CRHelper.MonthNumInQuarter(currentDate.Month) : currentDate.Month;
        ...
        private double OthersAssessionLimit()
        {
            if (IsQuaterPlanType)
            {
                return CRHelper.QuarterDaysCountToDate(currentDate) / CRHelper.QuarterDaysCount(currentDate);
            }
            else
            {

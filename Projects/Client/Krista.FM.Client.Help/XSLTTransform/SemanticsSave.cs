...
{
    public class SemanticsSave : CommonSave
    {
        public SemanticsSave(HelpManager manager)
            : base(manager)
        {
            CreateTransform();
        }
        ...
        public override void SaveToHtml(System.Xml.XmlNode nodeBase)
        {
            try
            {
                ...
                XmlWriter writer =
                    XmlWriter.Create(
                        String.Format(
                        @"{0}\{1}\semantics.html", AppDomain.CurrentDomain.BaseDirectory, HelpManager.result), settings);
        ...
        protected override void CreateTransform()
        {
            ...
            xslt.Load(String.Format(@"{0}\Semantics.xsl", HelpManager.profile));
        }
    }
}

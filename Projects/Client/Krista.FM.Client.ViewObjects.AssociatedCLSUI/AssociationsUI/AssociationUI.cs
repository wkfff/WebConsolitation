...
namespace Krista.FM.Client.ViewObjects.AssociatedCLSUI.Association
{
    public partial class AssociationUI : BaseViewObj
    {
        ...
        public override void InitializeData()
        {
            ...
            btn1 = (ButtonTool)associateView.utbmAssociate.Tools["ReplaceLink"];
            btn1.SharedProps.Visible = curentAssociation.AssociationClassType == AssociationClassTypes.BridgeBridge
                                           ? true
                                           : false;
            ...
            /*attClsData.UltraGridExComponent.IsReadOnly = false;
            attClsBridge.UltraGridExComponent.IsReadOnly = false;
            attClsData.UltraGridExComponent.AllowDeleteRows = false;
            attClsBridge.UltraGridExComponent.AllowDeleteRows = false;
            attClsData.UltraGridExComponent.AllowAddNewRecords = false;
            attClsBridge.UltraGridExComponent.AllowAddNewRecords = false;
            attClsData.UltraGridExComponent.AllowClearTable = false;
            attClsBridge.UltraGridExComponent.AllowClearTable = false;*/
        }
        ...
        private void AttachClsClassifiers()
        {
            ...
            if (curentAssociation.AssociationClassType == AssociationClassTypes.BridgeBridge)
            {
                attClsData.SaveLastSelectedDataSource = false;
            }
            ...
            if (curentAssociation.AssociationClassType == AssociationClassTypes.Bridge)
            {
                attClsBridge.SaveLastSelectedDataSource = false;
            }

            attClsBridge.AttachCls(associateView.scAssociation.Panel2, ref attClsBridge);
            attClsBridge.SelectDataSource += new VoidDelegate(AttClsBridge_SelectDataSource);
            attClsBridge.RefreshData += new VoidDelegate(attClsBridge_RefreshData);
            ...
            attClsData.InAssociateMode = true;
            attClsBridge.InAssociateMode = true;

            SetRelinkEnable();
        }

        void attClsBridge_RefreshData()
        {
            AttClsBridge_SelectDataSource();
            SetRelinkEnable();
        }
        ...
        void AttClsBridge_SelectDataSource()
        {
            if (!attClsBridge.IsCurrentVersion && curentAssociation.AssociationClassType != AssociationClassTypes.BridgeBridge)
            {
                ButtonTool buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["HandAssociate"];
                buttonTool.SharedProps.Enabled = false;
                buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["AssociateMaster"];
                buttonTool.SharedProps.Enabled = false;
                buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["AddToBridge"];
                buttonTool.SharedProps.Enabled = false;
                buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["AddToBridgeAll"];
                buttonTool.SharedProps.Enabled = false;
                buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["ClearCurentBridgeRef"];
                buttonTool.SharedProps.Enabled = false;
                buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["ClearAllBridgeRef"];
                buttonTool.SharedProps.Enabled = false;
            }
            else
            {
        ...
        private void SetRelinkEnable()
        {
            if (curentAssociation.AssociationClassType == AssociationClassTypes.BridgeBridge)
            {
                ButtonTool buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["CreateBridge"];
                buttonTool.SharedProps.Enabled = false;

                if (!attClsData.IsCurrentVersion || attClsBridge.IsCurrentVersion)
                {
                    buttonTool = (ButtonTool) associateView.utbmAssociate.Tools["HandAssociate"];
                    buttonTool.SharedProps.Enabled = false;
                    buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["AssociateMaster"];
                    buttonTool.SharedProps.Enabled = false;
                    buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["ReplaceLink"];
                    buttonTool.SharedProps.Enabled = false;
                    buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["AddToBridge"];
                    buttonTool.SharedProps.Enabled = false;
                    buttonTool = (ButtonTool)associateView.utbmAssociate.Tools["AddToBridgeAll"];
        ...
        void clsDataGrid_DragDrop(object sender, DragEventArgs e)
        {
            ...
            if (LastEnteredRow != null)
            {
                ...
                if (MessageBox.Show("Добавить запись в сопоставимый классификатор?", "Добавление записи", MessageBoxButtons.YesNo, MessageBoxIcon.Question) == DialogResult.Yes)
                    try
                    {
                        curentAssociation.CopyAndAssociateRow(Convert.ToInt32(DragDropRow.Cells["ID"].Value), attClsBridge.CurrentSourceID);
                        DragDropRow.Update();
                        needRefresh = true;
                    }
                    catch
        ...
        void clsDataGrid_SelectionDrag(object sender, System.ComponentModel.CancelEventArgs e)
        {
            ...
            // запрещаем сопоставление по хитрым алгоритмам
            if (curentAssociation.AssociationClassType == AssociationClassTypes.BridgeBridge)
            {
                if (!attClsData.IsCurrentVersion || attClsBridge.IsCurrentVersion) 
                    return;
        ...
        private void GetExcelReport()
        {
            ...
            query = query + string.Format(" from {0} dataCls, {1} bridgeCls where ((dataCls.{2} = bridgeCls.{3}) or (dataCls.{2} is null))",
                curentAssociation.RoleData.FullDBName, curentAssociation.RoleBridge.FullDBName,
                curentAssociation.FullDBName, "ID");

            string fileName = captionString.Replace(">", string.Empty);
            if (ExportImportHelper.GetFileName(fileName, "xls", true, ref fileName))
            {
                this.Workplace.OperationObj.Text = "Построение отчета";
                this.Workplace.OperationObj.StartOperation();
                Workbook wb = string.Compare(Path.GetExtension(fileName), "xls", true) == 0 ? new Workbook() : new Workbook(WorkbookFormat.Excel2007);
                ...
                tmpGrid.InitializeRow += tmpGrid_InitializeRow;
                ...
                try
                {
                    ...
                    foreach (KeyValuePair<int, string> kvp in dataSources)
                    {
                        ...
                        if (dt != null)
                        {
                            ...
                            Worksheet ws = wb.Worksheets.Add(string.Format("SourceID={0}", kvp.Key));

                            WorksheetRow excelRow = ws.Rows[0];

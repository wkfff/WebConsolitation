...
namespace Krista.FM.Server.Dashboards.reports.iPhone
{
	...
	/// детализация субъекта - кадры (годовая динамика)
	/// </summary>
    public partial class Sport_0001_0009 : CustomReportPage
    {
    	public string TEMPORARY_URL_PREFIX = "../../..";
		public string REPORT_ID = "Sport_0001_0009";
    	...
    	protected override void Page_Load(object sender, EventArgs e)
        {
			...
			DataTable msTable = DataProvider.GetDataTableForChart("Sport_0001_0009_minsport", DataProvidersFactory.PrimaryMASDataProvider);
			DataTable fmTable = DataProvider.GetDataTableForChart("Sport_0001_0009_population", DataProvidersFactory.SecondaryMASDataProvider);
	...
	public class GridHelper : UltraGridWrapper
	{
		...
		protected override void SetData()
		{
			...
			for (int i = 0; i < MsTable.Rows.Count; i++)
			{
				...
				DataRow row = Table.NewRow();
				row[0] = id;
				row[1] = Convert.ToDouble(CRHelper.DBValueConvertToDoubleOrZero(vStaff)).ToString("N0");
				row[2] = Convert.ToDouble(10000 * CRHelper.DBValueConvertToDoubleOrZero(vStaff) / CRHelper.DBValueConvertToDoubleOrZero(vPopulation)).ToString("N2");
				row[3] = Convert.ToDouble(CRHelper.DBValueConvertToDoubleOrZero(vSporter) / CRHelper.DBValueConvertToDoubleOrZero(vStaff)).ToString("N2");
				Table.Rows.Add(row);
			}

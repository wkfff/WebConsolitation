...
namespace Krista.FM.Server.Dashboards.reports.FO_0002_0001_Saratov
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string queryName =
                    string.Format("FO_0002_0001_Saratov_date_{0}",
                        ...
                        else
                        {
                            ...
                            if (selectedRegionType.Contains("МР"))
                            {
                                ...
                                switch (SelectedBudgetIndex)
                                {
                                    ...
                                    case 1:
                                        {
                                            UserParams.BudgetLevelEnum.Value = "[Уровни бюджета].[СКИФ].[Все].[Бюджет района]";
                                            break;
                                        }
                                    case 2:
                                        {
                                            UserParams.BudgetLevelEnum.Value = "[Уровни бюджета].[СКИФ].[Все].[Бюджет поселения]";
                                            break;
                                        }
                                }
        ...
        {
            string selectedBudget = regionSelected
                                        ? String.Format("{0} ({1})", ComboBudget.SelectedValue, MeasureButtonList.SelectedValue)
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Saratov_ekrOutcomes");
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Saratov_fkrOutcomes");
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Saratov_Incomes");
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Saratov_Deficit");
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Saratov_Debts");
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Saratov_fkrOutcomes_total");
...
//            e.Layout.Grid.Columns[0].Width = CRHelper.GetColumnWidth(275);
            ...
            bool isLeftColumnGrid = (gridCaption == outcomesTotalGridCaption || gridCaption == incomesGridCaption || gridCaption == balanceGridCaption);
            bool rateColumnHidden = (gridCaption == debtsGridCaption || gridCaption == balanceGridCaption);

            e.Layout.Grid.Columns[0].Width = CRHelper.GetColumnWidth(isLeftColumnGrid ? 210 : 240);
            ...
            e.Layout.Grid.Columns[i + 1].Width = CRHelper.GetColumnWidth(85);
            e.Layout.Grid.Columns[i + 2].Width = CRHelper.GetColumnWidth(gridCaption == balanceGridCaption ? 218 : gridCaption == debtsGridCaption ? 338 : 85);
            e.Layout.Grid.Columns[i + 3].Width = CRHelper.GetColumnWidth(70);
            e.Layout.Grid.Columns[i + 4].Width = CRHelper.GetColumnWidth(60);
            ...
            e.Layout.Grid.Columns[i + 1].Hidden = gridCaption == debtsGridCaption;
            ...
            e.Layout.Grid.Columns[i + 3].Hidden = rateColumnHidden;

            e.Layout.Grid.Columns[i + 4].Header.Caption = "Темп роста";
            e.Layout.Grid.Columns[i + 4].Header.Title = "Темп роста к прошлому году";
            e.Layout.Grid.Columns[i + 4].Hidden = rateColumnHidden;
            ...
            CRHelper.FormatNumberColumn(e.Layout.Grid.Columns[i + 4], "N0");
        }

        private bool IsInverseIndicator(string gridCaption)
        {
            // изменения по индикации откатили обратно
            return false;//(gridCaption == FKRGridCaption || gridCaption == EKRGridCaption || gridCaption == outcomesTotalGridCaption);
        }
        ...
        protected void grid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            {
                bool growRateColumn = (e.Row.Band.Grid.Columns[i].Header.Caption.ToLower().Contains("темп роста"));
                    ...
                    gridCaption == EKRGridCaption || gridCaption == outcomesTotalGridCaption)
                {
                    ...
                    if (growRateColumn)
                    {
                        if (e.Row.Cells[i].Value != null && e.Row.Cells[i].Value.ToString() != String.Empty)
                        {
                            ...
                            {
                                e.Row.Cells[i].Style.BackgroundImage = IsInverseIndicator(gridCaption) ? "~/images/arrowRedUpBB.png" : "~/images/arrowGreenUpBB.png";
                                e.Row.Cells[i].Title = "Прирост к прошлому году";
                            }
                            else if (Convert.ToDouble(e.Row.Cells[i].Value) < 100)
                            {
                                e.Row.Cells[i].Style.BackgroundImage = IsInverseIndicator(gridCaption) ? "~/images/arrowGreenDownBB.png" : "~/images/arrowRedDownBB.png";
                    ...
                    if (gridCaption == incomesGridCaption && e.Row.Cells[1] != null)
                    {
                        string rowName = e.Row.Cells[0].ToString();

                        if (rowName.ToUpper().Contains("НАЛОГОВЫЕ И НЕНАЛОГОВЫЕ ДОХОДЫ") ||
                            rowName.ToUpper().Contains("БЕЗВОЗМЕЗДНЫЕ ПОСТУПЛЕНИЯ") ||
                            rowName.ToUpper().Contains("ДОХОДЫ БЮДЖЕТОВ БЮДЖЕТНОЙ СИСТЕМЫ РОССИЙСКОЙ ФЕДЕРАЦИИ ОТ ВОЗВРАТА ОСТАТКОВ СУБСИДИЙ И СУБВЕНЦИЙ ПРОШЛЫХ ЛЕТ") ||
                            rowName.Contains("Доходы бюджета - ИТОГО"))
                        {
        ...
        private void ExcelExporter_EndExport(object sender, EndExportEventArgs e)
        {
            foreach (Worksheet sheet in e.Workbook.Worksheets)
            {
                if (sheet.Name == "Расходы по КОСГУ" || sheet.Name == "Расходы по РзПр")
                {
                    ...
                    sheet.Columns[5].CellFormat.FormatString = "#,##0.0";
                }
                else
                {
                    sheet.Columns[0].Width = 340 * 37;
        ...
        private void InitializeExportLayout(DocumentExportEventArgs e)
        {
            ...
            if (gridCaption == balanceGridCaption)
            {
                ...
                e.Layout.Grid.Columns[i + 2].Width = 200;
            }
            else if (gridCaption == debtsGridCaption)
            {
                e.Layout.Grid.Columns[i + 2].Width = 360;
            }

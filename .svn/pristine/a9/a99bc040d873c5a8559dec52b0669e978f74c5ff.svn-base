...
namespace Krista.FM.Server.Dashboards.reports.FNS_0001_0001_Omsk
{
    public partial class DefaultRegions : CustomReportPage
    {
        ...
        const string regionTypeColumnName = "Тип территории";

        RankCalculator moRank = new RankCalculator(RankDirection.Desc);
        RankCalculator northZoneRank = new RankCalculator(RankDirection.Desc);
        RankCalculator northForestZoneRank = new RankCalculator(RankDirection.Desc);
        RankCalculator southForestZoneRank = new RankCalculator(RankDirection.Desc);
        RankCalculator southSteppeZoneRank = new RankCalculator(RankDirection.Desc);
        ...
        // условие ранга
        private CustomParam rankCondiiton;
        // множество районов для расчета ранга
        private CustomParam rankRegionSet;
...
//            UltraChart2.Axis.X.StripLines.Visible = true;
            ...
            rankCondiiton = UserParams.CustomParam("rank_condition");
            rankRegionSet = UserParams.CustomParam("rank_region_set");
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string queryName = "FNS_0001_0001_Omsk_date_split";
            ...
            PageSubTitle.Text = string.Format("{3} за {0} {1} {2} года", month, CRHelper.RusManyMonthGenitive(month), year, ComboKD.SelectedValue.TrimEnd(' '));

            rankCondiiton.Value = UseClimaticZones ? "Measures.[isClimaticZone]" : "Measures.[isMR]";
            rankRegionSet.Value = UseClimaticZones ? "Климатические зоны" : "Районы";
            gridRegionSet.Value = UseClimaticZones ? "Климатические зоны c раойнами" : "Районы и города";
            ...
            if (!chartWebAsyncPanel2.IsAsyncPostBack)
            {
                if (UseClimaticZones)
                {
                    chart1RegionSet.Value = Chart1DistrictExcluding ? "Климатические зоны" : "Климатические зоны с ГО";
                }
                else
                {
            ...
            if (!chartWebAsyncPanel1.IsAsyncPostBack)
            {
                if (UseClimaticZones)
                {
                    chart2RegionSet.Value = Chart2DistrictExcluding ? "Климатические зоны" : "Климатические зоны с ГО";
                }
                else
                {
        ...
        {
            string queryName = "FNS_0001_0001_Omsk_regions_grid_split";
            string query = DataProvider.GetQueryText(queryName);
            ...
            {
                    query = DataProvider.GetQueryText("FNS_0001_0001_Omsk_regions_grid_split_monthReport");
                    ...
                    if (dtMonthReportGrid.Columns.Count > 1 && dtMonthReportGrid.Rows.Count > 0)
                    {
                            ...
                            if (monthReportRow[0] != DBNull.Value)
                            {
                                ...
                                if (monthReportRow[allIncomesColumnName] != DBNull.Value && monthReportRow[allIncomesColumnName].ToString() != String.Empty)
                                {
                                    ...
                                    if (dtRow != null)
                                    {
                                        ...
                                        RankCalculator densityRank = GetRankCalculator(dtRow);
                        ...
                        foreach (DataRow row in dtGrid.Rows)
                        {
                            ...
                            if (IsRankingRow(rowName) && row[densityColumnName] != DBNull.Value && row[densityColumnName].ToString() != String.Empty)
                            {
                                RankCalculator densityRank = GetRankCalculator(row);
        ...
        private RankCalculator GetRankCalculator(DataRow row)
        {
            ...
            if (row[regionTypeColumnName] != DBNull.Value)
            {
                regionType = row[regionTypeColumnName].ToString();
            }
        ...
        private static bool IsRankingRow(string rowName)
        {
            return rowName != "Консолидированный бюджет субъекта" && rowName != "Местные бюджеты" &&
                rowName != RegionSettingsHelper.Instance.OwnSubjectBudgetName && rowName != "Итого по ГО" && rowName != "Итого по МР" && rowName != "г.Омск";
        }
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            if (e.Layout.Bands[0].Columns.Count > 14)
            {
                ...
                SetColumnParams(e.Layout, 0, 4, "N0", rankWidth, false);
                ...
                SetColumnParams(e.Layout, 0, 16, "N0", width, true);
                SetColumnParams(e.Layout, 0, 17, "N0", width, true);
                ...
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 4, string.Format("Ранг по абсолютной сумме недоимки на 01.{0:00}.{1}", nextMonth, nextYear), "Ранг по абсолютной сумме недоимки");
        ...
        protected void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
        {
            for (int i = 0; i < e.Row.Cells.Count - 2; i++)
            {
                bool grow = (i == 6);
                bool growRate = (i == 9);
                bool rank = (i == 4 || i == 7 || i == 10 || i == 14);
                bool redValue = (i == 5 || i == 6);
                bool isClimaticZone = e.Row.Cells[0].ToString().Contains(" зона");

                int worseRankIndex = (i == 4) ? e.Row.Cells.Count - 2 : i + 1;

                if (e.Row.Cells[0].Value != null &&
                    (e.Row.Cells[0].ToString() == "Все территории" || isClimaticZone))
                {
                ...
                {
                    if (e.Row.Cells[i].Value != null && e.Row.Cells[worseRankIndex].Value != null &&
                        e.Row.Cells[i].Value.ToString() != string.Empty &&
                        e.Row.Cells[worseRankIndex].Value.ToString() != string.Empty)
                    {
                        ...
                        switch(i)
                        {
                            case 4:
                                {
                                    indicatorName = "абсолютный прирост недоимки";
                                    break;
                                }
                            case 7:
                                {
                        ...
                        else if (Convert.ToInt32(e.Row.Cells[i].Value) == Convert.ToInt32(e.Row.Cells[worseRankIndex].Value))
                        {
        ...
        {
            string queryName = "FNS_0001_0001_Omsk_regions_chart1_split";
            string query = DataProvider.GetQueryText(queryName);
        ...
        {
            string queryName = "FNS_0001_0001_Omsk_regions_chart2_split";
            string query = DataProvider.GetQueryText(queryName);
        ...
        private void ExcelExporter_EndExport(object sender, EndExportEventArgs e)
        {
            ...
            e.CurrentWorksheet.Columns[15].Width = width;
            ...
            e.CurrentWorksheet.Columns[4].CellFormat.FormatString = "#0";

...
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0002
{
	public partial class Default : CustomReportPage
	{
		...
		private const string PageSubTitleCaption = "Данные еженедельного мониторинга ситуации на рынке труда в ХМАО-Югре по выбранному муниципальному образованию";
		...
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			#region Ссылки
			CrossLink1.Text = "Мониторинг&nbspситуации&nbspна&nbspрынке&nbspтруда ХМАО-Югры";
			CrossLink1.NavigateUrl = "~/reports/STAT_0003_0001/Default.aspx";
			#endregion
		...
		protected void MakeDynamicText()
		{
			...
			DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "Период", dtText);
			...
			string text3 = "<br/>Численность населения на начало года составила <b>{0:N3}</b> тыс.чел., численность экономически активного населения составила <b>{1:N3}</b> тыс.чел. (<b>{2:P2}</b> от численности населения).";
		...
		protected void UltraWebGrid_InitializeLayout(object sender, Infragistics.WebUI.UltraWebGrid.LayoutEventArgs e)
		{
			...
			for (int i = 1; i < e.Layout.Bands[0].Columns.Count; ++i)
			{
                ...
                headerLayout.AddCell(e.Layout.Bands[0].Columns[i].Key.Contains("Численность незанятых граждан, состоящих на учёте в органах службы занятости населения, человек") ? e.Layout.Bands[0].Columns[i].Key + " (на начало месяца)" : e.Layout.Bands[0].Columns[i].Key);
			...
			for (int i = 1; i < e.Row.Cells.Count; ++i)
			{
				...
				if ((i == 3 || i == 6) && (e.Row.Index % 3 == 2))
				{
					if (Convert.ToInt32(cell.Value) == 1)
					{
                        ...
                        if (i == 3)
                        {
                            cell.Title = String.Format("Ранг по ХМАО-Югре на {1}: {0}\nСамый низкий уровень безработицы", cell.Value, date);
                        }
                        else
                        {
					...
					{
						cell.Title = String.Format("Ранг по ХМАО-Югре на {1}: {0}", cell.Value, date);
					}
				}

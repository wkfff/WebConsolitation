namespace MDXParser
{
    ...
    internal class MDXBinOpNode : MDXExpNode
    {
                ...
                if (!analyzer.InCommonSubExpr)
                {
                    ...
                    if (this.m_Op.Equals("=") || this.m_Op.Equals("<>"))
                    {
                        if (IsNameFunction(this.m_Exp1) || IsNameFunction(this.m_Exp2))
                        {
                            ...
                            m.Id = 12;
                            m.Text = "Use IS operator to compare objects instead of comparing them by name";
                            m.URL = "http://sqlblog.com/blogs/mosha/archive/2004/11/04/comparing-members-in-mdx.aspx";
                            m.Severity = 3;
                            analyzer.Add(m);
                        }
                        if (!this.m_Exp1.IsScalar() && !this.m_Exp2.IsScalar())
                        {
                            ...
                            message2.Id = 13;
                            message2.Text = string.Format("Operator '{0}' compares cell values. Consider comparing objects instead with IS operator", this.m_Op);
                            message2.Severity = 4;
                        ...
                        if (this.m_Exp1.GetLabel().Equals("NULL", StringComparison.InvariantCultureIgnoreCase) || this.m_Exp2.GetLabel().Equals("NULL", StringComparison.InvariantCultureIgnoreCase))
                        {
                            ...
                            message3.Id = 14;
                            message3.Text = "If you want to avoid division by zero - use 'value = 0' check, if you want to check whether cell is empty - use IsEmpty function, if you want to check that MDX member or tuple exists - use 'obj IS NULL' check";
                            message3.URL = "http://sqlblog.com/blogs/mosha/archive/2005/06/30/how-to-check-if-cell-is-empty-in-mdx.aspx";
                            message3.Severity = 3;
                    ...
                    if (this.m_Op.Equals("IS", StringComparison.InvariantCultureIgnoreCase) && (this.m_Exp1.GetLabel().Equals("NULL", StringComparison.InvariantCultureIgnoreCase) || this.m_Exp2.GetLabel().Equals("NULL", StringComparison.InvariantCultureIgnoreCase)))
                    {
                        ...
                        message4.Id = 15;
                        message4.Text = "If you want to avoid division by zero - use 'value = 0' check, if you want to check whether cell is empty - use IsEmpty function, if you want to check that MDX member or tuple exists - use 'obj IS NULL' check";
                        message4.URL = "http://sqlblog.com/blogs/mosha/archive/2005/06/30/how-to-check-if-cell-is-empty-in-mdx.aspx";
                        message4.Severity = 3;
                    ...
                    if (this.m_Op.Equals("*") && (this.m_Exp1.GetLabel().Equals("100") || this.m_Exp2.GetLabel().Equals("100")))
                    {
                        ...
                        message5.Id = 0x10;
                        message5.Text = "Are you trying to convert to percentage by multiplying by 100 ? If yes, consider using FORMAT_STRING='Percent' instead";
                        message5.Severity = 1;
                    ...
                    if (this.IsCompareOp() && ((IsFunction(this.m_Exp1, "Rank") && IsConstant(this.m_Exp2, 0)) || (IsFunction(this.m_Exp2, "Rank") && IsConstant(this.m_Exp1, 0))))
                    {
                        ...
                        message6.Id = 0x2c;
                        message6.Text = "Check inclusion of the element in the set can be done more efficiently with Intersect rather than with Rank";
                        message6.Severity = 1;
        ...
        internal override void AppendMDX(StringBuilder mdx, Formatter f, int indent, bool comma)
        {
            ...
            {
                string str = (this.m_Op.Length > 1) ? f.Keyword(this.m_Op) : this.m_Op;

    ...
    public partial class Oil_0006_0002_Text : UserControl
    {
        ...
        private void ChartDataBind()
        {
            ...
            if (dtChart.Rows.Count > 0)
            {
                ...
                Label1.Text = String.Format("На <span class='DigitsValue'>{0:dd.MM.yyyy}</span> г. рост цен на <span class='DigitsValue'>{2}</span> к {1:dd.MM.yyyy} г. наблюдается в следующих субъектах РФ и компаниях",
                    reportDate, lastDate, CRHelper.ToLowerFirstSymbol(oilName));
            }
            else
            {
                UltraChart.Visible = false;
                Label1.Text = String.Format("На <span class='DigitsValue'>{0:dd.MM.yyyy}</span> г. в субъектах РФ <span class='DigitsValue'>не наблюдается</span> роста цен к {1:dd.MM.yyyy} г. на <span class='DigitsValue'>{2}</span>",
                    reportDate, lastDate, CRHelper.ToLowerFirstSymbol(oilName));
            }
        }
        ...
        protected void UltraChart_FillSceneGraph(object sender, FillSceneGraphEventArgs e)
        {
            ...
            if (avg != Double.MinValue)
            {
               ...
               Text text = new Text();
                ...
                text.SetTextString(String.Format("Среднее по РФ\n{0:N2} руб.", avg));
                e.SceneGraph.Add(text);
            }
        }

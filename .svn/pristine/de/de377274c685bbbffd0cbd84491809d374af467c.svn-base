...
function processSaveQuestion(button) {
    if (button == 'yes') {
        ...
        var changeStatus = function (store) {
        ...
        var recursiveFunc = function (item) {
            try {
                var store = eval('StoreRow_' + item.id);
                store.data.each(recursiveFunc);
                changeStatus(store);
            } catch (e) {
            }
        };
        dsMarks.data.each(recursiveFunc);
        changeStatus(dsMarks);
    }
...
function showCellTip() {
    ...
    else if (cellIndex >= 8 && cellIndex <= 12 && record.get('OKEI') == 'ДА/НЕТ') {

...
{
    public partial class STAT_0001_0002_v : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            Label3.Text = string.Format("На {0:dd.MM.yyyy} по {1}", date, RegionsNamingHelper.ShortName(UserParams.StateArea.Value));
        ...
        {
            dt = new DataTable();
            string query = DataProvider.GetQueryText("STAT_0001_0002_debts_v");
            ...
            for (int i = 0; i < dt.Rows.Count; i++)
            {
                if ((dt.Rows[i][0].ToString().Contains("Уровень регистрируемой безработицы") ||
                             dt.Rows[i][0].ToString().Contains("Коэффициент напряженности на рынке труда")) && dt.Rows[i][0].ToString().Contains("Прирост"))
                {
        ...
        {
            dt = new DataTable();
            string query = DataProvider.GetQueryText("STAT_0001_0002_v");
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);
        ...
        private void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
        {
            for (int i = 1; i < e.Row.Cells.Count; i++)
            {
                if (e.Row.Index % 3 == 0 && e.Row.Cells[i].Value != null)
                {
                    ...
                    else if (e.Row.Cells[0].Value.ToString() == "Коэффициент напряженности на рынке труда")
                    {

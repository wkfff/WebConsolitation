...
namespace Krista.FM.Server.Dashboards.reports.FK_0004_0008
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            UltraGridBrick1.AutoSizeStyle = GridAutoSizeStyle.None;
            UltraGridBrick1.Height = 600;
            UltraGridBrick1.Width = Convert.ToInt32(CustomReportConst.minScreenWidth - 25);
            UltraGridBrick1.Grid.InitializeLayout += new InitializeLayoutEventHandler(GRBSGrid_InitializeLayout1);
            UltraGridBrick1.Grid.InitializeRow += new InitializeRowEventHandler(Grid_InitializeRow1);
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                    ...
                    selectedPeriod.Value != String.Empty)
                {
                    ...
                    // CustomCalendar1.WebCalendar.SelectedDate = CubeInfo.GetLastDate(DataProvidersFactory.SecondaryMASDataProvider, "FK_0004_0008_lastDate");
            ...
            Page.Title = String.Format("Характеристика исполнения основных показателей доходов и поступлений от ГАДБ федерального бюджета ");
            Label1.Text = Page.Title;
            Label2.Text = String.Format("по состоянию на <b>{0:dd.MM.yyyy} г., {1}</b>", currentDate, multiplierCaption);

            Label3.Text = String.Format("Исполнение доходов федерального бюджета Главными администраторами доходов бюджета по состоянию на {0:dd.MM.yyyy} года", currentDate);
            Label4.Text = String.Format("Исполнение доходов федерального бюджета по основным источникам поступлений по состоянию на {0:dd.MM.yyyy} года", currentDate);
...
<tr><td align='right'><b>{1:N2}%</b></td><td> {2}</td></tr>", lbInfo.Text, dtInfo.Rows[i][2], dtInfo.Rows[i][1]);
            }
...
<img src='../../images/StarYellowBB.png'>&nbsp;<b>Максимальный</b> процент исполнения:<table>", lbInfo.Text);
 ...
 <tr><td align='right'><b>{1:N2}%</b></td><td> {2}</td></tr>", lbInfo.Text, dtInfo.Rows[i][2], dtInfo.Rows[i][1]);
        ...
        {
            string query = DataProvider.GetQueryText("FK_0004_0008_grid");
            grbsGridDt = new DataTable();
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование доходного источника", grbsGridDt);
            ...
            if (grbsGridDt.Rows.Count > 0)
            {
                ...
                RankIndicatorRule rankRule = new RankIndicatorRule(5, 8);
                ...
                levelRule.AddFontLevel("2", GRBSGridBrick.BoldFont8pt);
                levelRule.AddFontLevel("3", GRBSGridBrick.Font8pt);
        ...
        void Grid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            if (e.Row.Cells[4].Value != null &&
                double.TryParse(e.Row.Cells[4].Value.ToString(), out value))
            {
                e.Row.Cells[4].Value = String.Format("<table style='width: 100%;'><tr><td style='border: none; width: 100%; align: right;'>{0:N2}% </td><td style='border: none'>{1}</td></tr></table>", value, GetGaugeUrl(value, String.Empty));
            }
        }
        ...
        protected void GRBSGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            e.Layout.Bands[0].Columns[2].Width = CRHelper.GetColumnWidth(120);
            e.Layout.Bands[0].Columns[3].Width = CRHelper.GetColumnWidth(120);
            e.Layout.Bands[0].Columns[4].Width = CRHelper.GetColumnWidth(200);
            ...
            headerLayout.AddCell(String.Format("Прирост поступлений за месяц, {0}", multiplierCaption));
        ...
        private void GridDataBind1()
        {
            string query = DataProvider.GetQueryText("FK_0004_0008_grid1");
            grbsGridDt = new DataTable();
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Главный администратор доходов федерального бюджета", grbsGridDt);
            ...
            if (grbsGridDt.Rows.Count > 0)
            {
                ...
                RankIndicatorRule rankRule = new RankIndicatorRule(5, 9);
                UltraGridBrick1.AddIndicatorRule(rankRule);

                UltraGridBrick1.DataTable = grbsGridDt;
            }
        }

        void Grid_InitializeRow1(object sender, RowEventArgs e)
        {
            double value;
        ...
        protected void GRBSGrid_InitializeLayout1(object sender, LayoutEventArgs e)
        {
            if (e.Layout.Bands[0].Columns.Count == 0)
            {
                return;

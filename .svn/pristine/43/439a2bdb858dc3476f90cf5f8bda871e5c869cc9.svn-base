...
{
    public partial class RG_0001_0003_Horisontal : CustomReportPage
    {
        // поля 
        ...
        private void BindInfoText()
        {
            ...
            {
                lbInfo.Text = string.Format(@"&nbsp;&nbsp; По состоянию на <b> {0} {1} {2} </b> общая численность детей в возрасте от 1 до 7 лет составляет <b> {3:N2} тыс. человек</b>, численность детей данного возраста, охваченных услугами дошкольного образования, составляет <b>{4:N2} тыс.человек</b> (&nbsp;<img align = 'center' src='../../../TemporaryImages/Chart_RG_0001_0003_{5}.png'> {5:N0}% от общей численности детей от 1 до 7 лет).", day, CRHelper.RusMonthGenitive(CRHelper.MonthNum(month)), year, dtInfo.Rows[0][1], dtInfo.Rows[0][2], dtInfo.Rows[0][3]);
            }
            ...
            if (dtInfo.Rows.Count > 0)
            {
                ...
                lbInfo.Text = String.Format(@"{0}  <br />&nbsp;&nbsp; Очередность в дошкольные образовательные   учреждения  в целом по ЦФО составляет <b>  {1:N0}  человек </b> (<img align = 'center' src='../../../TemporaryImages/Chart_RG_0001_0003_{3}.png'> {2:N0}% от общей численности детей в возрасте от 1 до 7 лет).", lbInfo.Text, dtInfo.Rows[0][1], dtInfo.Rows[0][2], value.ToString().Replace(',', '_').Replace('.', '_'));
            }
        ...
        private void Grid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            if (e.Row.Cells[6].Value != null)
            {
                ...
                e.Row.Cells[6].Style.CustomRules = "background-repeat: no-repeat; background-position: center center; padding-left: 2px";

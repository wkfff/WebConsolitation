...
namespace Krista.FM.Server.Dashboards.reports.FO_0039_0009
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                endYear = Convert.ToInt32(dtDate.Rows[0][0]) + 2;
            ...
            Group3Label.Text = "Группа 1 <img src=\"../../images/ballGreenBB.png\"> доля от 10% до 30%";

            Page.Title = String.Format("Расчет группы, присвоенной муниципальным образованиям Омской области по ст.136 БКРФ");
            ...
            {
                moGroupFilter.Value = string.Format("not IsEmpty([Показатели].[Оценка качества ОиОБП_Исходные данные].[Группа МР(ГО)]) and ([Показатели].[Оценка качества ОиОБП_Исходные данные].[Группа МР(ГО)]) = {0}", ComboMOGroup.SelectedIndex + 1);
            }
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            {
                string formatString;
                int columnWidth = (i == beginIndex) ? 80 : 100;
            ...
            CRHelper.SetHeaderCaption(e.Layout.Grid, 0, beginIndex, WithSettlements ? "Группа СП(ГП)" : "Группа МР(ГО)", "");
            ...
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                ...
                if (groupColumn && e.Row.Cells[i].Value != null && e.Row.Cells[i].Value.ToString() != String.Empty)
                {
                    ...
                    else
                    {
                        ...
                        e.Row.Cells[i].Title = String.Format("Доля межбюджетных трансфертов из других бюджетов бюджетной системы РФ не превышает {0} процентов объема собственных доходов местных бюджетов",
                                hintPercent);
                    }
                }

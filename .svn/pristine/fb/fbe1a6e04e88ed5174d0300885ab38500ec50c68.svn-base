...
namespace Krista.FM.Client.ViewObjects.AssociatedCLSUI.AssociatedCls
{
	public partial class AssociateMaster : Form
	{
		...
		private int masterSourceID = -1;

		private int NextPageIndex = 0;

		IWorkplace masterWorkplace = null;

        public AssociateMaster(IBridgeAssociation CurentAssociation, int CurrentSourceID, IWorkplace currentWorkplace)
		{
			InitializeComponent();
			masterAssociation = CurentAssociation;
			masterSourceID = CurrentSourceID;
        ...
        /// <returns></returns>
        public static bool Associate(IBridgeAssociation CurentAssociation, int CurrentSourceID, IWorkplace currentWorkplace)
        {
            AssociateMaster master = new AssociateMaster(CurentAssociation, CurrentSourceID, currentWorkplace);
		...
		private void btnAssociate_Click(object sender, EventArgs e)
		{
            if (rbAllClassifier.Checked)
                ...
                label1.Text = string.Format("¬сего записей: {0}", masterAssociation.GetRecordsCountByCurrentDataSource(masterSourceID));
			...
			try
			{
				...
				if (rbAllClassifier.Checked || masterSourceID < 0)
					masterSourceID = -1;
				...
				if (masterRule == null)
					associateResult = masterAssociation.Associate(masterSourceID);
				else
                    associateResult = masterAssociation.Associate(masterSourceID, ruleKey, ruleSetting, ruleParams);
		...
		{
            int allRecordsCount = masterAssociation.GetRecordsCountByCurrentDataSource(masterSourceID);// CommonMethods.GetDataCLSRecordCount(masterSourceID, true, masterAssociation, masterWorkplace);
            int unAssociateRecordsCount = masterAssociation.GetUnassociateRecordsCountByCurrentDataSource(masterSourceID);//CommonMethods.GetDataCLSRecordCount(masterSourceID, false, masterAssociation, masterWorkplace);

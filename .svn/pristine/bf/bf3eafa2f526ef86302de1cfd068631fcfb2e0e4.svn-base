...
{
    public partial class FNS_0001_0001_V : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            WeightGrid.Caption = "Район(ГО)";
            WeightGrid.InitializeRow += new InitializeRowEventHandler(regionsGrid_InitializeRow);
        }
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            UserParams.PeriodYear.Value = string.Format("[{0}].[{1}].[{2}].[{3}]", year, UserParams.PeriodHalfYear.Value, 
            ...
            regionsLabel.Text = "<span style=\"color:white;\"><b>5</b></span>&nbsp;территорий с наибольшей недоимкой* (Консолидированный бюджет МО)";
            regionsGrid.DataBind();
            regionsGrid.Height = Unit.Empty;

            OKVDLabel.Text = "<br /><span style=\"color:white;\"><b>5</b></span>&nbsp;ОКВЭД с наибольшей недоимкой* (Консолидированный бюджет субъекта)";
            OKVDGrid.DataBind();
            OKVDGrid.Height = Unit.Empty;

            KDLabel.Text = "<br /><span style=\"color:white;\"><b>5</b></span>&nbsp;доходных источников с наибольшей недоимкой* (Консолидированный бюджет субъекта)";
            KDGrid.DataBind();
            KDGrid.Height = Unit.Empty;

            lbWeigth.Text = "<br /><span style=\"color:white;\"><b>5</b></span>&nbsp;территорий с наибольшим удельным весом недоимки* в поступлениях (Консолидированный бюджет МО)";
            WeightGrid.DataBind();
            WeightGrid.Height = Unit.Empty;

            CRHelper.SetConditionImageGridCells(regionsGrid, 2, 0, CompareAction.Greater, "~/images/ArrowUpRed.gif", "~/images/ArrowDownGreen.gif");
            CRHelper.SetConditionImageGridCells(OKVDGrid, 2, 0, CompareAction.Greater, "~/images/ArrowUpRed.gif", "~/images/ArrowDownGreen.gif");
            CRHelper.SetConditionImageGridCells(KDGrid, 2, 0, CompareAction.Greater, "~/images/ArrowUpRed.gif", "~/images/ArrowDownGreen.gif");

            commentLabel.Text = "<*> - недоимка с учетом неурегулированной задолженности по всем категориям налогоплательщиков";
        }
        ...
        protected void OKVDGrid_DataBinding(object sender, EventArgs e)
        {
            ...
            string queryName = (fns28nSplitting) ? "FNS_0001_0001_top5_V_split" : "FNS_0001_0001_top5_V";
        ...
        protected void KDGrid_DataBinding(object sender, EventArgs e)
        {
            ...
            string budgetLevel = (fns28nSplitting)
                     ? ", [Уровни бюджетов].[Все].[Все уровни].[Конс.бюджет субъекта]"
                     : " ";
            UserParams.Filter.Value = String.Format("[Районы].[Сопоставимый].[Все районы] {0}", budgetLevel);
            string queryName = (fns28nSplitting) ? "FNS_0001_0001_top5_V_split" : "FNS_0001_0001_top5_V";
            string query = DataProvider.GetQueryText(queryName);
            dtKD = new DataTable();
        ...
        protected void WeightGrid_DataBinding(object sender, EventArgs e)
        {
            string query = DataProvider.GetQueryText("FNS_0001_0001_top5_V_weight");
            dtRegions = new DataTable();
            dtRegions.TableName = "Район (ГО)";
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, dtRegions.TableName, dtRegions);
            WeightGrid.DataSource = dtRegions;
        }
        ...
        private DataTable ConvertDT(DataTable dtSource, string title)
        {
            ...
            string prevMonth = string.Format("Недоимка* на 01.{0:00}.{1} и прирост", month, year);
            ...
            string cellFormat = IsThsRubMiltiplier ? "N0" : "N3";
            ...
            foreach (DataRow row in dtSource.Rows)
            {
                ...
                r[1] = string.Format("{0}<br /><span style=\"color:#d1d1d1;\">{1}</span>",
                    ParseDoubleDTValue(row, 1, cellFormat),
                    ParseDoubleDTValue(row, 2, "P2"));
                dt.Rows.Add(r);
            }
        ...
        protected void Grid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            if (e.Layout.Bands[0].Columns.Count == 2)
            {
                ...
                e.Layout.Bands[0].Columns[0].Width = 198;
                e.Layout.Bands[0].Columns[1].Width = 110;
            }
        }

        protected void WeightGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            e.Layout.GroupByBox.Hidden = true;
            ...
            if (e.Layout.Bands[0].Columns.Count == 2)
            {
                ...
                e.Layout.Bands[0].Columns[1].Width = 118;
        ...
        void regionsGrid_InitializeRow(object sender, RowEventArgs e)
        {
            ...
            {
                if (dt.Rows[e.Row.Index][2] != DBNull.Value &&
                    dt.Rows[e.Row.Index][2].ToString() != string.Empty)
                {
                    double value = Convert.ToDouble(dt.Rows[e.Row.Index][2]);
                    e.Row.Cells[1].Style.BackgroundImage = (value > 0)
                        ? "../../../images/arrowUpRed.png"
                        : "../../../images/arrowDownGreen.png";
                    e.Row.Cells[1].Style.CustomRules = "background-repeat: no-repeat; background-position: left center; padding-top: 2px";
                }
            }
        }

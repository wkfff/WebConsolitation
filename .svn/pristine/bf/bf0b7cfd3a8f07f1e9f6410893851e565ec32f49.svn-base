Ext.ux.ExcelGridView = Ext.extend(Ext.grid.GridView, {
        ...
        for (j = 0; j < len; j++) {
            ...
            if (templateData) {
                for (i = 0; i < colCount; i++) {
                    ...
                    if (cell) {
                        if (cell.id != "") {
                            var cellConfig = this.grid.getCellConfig(rowMetaId, cell.id);
                            var val = record.get(cell.id);
                            if (val != null) {
                                cell.value = this.formatCellValue(val, cellConfig);
                            }
                        }
                    }
                }
    ...
    refreshRowData: function (record, row) {
        ...
        for (var c = 0; c < row.children.length; c++) {
            ...
            if (match && match[1]) {
                ...
                if (innerCellEl) {
                    var metaRowId = record.get('RefFormRow');
                    var colName = match[1];
                    var cellConfig = this.grid.getCellConfig(metaRowId, colName);
                    innerCellEl.innerHTML = this.formatCellValue(cellValue, cellConfig);
                }
            }
        }
    ...
    */
    ...
    createItem: function (cell) {
        ...
        cell.editor = null;
        if (cell.visible) {
            cell.text = cell.text !== null ? this.trim(cell.text) : '';
        }
        ...
        if (cell.visible) {
            ...
            return this.createCell(cellItem, cell);
        }

        return cellItem;
    },

    createCell: function (cellItem, cell) {
    ...
    formatCellValue: function (val, cellConfig) {
        if (cellConfig.DataType == 'System.Int32') {
            return Ext.util.Format.number(val, '0.000,/i');
        } else if (cellConfig.DataType == 'System.Decimal') {

        ...
        protected void StartAnalyze(Analyzer analyzer)
        {
            if ((!this.IsCacheTrivial() && !analyzer.InCommonSubExpr) && ((base.GetType() == typeof(MDXUnaryOpNode)) || (this.GetMDXType() != MDXDataType.Set)))
            {
                ...
                if (analyzer.SeenExprs.TryGetValue(this.GetNormalizedMDX(), out node))
                {
                    ...
                    m.Id = 10;
                    m.Text = string.Format("Same expression was used before at Line {0} Column {1}. Consider eliminating common subexpressions for better performance and to take advantage of cache", node.Locator.Line, node.Locator.Column);
                    m.Severity = 2;
                    analyzer.Add(m);

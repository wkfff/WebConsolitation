...
namespace Krista.FM.Server.Dashboards.reports.FO_0002_0007_HMAO
{
    public partial class DefaultCompare : CustomReportPage
    {
        ...
        private bool IsFkrTotalSelected
        {
            get
            {
                return ComboFKR.SelectedValue.ToLower().Contains("расходы бюджета - итого") ||
                       ComboFKR.SelectedValue.Contains("Расходы бюджета – Всего"); 
            }
        }
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            ComboFKR.SetСheckedState(fkrGroupComboValue.Value, true);
            
            Page.Title = string.Format("Исполнение расходов");
            Label1.Text = Page.Title;
            Label2.Text = string.Format("({3}) за {0}&nbsp;{1}&nbsp;{2}&nbsp;года", monthNum, CRHelper.RusManyMonthGenitive(monthNum), yearNum, fkrGroupComboValue.Value.TrimEnd(' '));
            MoPercentLabel.Text = IsFkrTotalSelected 
                ? String.Empty
                : "* - выводится доля расходов бюджета в консолидированном бюджете субъекта РФ по выбранному в параметре РзПр";
...
//            DundasMap.ShapeFields["CompletePercent"].UniqueIdentifier = false;
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            if (e.Layout.Bands[0].Columns.Count != 0)
            {
                ...
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 1, "Уточненные годовые назначения, тыс.руб", "План на год");
                ...
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 4, "Численность постоянного населения, тыс.чел.", "Численность постоянного населения");
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 5, "Бюджетные расходы на душу населения,\n руб./чел.", "Бюджетные расходы на душу населения");
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 6, "Ранг бюдж. расх. МО", "Ранг (место) бюджета по бюджетным расходам на душу населения среди бюдежетов МО");
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 8, "Доля *", "Доля расхода в общей сумме расходов бюджета");
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 9, "Ранг доля МО", "Ранг (место) бюджета по доле расхода в общей сумме расходов среди бюджетов МО");
            }
        }

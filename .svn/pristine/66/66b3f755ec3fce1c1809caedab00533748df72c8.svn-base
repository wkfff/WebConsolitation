        ...
        public static bool GetFileName(string startFileName, string fileExtension, bool toSave, ref string finishFileName)
        {
            ...
            switch (fileExtension)
            {
                case "xls":
                    if (toSave)
                    {
                        dlg.Filter = OfficeHelper.GetExcelVersionNumber() < 12 ? "Excel документы *.xls|*.xls" :
                            "Excel документы *.xlsx|*.xlsx";
                    }
                    else
                    {
                        dlg.Filter = OfficeHelper.GetExcelVersionNumber() < 12 ? "Excel документы *.xls|*.xls" :
                            "Excel документы *.xlsx|*.xlsx|Excel документы 97 - 2003 *.xls|*.xls";
                    }
                    break;
                case "xml":
            ...
            if (dlg.ShowDialog() == DialogResult.OK)
            {
                ...
                var extension = Path.GetExtension(finishFileName);
                switch (fileExtension)
                {
                    case "xls":
                        if (toSave && (string.Compare(extension, ".xls", true) != 0 && string.Compare(extension, ".xlsx", true) != 0))
                            finishFileName += OfficeHelper.GetExcelVersionNumber() < 12 ? ".xls" : ".xlsx";
                        break;
                    case "xml":
                        if (toSave && string.Compare(extension, ".xml", true) != 0)
                            finishFileName += ".xml";
                        break;
                    case "txt":
                        if (toSave && string.Compare(extension, ".txt", true) != 0)
                            finishFileName += ".txt";
                        break;
                    default: dlg.Filter = string.Format("Произвольные документы *.{0}|*.{0}", fileExtension);
                        break;

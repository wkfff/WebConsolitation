...
{
    public partial class FO_0002_0007_Omsk : CustomReportPage
    {
        private DataTable dt;
        private DataTable newDt;

        private double avgOutcomesTotal;
        private double populationTotal;

        private double avgOutcomesMinValue;
        private double avgOutcomesMaxValue;
        private string avgOutcomesBestItem;
        private string avgOutcomesWorseItem;
        ...
        protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0002_0007_Omsk");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, " ", dt);

            if (dt.Columns.Count > 0 && dt.Rows.Count > 0)
            {
                AVGOutcomesCalculate();

                dt.Rows[0]["Численность постоянного населения"] = populationTotal;
                dt.Rows[0]["Коэффициент бюджетной обеспеченности населения"] = 1000 * avgOutcomesTotal;
                dt.Rows[0]["Лучший по коэфф.бюдж.обесп. МО"] = avgOutcomesBestItem;
                if (avgOutcomesMaxValue != double.MinValue)
                {
                    dt.Rows[0]["Лучшие коэфф.бюдж.обесп. МО"] = avgOutcomesMaxValue;
                }
                dt.Rows[0]["Худший по коэфф.бюдж.обесп. МО"] = avgOutcomesWorseItem;
                if (avgOutcomesMinValue != double.MaxValue)
                {
                    dt.Rows[0]["Худшие коэфф.бюдж.обесп. МО"] = avgOutcomesMinValue;
                }
            ...
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 8:
                        {
                            k = 0.001;
                            caption = "<div style=\"line-height:15px;\">План.расходы на душу нас.,<br />тыс.руб./чел.</div>";
                            break;
                        }
                    case 13:
                        {
        ...
        {
            return rowName != "Консолидир. бюджет" &&
                rowName != "г.Омск"; ;
        }

        private void AVGOutcomesCalculate()
        {
            string query = DataProvider.GetQueryText("F0_0002_0005_iphone_outcomes");
            DataTable OutcomesDT = new DataTable();
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, " ", OutcomesDT);

            if (OutcomesDT.Rows.Count > 0)
            {
                ...
                if (OutcomesDT.Columns.Count > 1)
                {
                    OutcomesDT.PrimaryKey = new DataColumn[] { OutcomesDT.Columns[0] };
                    ...
                    foreach (DataRow populationRow in populationDT.Rows)
                    {
                        if (populationRow[0] != DBNull.Value)
                        {
                                ...
                                populationRow[populationColumnName].ToString() != String.Empty)
                            {
                                ...
                                DataRow OutcomesRow = OutcomesDT.Rows.Find(rowName);
                                if (OutcomesRow != null)
                                {
                                    if (population != 0 && OutcomesRow[executeColumnName] != DBNull.Value &&
                                        OutcomesRow[executeColumnName].ToString() != String.Empty)
                                    {
                                        double execute = Convert.ToDouble(OutcomesRow[executeColumnName]);
                    ...
                    populationTotal = Convert.ToDouble(populationDT.Rows[0][populationColumnName]);
                    avgOutcomesTotal = Convert.ToDouble(OutcomesDT.Rows[0][executeColumnName]) / (1000 * populationTotal);

                    avgOutcomesMinValue = avgExecuteRank.GetMinValue();
                    avgOutcomesMaxValue = avgExecuteRank.GetMaxValue();
                    avgOutcomesBestItem = avgExecuteRank.GetBestItem();
                    avgOutcomesWorseItem = avgExecuteRank.GetWorseItem();
                }
            }
        }
        #endregion

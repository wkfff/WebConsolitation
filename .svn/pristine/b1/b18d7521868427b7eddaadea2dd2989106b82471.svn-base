...
{
    public partial class FO_0135_0005 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!dtDate.Rows[0][4].ToString().Contains("Заключительные обороты"))
            {
                ...
                else
                {
                    ...
                    lbDate.Text = "&nbsp;на&nbsp;<span style='color: white;'>" + date.ToString("dd.MM.yyyy") + "</span>&nbsp;по бюджету автономного округа, тыс.руб.";
                }
            }
            else
            {
        ...
        private void BindTagCloud()
        {
            ...
            int six = dtExecuteOutcome.Columns.Count - 4;
            int seven = dtExecuteOutcome.Columns.Count - 3;
            int eight = dtExecuteOutcome.Columns.Count - 2;
            int nine = dtExecuteOutcome.Columns.Count - 1;

            double ethalon = Convert.ToDouble(dtExecuteOutcome.Rows[dtExecuteOutcome.Rows.Count - 1][2]);
            dtExecuteOutcome.Rows.RemoveAt(dtExecuteOutcome.Rows.Count - 1);

            foreach (DataRow row in dtExecuteOutcome.Rows)
            {
                Tag tag = new Tag();
                tag.weight = Convert.ToInt32(row[six]);
                tag.key = String.Format("{0} ({1:P0})", row[eight], row[seven]);
                tag.toolTip = String.Format("<span style=\"font-family: Arial; font-size: 14pt\">{0}<br/>план {1:N0} тыс. руб.<br/>факт {2:N0} тыс. руб.<br/>исполнено {3:P0}</span>", 
                        iPadBricks.iPadBricks.iPadBricksHelper.GetWarpedHint(row[0].ToString()), row[six], row[nine], row[seven]);

                if (row[six] != DBNull.Value &&
                    row[six].ToString() != String.Empty)
                {
                    tags.Add(tag.key, tag);
                }

                double value = Convert.ToDouble(row[2]);
                bool crime = value < ethalon;
        ...
        private void MakeHtmlTableIncomes()
        {
            ...
            for (int i = 0; i < dtData.Rows.Count; i++)
            {
                ...
                TableCell cell;
                if (dtData.Rows[i][0].ToString() == "Доходы – всего")
                {

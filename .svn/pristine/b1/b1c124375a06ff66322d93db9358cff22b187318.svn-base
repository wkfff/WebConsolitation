...
namespace Krista.FM.Server.Dashboards.reports.FO_0021_0004_Saratov
{
    ...
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            CrossLink4.NavigateUrl = "~/reports/FO_0021_0005_Saratov/Default.aspx";
        }
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string query = DataProvider.GetQueryText("FO_0021_0004_Saratov_date");
                DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForPivotTable(query, dtDate);
                endYear = Convert.ToInt32(dtDate.Rows[0][0]);
                string quarter = dtDate.Rows[0][2].ToString();

                selectedIndicatorCaption.Value = "Ранг";
            ...
            indicatorDigest = new MemberAttributesDigest(DataProvidersFactory.PrimaryMASDataProvider, "FO_0021_0004_Saratov_indicatorList");
            ...
            sortingType.Value = selectedIndicatorName == "Ранг" ? "BASC" : "BDESC";
        ...
        public void SetMapSettings()
        {
            ...
            switch (selectedIndicatorCode)
            {
                case "Ранг":
                    {
                        coloringRule = ColoringRule.TopTreeColoringASC;
                ...
                case "Итоговая оценка":
                    {
                        coloringRule = ColoringRule.TopTreeColoringDESC;
            ...
            if ((coloringRule == ColoringRule.TopTreeColoringASC) || (coloringRule == ColoringRule.TopTreeColoringDESC))
            {
                LegendItem item = new LegendItem();
                item.Text = String.Format("МО с наивысшей оценкой");
                item.Color = beginMapColor;
                DundasMap.Legends["MapLegend"].Items.Add(item);

                item = new LegendItem();
                item.Text = String.Format("МО со средней оценкой");
                item.Color = middleMapColor;
                DundasMap.Legends["MapLegend"].Items.Add(item);

                item = new LegendItem();
                item.Text = String.Format("МО с наименьшей оценкой");
        ...
        {
            string queryQuality = DataProvider.GetQueryText("FO_0021_0004_Saratov_quality_limit");
            ...
            string query = DataProvider.GetQueryText("FO_0021_0004_Saratov_map");
            dtMap = new DataTable();
                ...
                foreach (Shape shape in shapeList)
                {
                    ...
                    if (row[1] != DBNull.Value && row[1].ToString() != String.Empty)
                    {
                        ...
                        switch (coloringRule)
                        {
                            ...
                            case ColoringRule.TopTreeColoringDESC:
                                {
                                    if (value >= beginColorLimit)
                                    {
                                        shape.Color = beginMapColor;
                                    }
                                    else if (value <= endColorLimit)
                                    {
                                        shape.Color = endMapColor;
                                    }
                                    else
                            ...
                            case ColoringRule.TopTreeColoringASC:
                                {
                                    if (value <= beginColorLimit)
                                    {
                                        shape.Color = beginMapColor;

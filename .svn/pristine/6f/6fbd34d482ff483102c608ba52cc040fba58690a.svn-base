...
{
    public sealed class DocumentsNewView : View
    {
        ...
        public override List<Component> Build(ViewPage page)
        {
            ...
            Page.Controls.Add(
                UpLoadGmuBtnControl.CreateUploadGmuWindow(
                    string.Concat(@"#{wndUploadGMU}.hide();" + UpLoadGmuBtnControl.SucHndler, @"window.dsDocumentsStore.reload();"), 
                    string.Concat(@"#{wndUploadGMU}.hide();", UpLoadGmuBtnControl.FailHndler), 
                    UiBuilders.GetUrl<UpLoadGmuController>("BatchExport"),
                    "window.wndUploadGMU.docs"));

            var store = StoreExtensions.StoreCreateDefault(string.Concat("ds", GridID), true, typeof(DocumentsController), viewModel, Page);
            ...
            var panel = UiBuilders.CreateGridPanel(GridID, store);
            panel.TrackMouseOver = false;
            panel.View[0].GetRowClass.Fn = string.Concat(Scope, ".getRowClass");
            
            panel.BottomBar.Add(new PagingToolbar
                                     {
                                         ID = "paggingDocs",
                                         StoreID = store.ID,
                                         PageSize = 200,
            ...
            panel.ColumnModel.AddColumn(() => viewModel.Check, DataAttributeTypes.dtBoolean).SetEditableBoolean().SetWidth(20);
            panel.ColumnModel.AddColumn(() => viewModel.StructureName, DataAttributeTypes.dtString).SetWidth(150);
            panel.ColumnModel.AddColumn(() => viewModel.StructureGrbs, DataAttributeTypes.dtString).SetWidth(80);
            panel.ColumnModel.AddColumn(() => viewModel.StructurePpo, DataAttributeTypes.dtString).SetWidth(80);
            panel.ColumnModel.AddColumn(() => viewModel.State, DataAttributeTypes.dtString).SetWidth(50);
            panel.ColumnModel.AddColumn(() => viewModel.Type, DataAttributeTypes.dtString).SetWidth(50);
            panel.ColumnModel.AddColumn(() => viewModel.Note, DataAttributeTypes.dtString).SetHidden(true);
            panel.ColumnModel.AddColumn(() => viewModel.Year, DataAttributeTypes.dtInteger).SetWidth(40).SetHidden(true);
            panel.ColumnModel.AddColumn(() => viewModel.StructureCloseDate, DataAttributeTypes.dtDateTime).SetWidth(50).SetHidden(true);

            panel.Plugins.Add(new GridFilters
             {
                 Local = false,
                 Filters =
                            {
                                new NumericFilter { DataIndex = viewModel.NameOf(() => viewModel.ID) },
                                new BooleanFilter { DataIndex = viewModel.NameOf(() => viewModel.Check) },
        ...
        private Panel CreateTopPanel()
        {
            ...
            var batchExportBtn = new Button
            {
                ID = "batchExport",
                Icon = Icon.WorldGo,
                ToolTip = @"Пакетный экспорт"
            };

            batchExportBtn.SetHandler("E86n.View.Documents.Grid.BatchExport", Scope);

            var selectAllBtn = new Button
            {
                ID = "selectAllBtn",
                Icon = Icon.Tick,
                ToolTip = @"Выбрать все документы"
            };

            selectAllBtn.SetHandler("E86n.View.Documents.Grid.SelectAll", Scope);
            ...
            deselectAllBtn.SetHandler("E86n.View.Documents.Grid.DeselectAll", Scope);

...
namespace Krista.FM.Server.Dashboards.reports.FO_0002_0001_Omsk
{
    public partial class Default : CustomReportPage
    {
        ...
        private static MemberAttributesDigest climaticZoneDigest;
...
//        private bool ekrGridWarning;
//        private bool fkrGridWarning;
//        private bool incomesGridWarning;
//        private bool deficitGridWarning;
//        private bool debtsGridWarning;
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string queryName =
                    string.Format("FO_0002_0001_Omsk_date_{0}",
                ...
                climaticZoneDigest = new MemberAttributesDigest(DataProvidersFactory.PrimaryMASDataProvider, "FO_0002_0001_Omsk_climaticZoneList");
                ...
                default:
                    {
                        ...
                        {
                            UserParams.Region.Value = budgetDigest.GetMemberUniqueName(ComboBudget.SelectedValue);
                            string selectedRegionType = budgetDigest.GetMemberType(ComboBudget.SelectedValue);
                            climaticZoneSelected = selectedRegionType.Contains("ПКЗ");
                            if (selectedRegionType.Contains("МР") || climaticZoneSelected)
                            {
                                regionSelected = true;
            ...
            if (selectMap)
            {
                if (climaticZoneSelected)
                {
                    foreach (string name in climaticZoneDigest.MemberTypes.Keys)
                    {
                        if (climaticZoneDigest.GetMemberType(name) ==  UserParams.SelectedMap.Value.TrimEnd(' '))
                        {
                            CRHelper.SelectMapShape(DundasMap, name, false, String.Empty);
                        }
                    }
                }
                else
...
//                 Server.MapPath(
//                     string.Format("../../maps/Субъекты/{0}/Соседние.shp", mapFolderName)),
...
//                    string waterObjects = Server.MapPath(string.Format("../../maps/Субъекты/{0}/Водные.shp", mapFolderName));
//                    // водных объектов может не быть
//                    if (File.Exists(waterObjects))
//                    {
//                        CRHelper.AddMapLayer(DundasMap, waterObjects, false, CRHelper.MapShapeType.WaterObjects);
//                    }
//                    CRHelper.AddMapLayer(DundasMap,
...
//                                         true, CRHelper.MapShapeType.Towns);
            ...
            foreach (Shape shape in DundasMap.Shapes)
            {
                ...
                if ((shape.Layer == "Города" || shape.Layer == "Территор") && !budgetDigest.UniqueNames.ContainsKey(shape.Name))
                {
                    shape.Visible = false;
                }
            }
        ...
        protected void map_Click(object sender, Dundas.Maps.WebControl.ClickEventArgs e)
        {
            ...
            if (result.Object is Shape)
            {
                ...
                if (shape.Layer != "Территор" && shape.Layer != "Города" || !budgetDigest.UniqueNames.ContainsKey(shape.Name))
                {
                    return;
                }
                ...
                UserParams.Region.Value = budgetDigest.GetMemberUniqueName(shape.Name);
            }
        }
...
//        private static void FillExecutePercent(DataTable dt, int startColumnIndex)
//        {
//            foreach (DataRow row in dt.Rows)
//            {
//                if (row[startColumnIndex] != DBNull.Value && row[startColumnIndex].ToString() != string.Empty &&
//                    row[startColumnIndex + 1] != DBNull.Value && row[startColumnIndex + 1].ToString() != string.Empty)
//                {
//                    double plan = Convert.ToDouble(row[startColumnIndex].ToString());
//                    double fact = Convert.ToDouble(row[startColumnIndex + 1].ToString());
//                    if (plan != 0)
//                    {
//                        row[startColumnIndex + 2] = fact / plan;
//                    }
//                    else
//                    {
//                        row[startColumnIndex + 2] = DBNull.Value;
//                    }
//                }
//            }
//
//        }
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Omsk_ekrOutcomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, EKRGridCaption, dt_ekrOutcomes);

//            ekrGridWarning = IsWarningMessage(dt_ekrOutcomes);

            //FillExecutePercent(dt_ekrOutcomes, 2);
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Omsk_fkrOutcomes");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, FKRGridCaption, dt_fkrOutcomes);

            //FillExecutePercent(dt_fkrOutcomes, 1);
        ...
        {
            string query = DataProvider.GetQueryText("FO_0002_0001_Omsk_fkrOutcomes_total");
...
//                        if (ekrGridWarning)
//                        {
//                            e.Layout.Grid.Columns[0].Header.Caption += "*";
//                            e.Layout.Grid.Columns[0].Header.Title = "В базе не содержится консолидированного отчета об исполнении бюджета субъекта РФ. \n"+
//                               "В отчет выводятся данные, полученные суммированием данных об исполнении бюджетов: \n"+
//                               " -собственного бюджета субъекта РФ, \n"+
//                               " -бюджетов городских округов, \n" +
//                               " -бюджетов муниципальных районов, \n" +
//                               " -бюджетов поселений. \n" +
//                               "Полученные при этом цифры могут расходиться с данными консолидированного отчета на величину внутренних оборотов.";
//                        }
            ...
            if (gridCaption == FKRGridCaption || gridCaption == EKRGridCaption)
            {
                ...
                CRHelper.FormatNumberColumn(e.Layout.Grid.Columns[j], gridCaption == EKRGridCaption ? "00 0" : "00 00");

                i++;
            }

            e.Layout.Grid.Columns[i + 1].Width = CRHelper.GetColumnWidth(105);
            e.Layout.Grid.Columns[i + 1].Header.Caption = String.Format("Годовые назначения, {0}", RubMiltiplierButtonList.SelectedValue);
            e.Layout.Grid.Columns[i + 1].Header.Title = "План на год";
...
//        private static bool IsWarningMessage(DataTable dt)
//        {
//            if (dt.Rows.Count < 1 || dt.Columns.Count == 0)
//            {
//                return false;
//            }
//
//            int columnIndex = dt.Columns.Count - 1;
//            if (dt.Rows[1][columnIndex] != DBNull.Value && dt.Rows[1][columnIndex].ToString() != String.Empty)
//            {
//                bool warning = Convert.ToBoolean(dt.Rows[1][columnIndex].ToString());
            ...
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                    ...
                    gridCaption == EKRGridCaption || gridCaption == outcomesTotalGridCaption)
                {
                    ...
                    if (gridCaption == EKRGridCaption && e.Row.Cells[1] != null)
                    {
                        ...
                        e.Row.Cells[i].Style.Font.Italic = levelName == "Подстатья";
                    }
                }

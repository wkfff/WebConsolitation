...
namespace Krista.FM.Server.Dashboards.reports.FO_0002_0016
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            Page.Title = "Мониторинг показателей эффективности деятельности дорожного комплекса (годовой)";
            Label1.Text = Page.Title;
            int yearNum = Convert.ToInt32(ComboYear.SelectedValue);
            date = new DateTime(yearNum, 01, 1).AddMonths(1);

            Label2.Text = String.Format("Данные ежегодного мониторинга основных показателей деятельности дорожного комплекса Новосибирской области за {0} год",
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
                    ...
                    case 1:
                        {
                            ...
                            e.Layout.Bands[0].Columns[i].Width = CRHelper.GetColumnWidth(112);
                            break;
                        }
                    case 2:
                    case 3:
            ...
            headerLayout.AddCell(string.Format("Единицы измерения"),
                string.Format("Единицы измерения"));
            headerLayout.AddCell(string.Format("{0} год",
                date.Year - 1),
                string.Format("{0} год", date.Year - 1));
            headerLayout.AddCell(string.Format("{0} год",
                date.Year),
                string.Format("{0} год", date.Year));
            headerLayout.AddCell(string.Format("Коэффициент роста, %"),
                string.Format("Коэффициент роста, %"));
        ...
        {          
               /* UltraGridRow row = CRHelper.FindGridRow(UltraWebGrid, currentRegion.Value, 1, 0);
                UltraWebGrid.Rows[0].Cells[1].Style.HorizontalAlign = HorizontalAlign.Right;
                ActivateGridRow(row); */
        }
        ...
        private void ExcelExportButton_Click(object sender, EventArgs e)
        {
            ...
            sheet1.Rows[4].Cells[1].CellFormat.Alignment = HorizontalCellAlignment.Right;
       }

...
namespace Krista.FM.Client.OLAPAdmin
{
	...
	{		
			...
			EventHandler _valueChangedUserHandler, DrawItemEventHandler drawItemHandler)
		{
			...
			{
				listBoxItems.DrawItem += drawItemHandler;
			}
			else
			{
		...
		private Brush GetDefaultTextColor(DrawItemEventArgs e)
		{
		...
		private void listBoxItems_DrawItem(object sender, DrawItemEventArgs e)
		{
			...
			{	
				...
				e.DrawBackground();
				Brush textBrush = GetDefaultTextColor(e);
				string itemName = listBox.Items[e.Index].ToString();
				if (obj is OLAPObjectHeader)
				{
					OLAPObjectHeader header = obj as OLAPObjectHeader;
					...
					{
						switch (header.VSSInfo.VSSState)
						{
							case Krista.FM.ServerLibrary.VSSFileStatus.VSSFILE_CHECKEDOUT:
								itemName = string.Format("{0}, ({1})", itemName, header.VSSInfo.VSSCheckUser);
								//e.Graphics.FillRectangle(Brushes.LightGray, e.Bounds);
								textBrush = Brushes.Red;
								break;
							case Krista.FM.ServerLibrary.VSSFileStatus.VSSFILE_CHECKEDOUT_ME:
								//e.Graphics.FillRectangle(Brushes.LightGray, e.Bounds);
				...
				e.Graphics.DrawString(itemName, e.Font, textBrush, e.Bounds, StringFormat.GenericDefault);				
        ...
        private void listBoxItems_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                if (SelectedItem is OLAPObjectHeader)
                {
                    //((OLAPObjectHeader)SelectedItem).ObjectInfo.InitializeContextMenu();
                    //((OLAPObjectHeader)SelectedItem).ObjectInfo.contextMenu.Show(listBoxItems, new Point(e.X, e.Y));
                }
            }
        }
	}

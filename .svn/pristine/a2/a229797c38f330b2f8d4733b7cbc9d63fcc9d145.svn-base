...
namespace Krista.FM.Common.MdxExpertHelpers
{
    public sealed class MdxExpertHelper
    {
        private const string MdxExpertRegPath = @"SOFTWARE\krista\FM\OLAPClient";

        private static string GetMdxExpertPath()
        {
            RegistryKey key = Registry.CurrentUser.OpenSubKey(MdxExpertRegPath, false);
        ...
        {
            RegistryKey key = Registry.CurrentUser.OpenSubKey(MdxExpertRegPath, true);
            if (key == null) return;
            try
            {

...
namespace Krista.FM.Server.Dashboards.reports.FO_0009_0001
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            CrossLink.Text = "Распределение&nbsp;ФФПП";
            CrossLink.NavigateUrl = "~/reports/FO_0009_0002/Default.aspx";
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0009_0001_nonEmptySource");
            ...
            if (!Page.IsPostBack)
            {
                ...
                ComboVariant.SetСheckedState(String.Format("Основной по МР(ГО) {0}", firstYear - 1), true);
            }
            ...
            Page.Title = "Распределение фонда финансовой поддержки муниципальных районов (городских округов)";
        ...
        {
            string query = DataProvider.GetQueryText("FO_0009_0001_grid");
            ...
            if (dtGrid.Rows.Count > 0)
            {
                ...
                for (int i = 2; i < dtGrid.Columns.Count - 1; i++)
                {
                    ...
                    {
                        string type = RegionsNamingHelper.LocalBudgetTypes[column.ColumnName];

                        switch (type)
                        {
                            case "МР":
                                {
                                    column.Caption = string.Format("Муниципальные районы;{0}", column.ColumnName);
                                    break;
                                }
                            case "ГО":
                                {
                                    column.Caption = string.Format("Городские округа;{0}", column.ColumnName);
                                    break;
                                }
                        }
                    }
                    ...
                    {
                        column.Caption = string.Format("{0};{0}", column.ColumnName);
                    }
                }
                ...
                if (captions.Length > 1)
                {
                    ...
                    else
                    {
                        if (currentCaption != headerCaption || i == e.Layout.Bands[0].Columns.Count - 2)
                        {
                            ...
                            if (headerCaption.ToLower().Contains("итого") || headerCaption.ToLower().Contains("всего") ||

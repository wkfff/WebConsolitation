...
namespace Krista.FM.Server.Dashboards.reports.FO_0037_0001_01
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            UltraWebGrid2.DisplayLayout.NoDataMessage = "Данные об исполнении бюджета по расходам предоставляются ежемесячно (выберите отчет на последнее число месяца).";
            ...
            CrossLink1.Text = "Еженедельная&nbsp;информация&nbsp;об&nbsp;исполнении&nbsp;областного&nbsp;бюджета";
            CrossLink1.NavigateUrl = "~/reports/FO_0037_0001_03/Default.aspx";
        }
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                string query = DataProvider.GetQueryText("FO_0037_0001_01_date");
                ...
                ComboPeriod.FillDictionaryValues(CustomMultiComboDataHelper.FillHotInfoNonEmptyDays(DataDictionariesHelper.HotInfoNonEmptyMonths));
            ...
            Page.Title = "Справочная информация об исполнении областного бюджета";
            PageTitle.Text = Page.Title;
            PageSubTitle.Text = dateTimeStr + " в динамике к предыдущему году";
            CommentText.Text = "Информация предоставляется согласно поручению губернатора Костромской области от 24.02.2008г. № ИС-П-3.";
            CommentText.Font.Italic = true;

            dataItemCaption1.Text = "(тыс.руб.)";
            dataItemCaption2.Text = "(тыс.руб.)";

            outcomesGridHintLabel.Text = @"В связи с внесением изменений в статью 21 Бюджетного кодекса Российской Федерации 
(в редакции Федерального закона от 30.09.2010 № 245-ФЗ) сравнение отчетных данных по разделам классификации расходов бюджетов 
с аналогичными данными прошлого года производить некорректно";

            incomesGridLabel.Text = string.Empty;
            outcomesGridLabel.Text = string.Empty;
            ...
            if (sts.Length > 0)
            {
                switch (level)
                {
                    case 1:
                        {
                            string month = sts[1].TrimEnd(' ');
                            DateTime dateTime = new DateTime(Convert.ToInt32(sts[0]), CRHelper.MonthNum(month), 1);
        ...
        protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
        {
            string query = (sender == UltraWebGrid1) ? 
                DataProvider.GetQueryText("FO_0037_0001_01_grid_incomes") : 
                DataProvider.GetQueryText("FO_0037_0001_01_grid_outcomes");
...
Доходов поступило на <b>{4:P2}</b> {6}, чем за аналогичный период предыдущего года (в {7} году поступления составили <b>{5:N0}</b> тыс.руб.).",
                ...
                {
                    outcomesGridLabel.Text = string.Format(
                            @"Расходы областного бюджета <b>{0}</b> произведены в объеме <b>{1:N0}</b> тыс.руб. Уточненные годовые назначения по расходам 
составляют <b>{2:N0}</b> тыс.руб., расходная часть бюджета исполнена на <b>{3:P2}</b>. Расходов произведено на <b>{4:P2}</b> {6}, чем за 
аналогичный период предыдущего года (в {7} году расходы составили <b>{5:N0}</b> тыс.руб.).",
                            dateTimeStr, currYearFact, currYearPlan, executePercent, Math.Abs(rateValue), prevYearFact, rateArrow, year - 1);
                }
            }
        }
...
", " ");
        ...
        private void ExcelExporter_BeginExport(object sender, BeginExportEventArgs e)
        {
            ...
            e.Workbook.Worksheets["Расходы бюджета - Итого"].Rows[0].Cells[0].Value = PageTitle.Text + " " + PageSubTitle.Text;
            e.Workbook.Worksheets["Расходы бюджета - Итого"].Rows[1].Cells[0].Value = CommentText.Text;
            e.Workbook.Worksheets["Расходы бюджета - Итого"].Rows[2].Cells[0].Value = CommentTextExportsReplaces(outcomesGridLabel.Text);
        }
        ...
        private void ExcelExportButton_Click(object sender, EventArgs e)
        {
            ...
            Worksheet sheet2 = workbook.Worksheets.Add("Расходы бюджета - Итого");
        ...
        private void PdfExportButton_Click(object sender, EventArgs e)
        {
            ...
            gridNumber++;
            UltraGridExporter1.PdfExporter.Export(UltraWebGrid2, section1);
        }
        ...
        private void PdfExporter_BeginExport(object sender, DocumentExportEventArgs e)
        {
            ...
            else
            {
                ...
                Font font = new Font("Verdana", 14);
                title.Style.Font = new Infragistics.Documents.Reports.Graphics.Font(font);
                title.AddContent("\n" + CommentTextExportsReplaces(outcomesGridLabel.Text));
            }

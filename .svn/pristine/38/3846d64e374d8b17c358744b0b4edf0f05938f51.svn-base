...
{
    public partial class FO_0037_0001_H : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            TextBox1.Text = String.Format("По доходам в динамике на {0}", period);

            period = String.Format("{0} {1} {2} года", dateOutcomes.Day, CRHelper.RusMonthGenitive(dateOutcomes.Month), dateOutcomes.Year);
            TextBox2.Text = String.Format("По расходам в динамике на {0}", period);
            ...
            Label2.Text = string.Format("данные на {0} {1} {2} года", reportDate.Day, CRHelper.RusMonthGenitive(reportDate.Month), reportDate.Year);
        ...
        protected void UltraWebGrid1_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0037_0001_H_grid1");
        ...
        protected void UltraWebGrid2_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0037_0001_H_grid2");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Показатель", dt);
            dt.Columns.RemoveAt(0);
            foreach (DataRow row in dt.Rows)
            {
        ...
        private void SetupLayout(LayoutEventArgs e, string caption)
        {
            ...
            DateTime tableDate;
            if (caption == "Показатель доходов")
            {
                tableDate = dateIncomes;
            }
            else
            {
                tableDate = dateOutcomes;
            }

            e.Layout.Bands[0].Columns[0].Header.Caption = caption;
            e.Layout.Bands[0].Columns[1].Header.Caption = String.Format("Исполнено за {0}, тыс. руб", tableDate.Year - 1);
            e.Layout.Bands[0].Columns[2].Header.Caption =
                String.Format("Исполнено на {0:dd.MM.yyyy}, тыс. руб.", tableDate.AddYears(-1));
            e.Layout.Bands[0].Columns[3].Header.Caption =
                String.Format("Исполнено на {0:dd.MM.yyyy}, тыс. руб.", tableDate);
            e.Layout.Bands[0].Columns[4].Header.Caption = String.Format("Рост/ снижение в % к {0} году", tableDate.Year - 1);

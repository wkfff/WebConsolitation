...
namespace Krista.FM.Server.Dashboards.reports.SKK.SKK_014
{
	public partial class Default : CustomReportPage
	{
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			PageTitle.Text = "Досмотр транспортных средств в пунктах пропуска выбранного вида международного сообщения";
			Page.Title = PageTitle.Text;
			PageSubTitle.Text = String.Format("Досмотр транспортных средств в пунктах пропуска вида сообщения \"<b>{0}</b>\" за <b>{1} {2} года</b> в сравнении с <b>{3} {4} года</b>, направление: <b>{5}</b>",
			...
			labelGrid.Text = "";
			labelChartDistricts.Text = "Досмотр ТС по федеральным округам";
			labelChartCompare.Text = "Досмотр ТС по выбранному виду международного сообщения"; 
			labelChartBorders.Text = "Досмотр ТС по участкам границы";
		...
		private void SetDataHelpers()
		{
			...
			gridTransportHelper = new GridUpDownTransport(GridBrick);
			gridTransportHelper.SetStyle();
			if (comboTransport.SelectedValue.Trim().ToLower().Equals("всего"))
			{
				// в разрезе видов сообщения
				labelGrid.Text = "Досмотр ТС по видам международного сообщения";
				gridTransportHelper.LabelCommon = "Вид сообщения";
				gridTransportHelper.SetData("skk_014_grid_transport");
			}
			else 
			{
				// в разрезе пунктов пропуска
				labelGrid.Text = "Досмотр ТС по пунктам пропуска";
				gridTransportHelper.LabelCommon = "Пункт пропуска";
				gridTransportHelper.SetData("skk_014_grid_points");
			}
			scriptBlock.JavaScript_SetVerticalScrollBar(gridTransportHelper.Grid);
			...
			chartCompareHelper.SetData("skk_014_chart_compare");

			// диаграмма по федеральным округам
			chartDistrictsHelper = new ChartDistrictsTransport(ChartDistricts);
			chartDistrictsHelper.SetStyle(SKKHelper.default1ItemsWidth, SKKHelper.defaultChartHeight);
			chartDistrictsHelper.SetData("skk_014_chart_districts");
			
			// диаграмма по участкам границы
			GridBorderPlace.Visible = false;
			chartBordersHelper = new ChartBordersTransport(ChartBorders);
				...
				&& !comboTransport.SelectedValue.Trim().ToLower().Equals("речной"))
			{
				...
				chartBordersHelper.SetData("skk_014_chart_borders");
				chartCompareHelper.SetChartHeight(chartBordersHelper.ChartHeight);
				...
				if ((chartBordersHelper.Chart.Series.Count > 0) && (chartBordersHelper.Chart.Series[0].GetPoints().Count == 1))
				{
					...
					gridBorderHelper.SetLabels(GridBorderLabelParam.Transport, GridBorderLabelCount.Transport);
					gridBorderHelper.SetStyle();
					gridBorderHelper.SetFullAutoSizes();
					gridBorderHelper.SetData("skk_014_grid_borders");

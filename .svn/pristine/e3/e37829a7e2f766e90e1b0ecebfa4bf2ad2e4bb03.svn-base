...
{
    public class ControlForOGVView : View
    {
        ...
        public ControlForOGVView(IFO51Extension extension, int periodId, D_Regions_Analysis region)
        {
        ...
        private Store CreateDefectsStore()
        {
            ...
            ds.SetHttpProxy("/FO51FormSbor/ControlDefects?periodId={0}&regionId={1}".FormatWith(periodId, region.ID))
                .SetJsonReader()
                .AddField("MarkId")
                .AddField("MarkName")
                .AddField("PassportMOPlanPeriod")
                .AddField("MesOtchPlanPeriod")
                .AddField("PassportMOFactPeriod")
                .AddField("MesOtchFactPeriod")
                .AddField("DefectPlanPeriod")
                .AddField("DefectFactPeriod");
        ...
        private GridPanel CreatePlanGrid(string storeId, ViewPage page)
        {
            ...
            AddColumn(gp, "MarkName", "Наименование показателя").Align = Alignment.Left;

            AddColumn(gp, "PassportMOFactPeriod", "Данные от МО").RendererFn(RendererFn).SetWrap(false);
            AddColumn(gp, "MesOtchFactPeriod", "Данные по месячной отчетности").RendererFn(RendererFn).SetWrap(false);
            AddColumn(gp, "DefectFactPeriod", "Отклонение").RendererFn(RendererFn).SetWrap(false);

            AddColumn(gp, "PassportMOPlanPeriod", "Данные от МО").RendererFn(RendererFn).SetWrap(false);
            AddColumn(gp, "MesOtchPlanPeriod", "Данные по месячной отчетности").RendererFn(RendererFn).SetWrap(false);
            AddColumn(gp, "DefectPlanPeriod", "Отклонение").RendererFn(RendererFn).SetWrap(false);

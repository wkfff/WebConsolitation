    ...
    public partial class DefaultOKVD : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            else
            {
                ...
                {
                    selectedRegion.Value = string.Format("{0}.[Все районы]",
            ...
            chartLabel1.Text = string.Format("Прирост/снижение н{1} по видам деятельности с начала {0} года",ComboYear.SelectedValue, genStr);
            chartLabel2.Text =string.Format("Оценка роста/снижения н{0} по видам деятельности в сравнении с прошлым годом", genStr);
            ...
            gridCommentLabel.Text = "* поступления по выбранному в параметре доходному источнику приводятся по данным УФНС России по ХМАО-Югре, передаваемым по Приказу Минфина РФ № 65н, ФНС РФ № ММ-3-1 295@ от 30 июня 2008 года";
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            if (e.Layout.Bands[0].Columns.Count > 13)
            {
                ...
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 7, string.Format("на 01.{0:00}.{1}", nextMonth, lastYear), string.Format("Объем поступлений* ({3}) за {0} {1} {2} года", month, CRHelper.RusManyMonthGenitive(month), lastYear, ComboKD.SelectedValue.TrimEnd(' ')));
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 8, string.Format("на 01.{0:00}.{1}", nextMonth, nextYear), string.Format("Объем поступлений* ({3}) за {0} {1} {2} года", month, CRHelper.RusManyMonthGenitive(month), year, ComboKD.SelectedValue.TrimEnd(' ')));
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 9, "отклонение", "отклонение общего объема поступлений"); CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 10, string.Format("на 01.{0:00}.{1}", nextMonth, lastYear), string.Format("Удельный вес н{3} в общем объеме поступлений за {0} {1} {2} года", month, CRHelper.RusManyMonthGenitive(month), lastYear, genStr));
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 11, string.Format("на 01.{0:00}.{1}", nextMonth, nextYear), string.Format("Удельный вес н{3} в общем объеме поступлений, % за {0} {1} {2} года", month, CRHelper.RusManyMonthGenitive(month), year, genStr));
                CRHelper.SetHeaderCaption(e.Layout.Grid, 0, 12, "отклонение", "отклонение удельного веса недоимки в общем объеме поступлений");
                ...
                ch.Caption = String.Format("Объем поступлений* ({0}), тыс.руб.", ComboKD.SelectedValue.TrimEnd(' '));
                ch.RowLayoutColumnInfo.OriginY = 0;
                ...
                ch = new ColumnHeader(true);
                ch.Caption = string.Format("Удельный вес н{0} в общем объеме поступлений, %", genStr);
                ch.RowLayoutColumnInfo.OriginY = 0;
            ...
            for (int i = 0; i < e.SceneGraph.Count; i++)
            {
                ...
                if (primitive is Box)
                {
                    ...
                    if (box.DataPoint != null && box.Value != null)
                    {
                        ...
                        if (value > 0)
                        {
                            box.DataPoint.Label = string.Format("{0}\nРост н{2} в сравнении с аналогичным периодом прошлого года\n{1}",kdName,AbsoluteMeasureSelected ? string.Format("{0:N2} тыс.руб.", value) : string.Format("{0:P2}", value),genStr);
                            box.PE.ElementType = PaintElementType.Gradient;
                        ...
                        {
                            box.DataPoint.Label = string.Format("{0}\nСнижение н{2} в сравнении с аналогичным периодом прошлого года\n{1}",
                                kdName,
                                AbsoluteMeasureSelected ? string.Format("{0:N2} тыс.руб.", value) : string.Format("{0:P2}", value), genStr);
                            box.PE.ElementType = PaintElementType.Gradient;
                            box.PE.FillGradientStyle = GradientStyle.Horizontal;

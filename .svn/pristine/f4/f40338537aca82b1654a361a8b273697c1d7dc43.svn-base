...
namespace Krista.FM.RIA.Extensions.E86N.Presentation.Controllers.PFHD
{
    public class RealAssetFundsController : SchemeBoundController
    {
        ...
        public ActionResult Read(int? parentId)
        {
            try
            {
                ...
                return new RestResult
                    {
                        Success = true, 
                        Data = newRestService.GetItems<F_Fin_realAssFunds>()
                            .Where(x => x.RefParametr.ID == parentId)
                            .Select(
                                item => new RealAssetFundsViewModel
                                    {
                                        ID = item.ID, 
        ...
        public RestResult Save(string data, int parentId)
        {
            try
            {
                F_Fin_realAssFunds newPfhd = JavaScriptDomainConverter<F_Fin_realAssFunds>.DeserializeSingle(data);
        ...
        {
            return newRestService.DeleteDocDetailAction<F_Fin_realAssFunds>(id, parentId);
        }

        protected string ValidateData(F_Fin_realAssFunds record)
        {

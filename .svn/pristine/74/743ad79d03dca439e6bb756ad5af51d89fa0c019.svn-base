...
namespace Krista.FM.Client.ViewObjects.AssociatedCLSUI.Association
{
    public partial class AssociationUI : BaseViewObj
    {
            ...
            switch (tool.Key)
            {
                ...
                case "AddToBridge":
                    ...
                    foreach (UltraGridRow row in gridClsData.Selected.Rows)
                    {
                        ...
                        try
                        {
                            id = curentAssociation.CopyAndAssociateRow(Convert.ToInt32(row.Cells["ID"].Value), attClsBridge.CurrentSourceID);
                            if (attClsBridge.IsCurrentVersion)
                            {
                ...
                case "AssociateMaster":
                    ...
                    if (AssociateMaster.Associate(curentAssociation, attClsData.CurrentSourceID, attClsBridge.CurrentSourceID, this.Workplace))
                    {
                        refreshDataCls = true;
                    }
                    break;
                ...
                case "CreateBridge":
                        ...
                        DialogResult.Yes)
                    {
                        ...
                        try
                        {
                            ...
                            curentAssociation.FormBridgeClassifier(attClsData.CurrentSourceID, attClsBridge.CurrentSourceID);
                ...
                case "AddToBridgeAll":
                    ...
                    try
                    {
                        if (curentAssociation as IBridgeAssociationReport != null)
                        {
                            ((IBridgeAssociationReport) curentAssociation).CopyAndAssociateAllRow(
                                attClsData.CurrentSourceID, attClsBridge.CurrentSourceID);
                    ...
                    break;
                case "ReplaceLink":
                    string sourceName = attClsBridge.CurrentSourceID == 0
                                                ? @"ФО\0022 Классификаторы - 2011"
                                                : Workplace.ActiveScheme.DataSourceManager.GetDataSourceName(
                                                    attClsBridge.CurrentSourceID);
                    if (MessageBox.Show(
                        String.Format("Вы действительно хотите перенеси ссылки на версию сопоставимого классификатора {0}", sourceName),
                        "Перенос ссылок",
                        MessageBoxButtons.OKCancel,
                        MessageBoxIcon.Information) == DialogResult.OK)
                    {
                        Workplace.OperationObj.Text = "Перенос ссылок на новую версию классификатора";
                        Workplace.OperationObj.StartOperation();
                        int count;
                        try
                        {
                            count = curentAssociation.ReplaceLinkToNewVersionCls(attClsBridge.CurrentSourceID, 0, attClsData.CurrentSourceID);
                        }
                        finally
                        {
                            Workplace.OperationObj.StopOperation();
                        ...
                        MessageBox.Show(
                                String.Format(
                                    "Количиство перенесенных записей на версию классификатора {0} - {1}.",
                                    sourceName, count),
                                "Перенос ссылок", MessageBoxButtons.OK, MessageBoxIcon.Information);
        ...
        private bool CheckAssociate()
        {
            if (this.curentAssociation.AssociationClassType == AssociationClassTypes.BridgeBridge)
            {
                int bridgeSourceID = curentAssociation.GetAssociateBridgeSourceID(null, attClsData.CurrentSourceID);
                if (bridgeSourceID == -1)
                {
                    return true;
                }

                if (bridgeSourceID != attClsBridge.CurrentSourceID)
                {
                    MessageBox.Show(
                        String.Format(
                            "Текущая версия сопоставимого классификатора сопоставлена с версией {0}. Для сопоставления с другой версией, необходимо очистить ранее установленное сопоставление.",
                            Workplace.ActiveScheme.DataSourceManager.GetDataSourceName(bridgeSourceID)),
                        "Предупреждение",

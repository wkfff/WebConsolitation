...
{
    public partial class EO_0002_0001 : CustomReportPage
    {
        #region Поля
        ...
        private void BindInfoText()
        {
           // string queryCountText = string.Format("select count (*) from {0} {1}", "d_Org_FNSDebtor", filter);
            ...
            string query = DataProvider.GetQueryText("EO_0002_0001_info");
            DataTable dtInfo = new DataTable();
            DataProvidersFactory.SecondaryMASDataProvider.GetDataTableForChart(query, "Наименование показателей", dtInfo);

            lbInfo.Text = String.Format(@"В Ханты-Мансийском автономном округе действует&nbsp;<span class='DigitsValueLarge'>{0:N0}</span>&nbsp;целевая программа на общую сумму&nbsp;<span class='DigitsValueLarge'>{1:N2}</span>&nbsp;млн.руб. со сроком реализации в&nbsp;<span class='DigitsValue'>2010-2020 гг.</span>&nbsp;<br/>
Плановый объем финансирования в&nbsp;<span class='DigitsValue'>{5:yyyy}</span>&nbsp;году по уточненной бюджетной росписи составил&nbsp;<span class='DigitsValueLarge'>{6:N2}</span>&nbsp;млн.руб.<br/>
По состоянию на&nbsp;<span class='DigitsValue'>{2:dd.MM.yyyy}</span>&nbsp;года&nbsp;<span class='DigitsValue'>фактически исполнено</span>&nbsp;за&nbsp;<span class='DigitsValue'>{5:yyyy}</span>&nbsp;год по целевым программам&nbsp;<span class='DigitsValueLarge'>{3:N2}</span>&nbsp;млн.руб. (<span class='DigitsValueLarge'>{4:P2}</span>&nbsp;от уточненной бюджетной росписи на&nbsp;<span class='DigitsValue'>{5:yyyy}</span>&nbsp;год)",
               dtInfo.Rows[0][1],
               dtInfo.Rows[0][2],
        ...
        {
            string query = DataProvider.GetQueryText("EO_0002_0001_grid");
        ...
        protected string GetGaugeUrl(object oValue, string alt)
        {
            ...
            if (value > 100)
                value = 100;
            string path = "EO_0002_0001_gauge_" + value.ToString("N0") + ".png";

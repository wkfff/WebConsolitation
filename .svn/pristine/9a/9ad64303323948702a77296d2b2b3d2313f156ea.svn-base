...
{
    public partial class FO_0735_0007 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!dtDate.Rows[0][4].ToString().Contains("Заключительные обороты"))
            {
                ...
                if (IsQuaterPlanType)
                {
                    ...
                    lbDate.Text = "&nbsp;(на&nbsp;<span style='color: white;'>" + date.ToString("dd.MM.yyyy") + "</span>) по главным распорядителям средств бюджета округа";
                }
                else
                {
        ...
        private TableCell GetCell(int i, int column)
        {
            ...
            CRHelper.SaveToErrorLog(dtData.Rows[dtData.Rows.Count - 1][column].ToString());
            double ethalon = Convert.ToDouble(dtData.Rows[dtData.Rows.Count - 1][column]);
        ...
        private string GetImageHint(double ethalon, int row, int column)
        {
            ...
            if (Double.TryParse(dtData.Rows[row][column].ToString(), out value))
            {
                return value >= ethalon ?
                        String.Format("<span style=\"font-family: Arial; font-size: 14pt\">Процент исполнения&nbsp<b>{0:P0}</b>&nbspвыше среднего по ГРБС (<b>{1:P0}</b>)</span>", dtData.Rows[row][column], ethalon) :
                        String.Format("<span style=\"font-family: Arial; font-size: 14pt\">Процент исполнения&nbsp<b>{0:P0}</b>&nbspниже среднего по ГРБС (<b>{1:P0}</b>)</span>", dtData.Rows[row][column], ethalon);
            }
            return String.Empty;
        }
        ...
        private TableCell GetNameCell(string name, bool child)
        {
            ...
            cell.CssClass = "HtmlTableCompact";
            lb = new Label();
            lb.Text = String.Format("<a href='webcommand?showPopoverReport=fo_0735_0008_GRBS={1}&width=690&height=530&fitByHorizontal=true'>{0}</a>", name, CustomParams.GetGrbsIdByName(name));
        ...
        private void AddHeaderRow(Table table)
        {
            ...
            cell.Width = 30;
            SetupHeaderCell(cell);
            cell.Text = "Начисл. на з/п";
            row.Cells.Add(cell);

            cell = new TableCell();
            cell.Width = 30;
            SetupHeaderCell(cell);
            cell.Text = "Коммун. услуги";

...
namespace Krista.FM.Server.Dashboards.reports.FK_0001_0003
{
    public partial class DefaultCompare : CustomReportPage
    {
        ...
        {
            string query = DataProvider.GetQueryText("FK_0001_0003_compare_map");
            mapDt = new DataTable();
            ...
            foreach (DataRow row in mapDt.Rows)
            {
                    ...
                    row[completePercentColumnName] != DBNull.Value && row[completePercentColumnName].ToString() != String.Empty)
                {
                    ...
                    if (AllFO && RegionsNamingHelper.IsFO(subject) || !AllFO && RegionsNamingHelper.IsSubject(subject))
                    {
                        ...
                        if (shape != null)
                        {
                            double value = Convert.ToDouble(row[completePercentColumnName]);
                            ...
                            shape["Complete"] = value / 1000;
                            shape.ToolTip = "#NAME\n#COMPLETE{N3} тыс.руб./чел.";
                            shape.TextVisibility = TextVisibility.Shown;
                            shape.Text = String.Format("{0}\n{1:N3}", subject.Replace(" ", "\n"), (value / 1000));
                            if (subject == "Камчатский край")
        ...
        {
            string query = DataProvider.GetQueryText("FK_0001_0003_compare_Grid");
        ...
        private void PdfExportButton_Click(object sender, EventArgs e)
        {
            ...
            ReportPDFExporter1.Export(GridBrick.GridHeaderLayout, Label2.Text.Replace("&nbsp;", " "), section1);

            ISection section2 = report.AddSection();
            DundasMap.Width = Unit.Pixel(Convert.ToInt32(DundasMap.Width.Value * 0.85));
            ReportPDFExporter1.Export(DundasMap, Label2.Text.Replace("&nbsp;", " "), section2);
        }

...
namespace Krista.FM.Server.Dashboards.GKH_0004
{
    public partial class _default : CustomReportPage
    {
        ...
        class CustomizerSize_1280x1024 : ICustomizerSize
        {
            ...
            protected override void ContfigurationGridIE(UltraWebGrid Grid)
            {
                ...
                int FirstWidthColPercent = 31;

                foreach (UltraGridColumn col in Grid.Columns)
                {
                    col.Width = (((98 - FirstWidthColPercent) / (Grid.Columns.Count - 1)) * onePercent);
                }

                Grid.Columns[0].Width = onePercent * FirstWidthColPercent;
            }
        ...
        private void ConfCombo(bool IsSmall)
        {
            ...
            //ComboRegion.Width = IsSmall ? 430 : 320;
            //ComboComapraDate.Width = IsSmall ? 430 : 300;

            ComboCurDay.Width = IsSmall ? 430 : 550;///250;
            ComboRegion.Width = IsSmall ? 430 : 550;///320;
            ComboComapraDate.Width = IsSmall ? 430 : 550;//300;


            ComboComapraDate.Title = "Выберите период для сравнения";

            Link1.Text = "Мониторинг&nbsp;средних&nbsp;тарифов&nbsp;на&nbsp;коммунальные<br>услуги&nbsp;в&nbsp;разрезе&nbsp;муниципальных&nbsp;образований";

            //if (IsSmall)
            if (true)
            {
                Link1.Text = "Мониторинг&nbsp;средних<br>тарифов&nbsp;на коммунальные<br>услуги&nbsp;в&nbsp;разрезе<br>муниципальных&nbsp;образований";
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            Grid.Height = CustomizerSize is CustomizerSize_800x600?Unit.Empty:470;
        ...
        private void FillComboRegion()
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                //ComboRegion.SetСheckedState(ComboRegion.SelectedNode.Parent.Text, true);
        ...
        {
            SelectCity = ComboRegion.SelectedNode.Nodes.Count <= 0;

            ChosenRegionGrid.Value = SetRegion.DataUniqeName[ComboRegion.SelectedValue] + (!SelectCity ? ".children" : "");
        ...
        private void fillStrRow(DataRow GridRow, string[] ParsRow)
        {
            ...
            GridRow["Yslyga"] = ParsRow[isBig.Checked ? 2 : 1].ToString();//SetEdizm(,EdIzmTable);
        ...
        void FuzzyRemoveCol(DataTable Table, string ColName)
        {
            string realname = "";

            foreach (DataColumn col in Table.Columns)
            {
                if (col.ColumnName.Contains(ColName))
                {
                    realname = col.ColumnName;
                    break;
                }
            }
            if (realname != "")
        ...
        DataTable FormatBaseTable(DataTable BaseTable)
        {
            ...
            //string PrevMounth = SetCurDay.OtherInfo[GetPrevNode(ComboCurDay.SelectedNode).Text]["mounth"];
       ...
       CurMounth + "; при наличии приборов учёта; Тариф ");
            ...
            FuzzyRemoveCol(BaseTable, CurMounth + "; по приборам учета; Тариф ");
            FuzzyRemoveCol(BaseTable, CurMounth + "; по приборам учета; Норматив ");
            FuzzyRemoveCol(BaseTable, PrevMounth + "; по приборам учета; Тариф ");
            FuzzyRemoveCol(BaseTable, PrevMounth + "; по приборам учета; Норматив ");
        ...
        private string SetEdizm(string p, DataTable EdIzmTable)
        {
            foreach (DataRow row in EdIzmTable.Rows)
            {
                if (p.Contains(row["Yslyg"].ToString()))
                {
                    return string.Format("{0}, рублей за {1}", p, row["edizm"]);
                }
            }
            return "";
        }

        List<string> ListYsl = new System.Collections.Generic.List<string>();

        DataTable EdIzmTable = null;
        private void DataBindGrid()
        {
            DataTable BaseTable = DataBaseHelper.ExecQueryByID(
                (!isBig.Checked ? "SmalGrid" : "Grid"), "Field");

            EdIzmTable = DataBaseHelper.ExecQueryByID("EdIzm", "Yslyg");

            FormatBaseTable(BaseTable);
            ...
            foreach (DataRow BaseRow in BaseTable.Rows)
            {
                ...
                string ysl = SetEdizm(ParsRow[1], EdIzmTable);
                string Reg = ParsRow[0];

                if (Reg != TitleRegion)
                {
                    DataRow TitleRowRegion = GridTable.AddRow();
                    TitleRegion = Reg;
                    TitleRowRegion["Yslyga"] = Reg;
                    ListYsl.Add(Reg);
                }
                ...
                if (ysl != lastYsl)
                {
                    ...
                    {
                        lastYsl = ysl;
                        DataRow TitleRow = GridTable.AddRow();
                        TitleRow["Yslyga"] = ysl;
                        ListYsl.Add(ysl);
                    }
                }
                ...
                if (!isBig.Checked)
                {
                    GridRow["Yslyga"] = SetEdizm(GridRow["Yslyga"].ToString(), EdIzmTable);
                }
                
                GridRow["val1"] = BaseRow[1];
                GridRow["val2"] = BaseRow[2];
                if (isBig.Checked)
                {
                    GridRow["val3"] = BaseRow[3];
                }

                int adededPrevColNumber = 3; 

                GridRow["val1-Deviation"] = GetDeviation(BaseRow[1], BaseRow[1 + adededPrevColNumber]);
                GridRow["val2-Deviation"] = GetDeviation(BaseRow[2], BaseRow[2 + adededPrevColNumber]);

                GridRow["val1-SpeedDeviation"] = GetSpeedDeviation(BaseRow[1], BaseRow[1 + adededPrevColNumber]);
                GridRow["val2-SpeedDeviation"] = GetSpeedDeviation(BaseRow[2], BaseRow[2 + adededPrevColNumber]);
            }



            Grid.DataSource = GridTable.Rows.Count > 0 ? GridTable : null;
            Grid.DataBind();
        }
        ...
        private void ConfHeader()
        {
            ...
            HL.AddCell("Вид благоустройства");
            ...
            ghc.AddCell("Средний тариф");
            ghc.AddCell("Абсолютное отклонение");
            ghc.AddCell("Темп прироста, %");
            if (isBig.Checked)
            {
                ghc.AddCell("Норматив");
            }
            HL.ApplyHeaderInfo();
        ...
        private void FormatRowYsly(UltraWebGrid Grid, string p)
        {
            ...
            foreach (UltraGridRow Row in Grid.Rows)
            {
                ...
                if (ListYsl.Contains(cell.Text))
                //if (cell.Text[0] != ' ')
                {
                    //if ((cell.Text.Contains("район"))
                    // || (cell.Text.Contains("Город")))
                    //{
                    //    cell.Style.Font.Bold = true;
                    //    cell.Style.BackColor = Color.FromArgb(200, 200, 200);
                    //    cell.ColSpan = 7;
                    //    cell.Text = ClearDataMember(cell.Text);
                    //}
                    //elsepa
                    //{
                    ...
                    cell.Style.Font.Bold = cell.Text.Contains("район") || cell.Text.Contains("город");
                    //SetRegion.DataForCombo.ContainsKey(cell.Text); 
                }
                else
                {
        ...
        private void FormatRow()
        {
            ...
            foreach (UltraGridRow Row in Grid.Rows)
            {
                try
                {
                    if (Row.Cells[0].Text.Contains("район") || Row.Cells[0].Text.Contains("Город"))
                    {
                        
                        Row.Cells[0].Style.Font.Bold =
                            Row.Cells[0].Text.Contains("район") ||
                            Row.Cells[0].Text.Contains("Город");
                    }
                    if (Row.Cells[0].Text[0] != ' ')
                    {
                        Row.Cells[0].Style.Font.Bold = true;
                        Row.Cells[0].ColSpan = (isBig.Checked?8:7);
                        Row.Cells[0].Style.BackColor = Color.FromArgb(222, 222, 222);
                    }
        ...
        void CustomizerOther()
        {
            ...
            Grid.DisplayLayout.AllowSortingDefault = isBig.Checked ? AllowSorting.No : AllowSorting.OnClient;
        ...
        void SaveToErrorLogArrayObj(object[] objs)
        {
            string s = "";
        ...
        private void GenTextovka()
        {
            ...
            ChosenPrevPeriod.Value = ChosenCurPeriod.Value;
            DataTable TableValue = DataBaseHelper.ExecQueryByID("SmalGrid_", "Field");

            FormatBaseTable(TableValue);
            ...
            {
                {
                    try
                    {
                        decimal avgValue = (decimal)Row[2];
                        decimal NormValue = (decimal)Row[3];

                        if (Row[0].ToString().Contains("Отопление"))
                        {
                            sum += NormValue * avgValue * 54;
                        }
                        else
                        {
                            sum += NormValue * avgValue * 3;
                        }
                    }
                    catch { }
                }
            ...
            if (sum != 0)
            {
                Textovka.Text = string.Format("Примерная стоимость содержания типовой двухкомнатной неприватизированной квартиры для семьи из трех человек общей площадью 54 м<sup>2</sup> составляет {0:N2} рублей.", sum);
            }
        ...
        {
            PageSubTitle.Text = string.Format("Анализ средних тарифов для населения с учётом надбавки, НДС и прочих выплат, применяемых при начислении платежа гражданину, на коммунальные услуги, {1}, по состоянию на {0} по сравнению с {2}",
                ComboCurDay.SelectedValue.ToLower(), ComboRegion.SelectedValue, GetAnalogForHeader()); 

            Hederglobal.Text = "Мониторинг средних тарифов на коммунальные услуги муниципального образования";
            Page.Title = Hederglobal.Text;

            
            Link1.NavigateUrl = "../GKH_0003/default.aspx";

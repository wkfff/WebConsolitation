...
namespace Krista.FM.Server.Dashboards.reports.SEP_001
{
    public partial class _default : CustomReportPage
    {
                ...
                for (int i = StartRow; i < RowCount; i++)
                {
                    for (int j = StartRow; j < RowCount; j++)
                    {
                        if ((Table.Rows[j][ColumnValueName] != System.DBNull.Value) &&
                            (Table.Rows[IndexMax][ColumnValueName] != DBNull.Value))
                        {
                            if (revrce)
                            {
        ...
        private static class CustomizeUltraGridHelper
        {
            ...
            public static void SetStarGrayFromValue(UltraWebGrid Grid, int ColFromImage, bool reverce, string Value)
            {
                ...
                if (IndexRow != null)
                {
                    CustomizeUltraGridHelper.SetStar(Grid, ColFromImage, (int)IndexRow, "~/images/starGrayBB.png", reverce ? "Наименьшее отклонение" : "Наибольшее отклонение");
                }
            }
        ...
        class DataSetComboTwoLevel : IDataSetCombo
        {
            public override void LoadData(DataTable Table)
            {
                ...
                {
                    string UniqueName = row["UniqueName"].ToString();
                    if (UniqueName.ToUpper().Contains("DATAMEMBER"))
                    {
                        continue;
                    }
                    string Child = ParserDataUniqName.GetDouwnLevel(UniqueName);
                    ...
                    if (Child.Contains("Индекс промышленного производства"))
                    {
                        UniqueName = "[СЭП__Годовой сборник].[СЭП__Годовой сборник].[Индекс промышленного производства: добыча полезных ископаемых]";

                        Child = UserComboBox.getLastBlock(UniqueName);

                        this.AddItem(Child, Parent, UniqueName);

                        this.addOtherInfo(Table, row, Child);

                        UniqueName = "[СЭП__Годовой сборник].[СЭП__Годовой сборник].[Индекс промышленного производства: обрабатывающие производства]";
                        ...
                        UniqueName = "[СЭП__Годовой сборник].[СЭП__Годовой сборник].[Индекс промышленного производства: производство и распределение электроэнергии, газа и воды]";
        ...
        private void ConfigurationMultiCombo()
        {
            ...
            ChosenRegion.Value = RegionsNamingHelper.GetFoBySubject(RegionSettings.Instance.Name);
        ...
        protected void SetColumnNameGrid()
        {
            ...
            ColsGrid.FromKey("Growth").Header.Caption = "Абсолютное отклонение от АППГ";
            ColsGrid.FromKey("Growth").Header.Title = "Абсолютное отклонение от аналогичного периода предыдущего года";

            ColsGrid.FromKey("SpeedGrowth").Header.Caption = "Темп роста к АППГ";
            ColsGrid.FromKey("SpeedGrowth").Header.Title = "Темп роста к аналогичному периоду предыдущего года";

            ColsGrid.FromKey("DeviationFO").Header.Caption = "Отклонение (от ФО)";
            ColsGrid.FromKey("DeviationFO").Header.Title = "Отклонение (от Федерального округа)";
            ...
            ColsGrid.FromKey("DeviationRF").Header.Title = "Отклонение (от Российской Федерации)";
        ...
        private string GetMinColVal(UltraGridColumn col)
        {
            int max = int.MaxValue;

            foreach (UltraGridRow row in col.Band.Grid.Rows)
            {
                if (row.Cells.FromKey(col.BaseColumnName).Value != null)
                {
                    int valRang = int.Parse(row.Cells.FromKey(col.BaseColumnName).Value.ToString());
                    max = valRang < max ? valRang : max;
                }
            }
        ...
        private string GetMaxColVal(UltraGridColumn col)
        {
        ...
        protected void ConfigurationRows()
        {
            ...
            CustomizeUltraGridHelper.SetStarGrayFromValue(Grid, Grid.Columns.FromKey("RangFO").Index, CurentInverce,
                GetMaxColVal(Grid.Columns.FromKey("RangFO")));
            ...
            {
                CustomizeUltraGridHelper.SetStarGrayFromValue(
                    Grid, 
                    Grid.Columns.FromKey("RangRF").Index, 
                    CurentInverce, 
                    "8");
            }
        }
        ...
        protected void DelRow(DataTable Table, int indexRow)
        {
            if (Table.Rows.Count > indexRow)
            {
        ...
        protected void AddLegend()
        {
            ...
            legend1 = new Legend("CompleteLegend");
            legend1.Title = string.Format("{0}:\n{1} ({2} {3})", ComboDirection.SelectedNodeParent, SetHyphen(ComboDirection.SelectedValue),
        ...
        private void FillShape(Shape shape, DataRow dataRow)
        {
            ...
            CRHelper.SaveToErrorLog(dataRow["Value"].GetType().ToString());
            if (DirectIsindex)
            {
                if ((double)dataRow["Value"] > 100)
                {
        ...
        protected void DataBindMap()
        {
            ...
            {
                Shape shape = GetShapefromRegionName(
                    TableGrid.Rows[i]["ShortName"].ToString(),
                    TableGrid.Rows[i]["Region"].ToString());
        ...
        string GenShortFOName(string Name)
        {
            return Name.Replace("федеральный округ", "ФО");
        }

        protected void SetHeader()
        {
            Hederglobal.Text = "Ежемесячный мониторинг социально-экономического развития (в разрезе субъектов РФ)";
            //
            PageSubTitle.Text = string.Format(@"Анализ социально-экономического положения территории по выбранному показателю в разрезе субъектов РФ, {2}, по состоянию на {1} {0} года",
                "&nbsp<b>" + DataComboPeriod.OtherInfo[ComboPeriod.SelectedValue]["Year"] + "</b>",
                "<b>" + DataComboPeriod.OtherInfo[ComboPeriod.SelectedValue]["Mounth"].ToLower() + "</b>",
                "<b>" + ComboRegion.SelectedValue + "</b>");

            Page.Title = Hederglobal.Text;

            HeaderChart.Text = string.Format("Распределение субъектов РФ по показателю «{0}», {1}, по состоянию на {2}{3} года ({4})",
                ComboDirection.SelectedValue,
                CurrentUnit.ToLower(),
                "<b>" + DataComboPeriod.OtherInfo[ComboPeriod.SelectedValue]["Mounth"].ToLower() + "</b>&nbsp",
                "<b>" + DataComboPeriod.OtherInfo[ComboPeriod.SelectedValue]["Year"] + "</b>",
                GenShortFOName(ComboRegion.SelectedValue));

            HeaderGrid.Text = ComboDirection.SelectedValue;

            HeaderMap.Text = string.Format("{0}, {1} ({2})",
                ComboDirection.SelectedValue,
                CurrentUnit.ToLower(),
                GenShortFOName(ComboRegion.SelectedValue));
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            DirectIsindex = ComboDirection.SelectedValue.Contains("Индекс");
        ...
        {
            e.CurrentWorksheet.Rows[0].Cells[0].Value = Hederglobal.Text.Replace("август", " август");
            ...
            e.CurrentWorksheet.Rows[1].Cells[0].Value = PageSubTitle.Text.Replace("<b>", "").Replace("</b>", "").Replace("август", " август").Replace("&nbsp", " ");
        }
        ...
        private void ExcelExporter_EndExport(object sender, Infragistics.WebUI.UltraWebGrid.ExcelExport.EndExportEventArgs e)
        {
            ...
            try
            {
                e.Workbook.Worksheets["Диаграмма"].Rows[0].Cells[0].Value = HeaderChart.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", " ").Replace("август", " август");
                ReportExcelExporter.ChartExcelExport(e.Workbook.Worksheets["Диаграмма"].Rows[1].Cells[0], Chart);
                e.Workbook.Worksheets["Карта"].Rows[0].Cells[0].Value = HeaderMap.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", " ").Replace("август", " август");
        ...
        private void PdfExportButton_Click(object sender, EventArgs e)
        {
            ...
            title.AddContent(HeaderChart.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", ""));
            Chart.Height = 400;
            ...
            title.Style.Font = new Infragistics.Documents.Reports.Graphics.Font(font);
            title.Style.Font.Bold = true;
            title.AddContent(Hederglobal.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", "").Replace("август", " август"));
            title = section1.AddText();
            font = new Font("Verdana", 14);
            title.Style.Font = new Infragistics.Documents.Reports.Graphics.Font(font);
            title.AddContent(PageSubTitle.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", "").Replace("август", " август"));

            title = section1.AddText();
            font = new Font("Verdana", 14);
            title.Style.Font = new Infragistics.Documents.Reports.Graphics.Font(font);
            title.AddContent(HeaderGrid.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", "").Replace("август", " август"));
            ...
            font = new Font("Verdana", 14);
            title.Style.Font = new Infragistics.Documents.Reports.Graphics.Font(font);
            title.AddContent(HeaderMap.Text.Replace("<b>", "").Replace("</b>", "").Replace("&nbsp", "").Replace("август", " август"));

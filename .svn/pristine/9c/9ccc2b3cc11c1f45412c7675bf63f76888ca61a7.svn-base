...
{
    public partial class FO_0002_0005_Omsk : CustomReportPage
    {
        ...
        protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
        {
            ...
            string query = DataProvider.GetQueryText("FO_0002_0005_Omsk");
            ...
            if (dt.Columns.Count > 0 && dt.Rows.Count > 0)
            {
                ...
                dt.Rows[0]["Среднедушевые доходы"] = 1000 * avgIncomesTotal;
                dt.Rows[0]["Лучший по среднедуш.доходам МО"] = avgIncomesBestItem;
                if (avgIncomesMaxValue != double.MinValue)
                {
                    dt.Rows[0]["Лучшие среднедуш.доходы МО"] = avgIncomesMaxValue;
                }
                dt.Rows[0]["Худший по среднедуш.доходам МО"] = avgIncomesWorseItem;
                if (avgIncomesMinValue != double.MaxValue)
                {
                    dt.Rows[0]["Худшие среднедуш.доходы МО"] = avgIncomesMinValue;
                }
            ...
            for (int i = 0; i < dt.Columns.Count; i++)
            {
                ...
                switch (i)
                {
                    ...
                    case 8:
                        {
                            k = 0.001;
                            caption = "План.доходы на душу нас., тыс.руб./чел.";
                            break;
                        }
                    case 13:
                        {
            ...
            // строка для мин и макс среднедушевых доходов
            row = newDt.NewRow();
            row[0] = string.Empty;
        ...
        {
            return rowName != "Консолидир. бюджет" && rowName != "г.Омск";
        }
        ...
        private void AVGIncomesCalculate()
        {
            ...
            {
                query = DataProvider.GetQueryText("FO_0002_0005_Omsk_population");
                ...
                if (incomesDT.Columns.Count > 1)
                {
                    ...
                    avgIncomesTotal = Convert.ToDouble(incomesDT.Rows[0][executeColumnName]) / (1000 * populationTotal);
        ...
        protected void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
        {
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                ...
                if (executePercentRank || avgIncomesRank)
                {
                    ...
                    e.Row.Cells[0].Value = string.Format("макс&nbsp;&nbsp;<span style=\"color:white;\">{0}</span>&nbsp;</span>&nbsp;<img src=\"../../../images/starYellow.png\" width=\"15px\" height=\"15px\"/>&nbsp;{1}<br />" +

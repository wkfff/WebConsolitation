...
{
    public partial class FO_0035_0021 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            lbExecutedIncomes.Text = String.Format("{0:P2}", dtData.Rows[0]["Процент исполнения "]);
            lbPlanIncomes.Text = String.Format("{0:N2}", dtData.Rows[0]["Уточненный кассовый план "]);
            lbFactIncomes.Text = String.Format("{0:N2}", dtData.Rows[0]["Исполнено с начала года "]);

            double value = dtData.Rows[1]["Процент исполнения "] != DBNull.Value
                               ? Convert.ToDouble(dtData.Rows[0]["Процент исполнения "]) * 100
                               : 0;

            UltraGauge gauge = GetGauge(value);
            PlaceHolderIncomes.Controls.Add(gauge);

            lbExecutedOutcomes.Text = String.Format("{0:P2}", dtData.Rows[1]["Процент исполнения "]);
            lbPlanOutcomes.Text = String.Format("{0:N2}", dtData.Rows[1]["Уточненный кассовый план "]);
            lbFactOutcomes.Text = String.Format("{0:N2}", dtData.Rows[1]["Исполнено с начала года "]);

            value = dtData.Rows[1]["Процент исполнения "] != DBNull.Value
                               ? Convert.ToDouble(dtData.Rows[1]["Процент исполнения "]) * 100
                               : 0;
            gauge = GetGauge(value);
            PlaceHolderOutcomes.Controls.Add(gauge);

            lbRestStartPlan.Text = String.Format("{0:N2}", dtData.Rows[2]["Уточненный кассовый план "]);
            lbRestStartFact.Text = String.Format("{0:N2}", dtData.Rows[2]["Исполнено с начала года "]);
        }

...
namespace Krista.FM.Server.Dashboards.reports.SKK.SKK_013
{
	public partial class Default : CustomReportPage
	{
		...
		protected override void Page_Load(object sender, EventArgs e)
		{
			...
			PageTitle.Text = "Досмотр транспортных средств на выбранной территории";
			Page.Title = PageTitle.Text;
			PageSubTitle.Text = String.Format("Досмотр транспортных средств на территории \"<b>{0}</b>\" за <b>{1} {2} года</b> в сравнении с <b>{3} {4} года</b>, направление: <b>{5}</b>",
			...
			LabelGrid.Text = "Досмотр ТС на выбранной территории";
			LabelChartTerra.Text = "";
			LabelChartCompare.Text = "Досмотр ТС на выбранной территории";
			LabelChartBorder.Text = "Досмотр ТС по участкам границы"; 
			LabelTransport.Text = "Досмотр ТС по виду международного сообщения";
		...
		private void SetDataHelpers()
		{
			...
			gridUpDownHelper = new GridUpDownTransport(GridUpDown);
			gridUpDownHelper.SetStyle();
			gridUpDownHelper.LabelCommon = "Территория";
			gridUpDownHelper.SetData("skk_013_grid_updown");
			...
			{
				// по ФО
				LabelChartTerra.Text = "Досмотр ТС по федеральным округам";
				chartTerraHelper = new ChartDistrictsTransport(ChartTerra);
				chartTerraHelper.SetData("skk_013_chart_districts");
			}
			else if (comboTerra.SelectedNode.Level == 0)
			{
				// по субъектам
				LabelChartTerra.Text = "Досмотр ТС по субъектам";
				chartTerraHelper = new ChartSubjectsTransport(ChartTerra);
				chartTerraHelper.SetData("skk_013_chart_subjects");
			}
			else if (comboTerra.SelectedNode.Level == 1)
			{
				// по пунктам пропуска
				LabelChartTerra.Text = "Досмотр ТС по пунктам пропуска";
				chartTerraHelper = new ChartPointsTransport(ChartTerra);
				chartTerraHelper.SetData("skk_013_chart_points");
			}
			chartTerraHelper.SetStyle(SKKHelper.default1ItemsWidth, SKKHelper.defaultChartHeight);

			// сравнение, диаграмма
			chartCompareHelper = new ChartCmpTransport(ChartCompare);
			...
			chartCompareHelper.SetData("skk_013_chart_compare");

			// по участкам границы, диаграмма
			chartBordersHelper = new ChartBordersTransport(ChartBorder);
			...
			chartBordersHelper.SetData("skk_013_chart_borders");
			...
			if ((chartBordersHelper.Chart.Series.Count > 0) && (chartBordersHelper.Chart.Series[0].GetPoints().Count == 1))
			{
				...
				gridBorderHelper.SetLabels(GridBorderLabelParam.Terra, GridBorderLabelCount.Transport);
				gridBorderHelper.SetStyle();
				gridBorderHelper.SetFullAutoSizes();
				gridBorderHelper.SetData("skk_013_grid_borders");

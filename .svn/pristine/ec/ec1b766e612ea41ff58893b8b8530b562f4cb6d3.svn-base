...
namespace Krista.FM.Server.Dashboards.reports.FO_0035_0004
{
    public partial class Default : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                ComboBudget.SetСheckedState("Областной бюджет ", true);
            }
            int monthNum = ComboMonth.SelectedIndex + 1;
            int yearNum = Convert.ToInt32(ComboYear.SelectedValue);

            Page.Title = "Прогноз расходования средств областного бюджета";
        ...
        {
            string query = DataProvider.GetQueryText("FO_0035_0004_grid1");
            ...
            if (dtGrid1.Rows.Count > 0)
            {
                foreach (DataRow row in dtGrid1.Rows)
                {
                    ...
                    {
                        if (((i > 7 && i < 21) || (i == row.ItemArray.Length - 1)) && row[i] != DBNull.Value)
                        {
        ...
        {
            string query = DataProvider.GetQueryText("FO_0035_0004_grid2");
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            e.Layout.RowSizingDefault = AllowSizing.Fixed;
            ...
            {
                string formatString =  i == percentColumnIndex ? "P0" : "N0";
                int widthColumn = 90;
                switch(i)
                {
                    case 2:
        ...
        protected void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
        {
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                ...
                if (nameCell.Value != null && nameCell.Value.ToString() != string.Empty)
                {
                    ...
                    bool documentCountRow = nameCell.Value.ToString().ToLower().Contains("платежных документов");
                    bool lastColumn = i == e.Row.Cells.Count - 1;

                    if (i == 0 && (percentRow || remainsRow || documentCountRow))
                    {
                        cell.ColSpan = 8;
                    }
                    ...
                    {
                        cell.Value = (Convert.ToDouble(cell.Value) * 1000).ToString("P1");
                    }

                    if (i != 0 && !percentRow && !remainsRow && !documentCountRow && !monthEndRemainsRow)
                    {
                    ...
                    if (totalRow && lastColumn)
                    {
                        ...
                        //cell.RowSpan = 6;
                        int width = Convert.ToInt32(cell.Column.Width.Value) - 2;
                        string browser = HttpContext.Current.Request.Browser.Browser;
                        int height = browser == "Firefox" ? 118 : 135;
                        cell.Value = string.Format("<div style=\"position: absolute; width: {0}px; height: {1}px; border: none; margin-top: -10px; margin-left: -3px; * margin-left: -54px; *margin-top: -8px; *width: 112px; moz-height: 105px; background-color: #f1f1f2\">Дефицит средств для проведения расходов по доведенным лимитам</div>",
                            width, height);
...
//                        }
        ...
        {
            string query = DataProvider.GetQueryText("FO_0035_0004_commentText");
            ...
            if (dtCommentText.Rows.Count > 0)
            {
                ...
                string stateRemainsStr = totalRemains == double.MinValue
                             ? string.Empty
                             : string.Format("Федеральные средства (заявлено): <b>{0:N0}</b> тыс.руб.", stateRemains);
        ...
        private void ExcelExporter_EndExport(object sender, EndExportEventArgs e)
        {
            ...
            for (int i = 4; i < rowsCount + 4; i++)
            {
                ...
                if (i == rowsCount - 3)
                {
                    e.CurrentWorksheet.Rows[i].Cells[columnCount - 1].Value = "Дефицит средств для проведения расходов по доведенным лимитам";
                }
            }
            ...
            if (e.CurrentWorksheet.Name == "Прогноз")
            {
                e.CurrentWorksheet.Rows[rowsCount + 6].Cells[0].Value = CommentTextExportsReplaces(MeansDeficit.Text);
            }
//            else
//            {

 ...
 */
namespace Krista.FM.Server.Dashboards.reports.ORG_0001_0005_YANAO
{
    public partial class Default : CustomReportPage
    {
        ...
        private const string PageTitleCaption = "Анализ средних розничных цен на отдельные виды социально значимых продовольственных товаров первой необходимости";
        private const string PageSubTitleCaption = "Еженедельный мониторинг средних розничных цен на отдельные виды социально значимых продовольственных товаров первой необходимости, Ямало-Ненецкий автономный округ, по состоянию на {0}";
        private const string Chart1TitleCaption = "Динамика средней розничной цены на товар «{0}» по ЯНАО, рублей за {1}";
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            ImageFood.ImageUrl = String.Format("../../images/Продукты_ЯМал/{0}.jpg", GetIndexForImage(ComboFood.SelectedIndex));

            LabelLink.Text = String.Format("<a href='../ORG_0001_0006_YANAO/Default.aspx?paramlist=food={0};selectedDate={1}'>Подробнее...</a>", HttpContext.Current.Server.UrlEncode(ComboFood.SelectedValue), HttpContext.Current.Server.UrlEncode(selectedDate.Value));
        ...
        protected void FillData()
        {
            ...
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForPivotTable(query, dtData1);
            DataTable dtData2 = new DataTable();
            query = DataProvider.GetQueryText("ORG_0001_0005_data_part2");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Территория", dtData2);
            ParseData(dtData1, dtData2);

            DataTable dtData3 = new DataTable();
            query = DataProvider.GetQueryText("HMAO-VAL-CUR");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Территория", dtData3);

            DataTable dtData4 = new DataTable();
            query = DataProvider.GetQueryText("HMAO-VAL-PREV");
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(query, "Территория", dtData4);
        ...
        protected void UltraChart1_DataBinding(object sender, EventArgs e)
        {
            ...
            if (dtDate != null)
            {
                ...
                for (int i = start; i < dtDate.Rows.Count; ++i)
                {
                    ...
                    date.Value = 
                        StringToMDXDate(day + " " + CRHelper.RusMonthGenitive(CRHelper.MonthNum(month)) + ' ' + year + " года");
            ...
            try
            {
                //SetHmaoValue(100.0 * (CurHMAO / PrevHMAO - 1));
            }
            catch { }
        }
        ...
        private void SetHmaoValue(double HMAO)
        {
            ...
            if (HMAO > 0)
                LabelHMAO.Text = String.Format("{0} +{1:P2}", "Ямало-Ненецкий автономный округ", HMAO);
            else
                LabelHMAO.Text = String.Format("{0} {1:P2}", "Ямало-Ненецкий автономный округ", HMAO);
        }
        ...
        private void SetLabelHmao()
        {
            ...
            {
                LabelHMAO.Text = "Изменений средней розничной цены в ЯНАО не было зарегистрировано";
            }
        ...
        protected void ParseData(DataTable dtData1, DataTable dtData2)
        {
            ...
            DataProvidersFactory.PrimaryMASDataProvider.GetDataTableForChart(DataProvider.GetQueryText("textovkadata"), "-", date);
            ...
            foreach (DataRow row in date.Rows)
            {
                try
                {
                    decimal curval = (decimal)row[1];
                    decimal Prevval = (decimal)row[2];
                    if (index < 1)
                    {
                        comparePriceHMAO = (double)Prevval;
                        selectedPriceHMAO = (double)curval;
                    }
                    else
                    {

                        string NameR = row[0].ToString();

                        regionNames[index] = NameR;
                        regionValues[index] = (double)curval / (double)Prevval - 1;
                    }
                    index++;
                }
                catch { }

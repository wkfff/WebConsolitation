...
namespace Krista.FM.Server.Dashboards.reports.FO_0002_0006
{
    public partial class DefaultDetail : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            Page.Title = string.Format("Расходы ({0})", ComboRegion.SelectedValue);
            Label1.Text = Page.Title;
            Label2.Text = string.Format("Информация об исполнении за {0} {1} {2} года в разрезе разделов, подразделов классификации расходов",
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            groupCell.AddCell("Исполнено %", "Процент выполнения назначений. Индикация сравнения со средним исполнением по региону");
                ...
                if (executePercent)
                {
                        ...
                        e.Row.Cells[i].Value.ToString() != string.Empty)
                    {
                        ...
                        if (value < avgValue)
                        {
                            ...
                            e.Row.Cells[i].Title = string.Format("Ниже среднего исполнения по субъекту РФ ({0:N0}%)",
                                                                 avgValue);
                        }
                        else if (value > avgValue)
                        {
                            e.Row.Cells[i].Style.BackgroundImage = "~/images/CornerGreen.gif";
                            e.Row.Cells[i].Title = string.Format("Выше среднего исполнения по субъекту РФ ({0:N0}%)",
                                                                 avgValue);
                        }
        ...
        private void PdfExportButton_Click(object sender, EventArgs e)
        {
            ...
            ReportPDFExporter1.Export(headerLayout, Label2.Text);
        }

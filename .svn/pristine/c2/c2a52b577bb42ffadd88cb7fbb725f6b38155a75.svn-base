...
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0003_Novosib
{
	public partial class Default : CustomReportPage
	{
			...
			for (int i = 0; i < e.SceneGraph.Count; i++)
			{
				...
				if (primitive is Polyline)
				{
					...
					foreach (DataPoint point in polyline.points)
					{
						...
						{
                            CRHelper.SaveToErrorLog(((NumericDataPoint)point.DataPoint).Value.ToString());
				...
				if (primitive is Polyline)
				{
					...
					foreach (DataPoint point in polyline.points)
					{
						if (point.Series != null)
						{
                            ...
                            point.DataPoint.Label = point.DataPoint.Label;// + "|" + label.ToString();

                            //Численность работников, перед которыми присутствует просроченная задолженность по заработной плате, Баганский муниципальный район, человек

                            //Численность работников, перед которыми присутствует просроченная задолженность по заработной плате, Новосибирская область, человек
                            ...
                            if (label.ToString() == "Численность работников, перед которыми присутствует просроченная задолженность по заработной плате, Новосибирская область, человек")
                            {
                                point.DataPoint.Label = string.Format("Численность работников, перед которыми присутствует просроченная задолженность по заработной плате,\nНовосибирская область,\nЧеловек на {0}\n{1}", point.DataPoint.Label, ((NumericDataPoint)point.DataPoint).Value);
                                    //"{1} на {2}\n {0:N0}", 
                                    //((NumericDataPoint)point.DataPoint).Value, label.ToString().Replace(",", ",<br>"), point.DataPoint.Label);
                                //<b><\b>
                            }
                            ...
                            {
                                //point.DataPoint.Label = string.Format("{1} на {2}\n {0:N0}", ((NumericDataPoint)point.DataPoint).Value, label.ToString().Replace(",", ",<br>"), point.DataPoint.Label);
		...
		private void PdfExportButton_Click(object sender, EventArgs e)
		{
			...
			title.Style.Font = new Infragistics.Documents.Graphics.Font(font);
		...
		{
			e.CurrentWorksheet.PrintOptions.Orientation = Infragistics.Excel.Orientation.Landscape;

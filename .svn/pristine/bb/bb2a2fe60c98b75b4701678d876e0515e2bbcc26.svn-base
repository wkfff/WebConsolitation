...
{
    public partial class FO_0035_0018_Horizontal : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            SetupChart(UltraChart1, 15);

            dt = GetDataSource("FO_0035_0018_Horizontal_chart_incomes");

            UltraChart1.Series.Clear();
            UltraChart1.Series.Add(CRHelper.GetNumericSeries(1, dt));
            UltraChart1.Series.Add(CRHelper.GetNumericSeries(2, dt));
            UltraChart1.Series.Add(CRHelper.GetNumericSeries(3, dt));
            UltraChart1.Series.Add(CRHelper.GetNumericSeries(4, dt));

            UltraChart1.DataBind();

            lbDescription.Text = "Расходы областного бюджета с учетом восстановления кассовых расходов и платежей, отклоненных банком";
        }
        ...
        private DataTable GetDataSource(string queryName)
        {
            ...
            for (int i = 0; i < dtSource.Rows.Count; i++)
            {
                ...
                if (dtSource.Rows[i][2] != DBNull.Value &&
                    dtSource.Rows[i][3] != DBNull.Value)
                {
                    dtSource.Rows[i][2] = Convert.ToDouble(dtSource.Rows[i][2].ToString()) + Convert.ToDouble(dtSource.Rows[i][3].ToString());
                }
            }
                    ...
                    case 3:
                        {
                            ...
                            PaintElement iconPe = new PaintElement();
                            iconPe.Fill = Color.Aqua;
                            iconPe.FillStopColor = Color.Aqua;
                            iconPe.ElementType = PaintElementType.Gradient;
                            iconPe.FillGradientStyle = GradientStyle.ForwardDiagonal;
                            iconPe.FillOpacity = (byte)150;
                            iconPe.FillStopOpacity = (byte)150;
                            lineAppearance2.IconAppearance.PE = iconPe;
                    ...
                    case 4:
                        {
                            ...
                            PaintElement iconPe = new PaintElement();
                            iconPe.Fill = Color.MediumSeaGreen;
                            iconPe.FillStopColor = Color.Green;
                            iconPe.ElementType = PaintElementType.Gradient;
                            iconPe.FillGradientStyle = GradientStyle.ForwardDiagonal;
                            iconPe.FillOpacity = (byte)150;
                            iconPe.FillStopOpacity = (byte)150;
            ...
            for (int i = 0; i < e.SceneGraph.Count; i++)
            {
                ...
                if (primitive is PointSet)
                {
                    ...
                    foreach (DataPoint point in pointSet.points)
                    {
                        ...
                        {
                            double value = point.Row == 0 || point.Row == 2 ? Convert.ToDouble(dt.Rows[point.Column][point.Row + 1].ToString()) - Convert.ToDouble(dt.Rows[point.Column][point.Row + 2].ToString()) : Convert.ToDouble(dt.Rows[point.Column][point.Row + 1].ToString());
                            int year = Convert.ToInt32(dt.Rows[point.Column][5].ToString());

                            point.DataPoint.Label = point.Row > 1 ?
                                String.Format("<b>{0:N1}</b> тыс.руб.", dt.Rows[point.Column][point.Row + 2]) :
                                String.Format("{0} {1} {3} года<br/><b>{2:N1}</b> тыс.руб.", point.DataPoint.Label.Split()[0], CRHelper.RusMonthGenitive(CRHelper.MonthNum(point.DataPoint.Label.Split()[1])), value, year);
                        }
                    }
                }

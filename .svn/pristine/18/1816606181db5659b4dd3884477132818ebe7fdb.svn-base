...
namespace Krista.FM.Server.Dashboards.reports.FST_0002_0001
{
    public partial class Default : CustomReportPage
    {
        ...
        private CustomParam currPeriodComboValue;
        private CustomParam comparePeriodComboValue;
        private CustomParam subjectComboValue;
        private CustomParam serviceComboValue;
        private CustomParam increaseMode;
        ...
        protected override void Page_PreLoad(object sender, EventArgs e)
        {
            ...
            currPeriodComboValue = UserParams.CustomParam("currPeriod_combo_value", true);
            comparePeriodComboValue = UserParams.CustomParam("comparePeriod_combo_value", true);
            subjectComboValue = UserParams.CustomParam("subject_combo_value", true);
            serviceComboValue = UserParams.CustomParam("service_combo_value", true);
            increaseMode = UserParams.CustomParam("increase_mode", true);
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!Page.IsPostBack)
            {
                currPeriodComboValue.Value = "Февраль 2012 года";
                comparePeriodComboValue.Value = "Декабрь 2011 года";
                subjectComboValue.Value = "Центральный федеральный округ";
                serviceComboValue.Value = "Водоснабжение";
                increaseMode.Value = "true";

                ComboPeriod.Title = "Период";
                ComboPeriod.Width = 250;
                ComboPeriod.MultiSelect = false;
                ComboPeriod.ParentSelect = false;
                periodDigest = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider, "FST_0002_0001_periodDigest");
                ...
                ComboPeriod.SetСheckedState(currPeriodComboValue.Value, true);

                ComboComparePeriod.Title = "Период для сравнения";
                ComboComparePeriod.Width = 250;
                ComboComparePeriod.MultiSelect = false;
                ComboComparePeriod.ParentSelect = false;
                periodCompareDigest = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider, "FST_0002_0001_periodDigest");
                ...
                ComboComparePeriod.SetСheckedState(comparePeriodComboValue.Value, true);
  
                ComboSubject.Title = "Субъект РФ";
                ComboSubject.Width = 250;
                ComboSubject.MultiSelect = false;
                ComboSubject.ParentSelect = true;
                subjectDigest = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider, "FST_0002_0001_subjectDigest");
                ComboSubject.FillDictionaryValues(CustomMultiComboDataHelper.FillMemberUniqueNameList(subjectDigest.UniqueNames, subjectDigest.MemberLevels));
                ComboSubject.SetСheckedState(subjectComboValue.Value, true);

                ComboService.Title = "Услуга";
                ComboService.Width = 250;
                ComboService.MultiSelect = false;
                serviceDigest = new MemberAttributesDigest(DataProvidersFactory.SpareMASDataProvider, "FST_0002_0001_serviceDigest");
                ...
                ComboService.SetСheckedState(serviceComboValue.Value, true);

                CompareButtonList.SelectedIndex = Convert.ToBoolean(increaseMode.Value) ? 0 : 1;
            }
            ...
            selectedSubject.Value = subjectDigest.GetMemberUniqueName(ComboSubject.SelectedValue);
            selectedService.Value = serviceDigest.GetMemberUniqueName(ComboService.SelectedValue);

            Page.Title = "Тарифы на коммунальные услуги ЦФО";
        ...
        private void GridDataBind()
        {
            ...
            string query = DataProvider.GetQueryText("FST_0002_0001_grid");
            ...
            if (gridDt.Rows.Count > 0)
            {
                ...
                CommentTextLabel.Text = String.Format("<b>{6}</b>.<br/><b>{1} тарифа</b> на <b>{0}</b> за период <b>{2} {3}</b> г. - <b>{4} {5}</b> г. наблюдался в следующих муниципальных образованиях и организациях",
                    ComboService.SelectedValue.ToLower(), deviationText, 
                    CRHelper.RusMonth(compareDate.Month), compareDate.Year, CRHelper.RusMonth(currentDate.Month), currentDate.Year, ComboSubject.SelectedValue);
            }
            else
            {
                GridBrick.Visible = false;
                CommentTextLabel.Text = String.Format("<b>{6}</b>.<br/><b>{1} тарифа</b> на на <b>{0}</b> за период <b>{2} {3}</b> г. - <b>{4} {5}</b> г. не наблюдалось",

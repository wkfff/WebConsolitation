...
namespace Krista.FM.Client.MDXExpert
{
    ...
    public class  GaugeFormatBrowseClass : FilterablePropertyBase
    {
        #region Поля
        ...
        private bool showUnitsDisplayType = false;
        ...
        [Browsable(false)]
        public bool ShowUnitsDisplayType
        {
            get { return this.showUnitsDisplayType; }
            set { this.showUnitsDisplayType = value; }
        }
        ...
        {
            get { return (FormatCanUnits(this.FormatType) && this.ShowUnitsDisplayType); }
        }
        ...
        public GaugeFormatBrowseClass(string formatString)
        {
        ...
        {
            string beginLabel = "<DATA_VALUE:";
            int startIndex = text.IndexOf(beginLabel);
            ...
            while (startIndex > -1)
            {
                ...
                text = text.Remove(startIndex + beginLabel.Length, endIndex - startIndex - beginLabel.Length);
                text = text.Insert(startIndex + beginLabel.Length, formatString);
                startIndex = text.IndexOf(beginLabel, startIndex + beginLabel.Length);
            }
            return text;
        }
        ...
        public static string GetMaskFromText(string text)
        {
            string beginLabel = "<DATA_VALUE:";

            int beginMask = text.IndexOf(beginLabel);
            if (beginMask > -1)
            {
                int endMask = text.IndexOf(">");
                if(beginMask < endMask)
                {
                    return text.Substring(beginMask + beginLabel.Length, endMask - beginMask - beginLabel.Length);
                }
            }
            else
            {

...
{
    public class MonthTaskBuilder : ITaskBuilder
    {
        ...
        public IList<D_CD_Task> Build(D_CD_Reglaments reglament, DateTime start, DateTime end)
        {
            ...
            for (int period = start.Year; period <= end.Year; period++)
            {
                ...
                foreach (var subject in subjects)
                {
                    for (int i = 0; i < 12; i++)
                    {
                        var beginDate = new DateTime(period, 1, 1).AddMonths(i);
                        var endDate = new DateTime(period, 1, 1).AddMonths((i + 1));
                        ...
                        if (beginDate >= start && endDate <= end)
                        {
                                    ...
                                    && x.RefYear == year))
                            {
                                ...
                                {
                                    Name = "{0} за {1}".FormatWith(reglament.RefTemplate.Name, beginDate.ToString("MMMM").ToLower()),

...
namespace Krista.FM.Server.DataPumps.Form4NMPump
{
    ...
    // ФНС - 0006 - Форма 4-НМ (коды классификаторов)
    public partial class Form4NMPumpModule : CorrectedPumpModuleBase
    {

        #region Коды "Задолженность.ФНС"

        private Dictionary<int, int[]> GetArrearsCodes2005()
        {
            ...
            // раздел IV.I
            codes.Add(6, new int[] { 4600, 4610, 4620, 4630 });
            // раздел V
            codes.Add(7, new int[] {
        ...
        private Dictionary<int, int[]> GetArrearsCodes2006()
        {
            Dictionary<int, int[]> codes = new Dictionary<int, int[]>();
        ...
        private Dictionary<int, int[]> GetArrearsCodes2009()
        {
            Dictionary<int, int[]> codes = new Dictionary<int, int[]>();
        ...
        private Dictionary<int, int[]> GetArrearsCodes2010()
        {
            Dictionary<int, int[]> codes = new Dictionary<int, int[]>();
        ...
        private Dictionary<int, int[]> GetArrearsCodes2011()
        {
            Dictionary<int, int[]> codes = new Dictionary<int, int[]>();
        ...
        // возвращает коллекцию кодов задолженностей
        // ключ: номер раздела
        // значение: массив кодов задолженностей, соответствующих этому разделу
        private Dictionary<int, int[]> GetArrearsCodes()
        {
        ...
        // возвращает все коды задолженности одним списком
        private List<int> GetAllArrearsCodes()
        {
            List<int> allCodes = new List<int>();
            foreach (int[] codes in allArrearsCodes.Values)
            {
        ...
        // возвращает коды задолженности, для которых формируется третий уровень иерархии
        private int[] GetArrearsCodesWithThirdLevel()
        {
            return new int[] {
                2400, 2410, 2420, 2430, 2440, 2450, 2460, 2470,
                2900, 2910, 2920, 2930, 2940, 2925, 2950, 2960 };
        }

        #endregion

        #region Коды "Доходы.Группы ФНС"
        ...
        private Dictionary<int, int[]> GetIncomesMappings2005()
        {
            ...
            // раздел I
            mappings.Add(0, new int[] {
                3, 0, 4, 100000000, 5, 101000000, 6, 101010000, 7, 102000000, 8, 103000000, 
                9, 103010000, 10, 104000000, 11, 200000000, 12, 300000000, 13, 400000000 });
            // раздел II
            mappings.Add(1, new int[] {
                3, 0, 4, 0, 5, 0, 6, 100000000, 7, 101000000, 8, 101010000, 9, 102000000,
                10, 103000000, 11, 103010000, 12, 104000000, 13, 200000000, 14, 300000000, 15, 400000000 });
            // раздел II.I
            mappings.Add(2, new int[] { 3, 0, 4, 0, 5, 0, 6, 0 });
            // раздел III
            mappings.Add(3, new int[] {
                3, 104010000, 4, 104010100, 5, 104010101, 6, 104010102, 7, 104010200, 8, 104010300,
        ...
        private Dictionary<int, int[]> GetIncomesMappings2006()
        {
            ...
            // раздел I
            mappings.Add(0, new int[] {
                3, 0, 4, 100000000, 5, 101000000, 6, 101010000, 7, 102000000, 8, 102010000,
            ...
            // раздел II.I
            if (fromAugust2011)
                mappings.Add(2, new int[] { 4, 0, 5, 0, 6, 0, 7, 0, 8, 0, 9, 0 });
            else
                mappings.Add(2, new int[] { 3, 0, 4, 0, 5, 0, 6, 0 });
            // раздел III
            mappings.Add(3, new int[] {
        ...
        // возвращает коллекцию маппингов полей с кодами доходов
        // ключ: номер раздела
        // значение: массив-маппинг, в котором чередуются номер столбца в отчете и соответствующий ему код дохода
        ...
        private int[] GetIncomesByArrearsMapping2005()
        {
            return new int[] {
        ...
        private int[] GetIncomesByArrearsMapping2006()
        {
            return new int[] {
                ...
                2590, 104020000, 2600, 104020000, 2810, 104020000, 2820, 104020000,
        ...
        private int[] GetIncomesByArrearsMapping2010()
        {
            return new int[] {
                2490, 103020000, 2500, 103020000, 2690, 103020000, 2700, 103020000,
                2510, 103030000, 2520, 103030000, 2710, 103030000, 2720, 103030000,
        ...
        private int[] GetIncomesByArrearsMapping2010_4()
        {
            return new int[] {
                2490, 103020000, 2500, 103020000, 2690, 103020000, 2700, 103020000,
                2510, 103030000, 2520, 103030000, 2710, 103030000, 2720, 103030000,
        ...
        private int[] GetIncomesByArrearsMapping2011()
        {
            return new int[] {
                2490, 103020000, 2500, 103020000, 2690, 103020000, 2700, 103020000,
                2510, 103030000, 2520, 103030000, 2710, 103030000, 2720, 103030000,
                ...
                2960, 107010000,
                2840, 201000000, 2841, 201000000, 2620, 201000000, 2621, 201000000,
        ...
        // заполняет коллекцию соотношений классификаторов "Задолженность" и "Доходы"
        // ключ: код задолженности
        // значение: код дохода (-1 - неуказанное значение)
        private Dictionary<int, int> GetIncomesByArrears()
        {
            Dictionary<int, int> incomes = new Dictionary<int, int>();

            int[] mapping;
            if (this.DataSource.Year >= 2011)
            {
                mapping = GetIncomesByArrearsMapping2011();
            }
            else if (this.DataSource.Year >= 2010)
            {
                if (this.DataSource.Month >= 4)
                    mapping = GetIncomesByArrearsMapping2010_4();
                else
                    mapping = GetIncomesByArrearsMapping2010();
            }
            else if (this.DataSource.Year >= 2006)
            {
                mapping = GetIncomesByArrearsMapping2006();
            }
            else
            {
                mapping = GetIncomesByArrearsMapping2005();
            }

            int count = mapping.GetLength(0);
            for (int i = 0; i < count; i += 2)
            {
                incomes.Add(mapping[i], mapping[i + 1]);
            }
        ...
        #region Первый уровень иерархии классификатора Задолженность.ФНС

        // возвращает список наименований всех разделов
        ...
        // возвращает коллекцию IDs задолженностей первого уровня иерархии
        private Dictionary<int, int> GetArrearsParentIds()
        {
            Dictionary<int, int> parentIds = new Dictionary<int, int>();

            // выбираем из датасета Задолженности записи первого уровня (Code == 0)
            DataRow[] parentArrearsRows = dsArrears.Tables[0].Select("CODE = 0");
            foreach (DataRow row in parentArrearsRows)
            {
                int sectionIndex = GetSectionIndexByName(Convert.ToString(row["Name"]));
                if (parentIds.ContainsKey(sectionIndex))
                    continue;
                parentIds.Add(sectionIndex, Convert.ToInt32(row["ID"]));
            }

            // дополняем parentIds недостающими разделами
            string[] sectionNames = GetSectionsNames();
            for (int sectionIndex = 0; sectionIndex < sectionNames.GetLength(0); sectionIndex++)
            {
                if (parentIds.ContainsKey(sectionIndex))
                    continue;

                object[] mapping = new object[] { "CODE", 0, "NAME", sectionNames[sectionIndex] };
                int parentId = PumpRow(dsArrears.Tables[0], clsArrears, mapping);
                parentIds.Add(sectionIndex, parentId);
            }
        ...
        {
            allArrearsCodes = GetArrearsCodes();
            arrearsParentIds = GetArrearsParentIds();
            incomesMappings = GetIncomesMappings();
            incomesByArrears = GetIncomesByArrears();
            absentArrearsCodesStr = GetAllArrearsCodes();
        }

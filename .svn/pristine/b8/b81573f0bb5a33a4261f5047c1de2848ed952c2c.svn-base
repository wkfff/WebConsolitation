	...
	public sealed class RtfParser : RtfParserBase
	{
		...
		private bool HandleTag( TextReader reader, IRtfTag tag )
		{
			...
			switch ( tagName )
			{
				...
				default:
                    if (tag.FullName != "lang1033" && tag.FullName != "lang1049" && tag.FullName != "f1" && tag.FullName != "f0")
                    {
                        FlushText();
                    }
			        NotifyTagFound(tag);
		...
		{
            return (character >= 'a' && character <= 'z') || (character >= 'A' && character <= 'Z') || (character >= 'à' && character <= 'ÿ') || (character >= 'À' && character <= 'ß');
		} // IsASCIILetter

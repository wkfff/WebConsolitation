...
{
    public partial class Food_0001_0003 : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
			...
			string filename = HttpContext.Current.Server.MapPath("~/TemporaryImages/Food_0001_0003/") + "TouchElementBounds.xml";
			Directory.CreateDirectory(HttpContext.Current.Server.MapPath("~/TemporaryImages/Food_0001_0003/"));
            File.WriteAllText(filename, String.Format("<?xml version=\"1.0\" encoding=\"UTF-8\"?><touchElements><element id=\"Food_0002_0003_Food={0}\" bounds=\"x=0;y=400;width=768;height=100\" openMode=\"incomes\"/><element id=\"Food_0004_0003_Food={0}\" bounds=\"x=0;y=500;width=768;height=230\" openMode=\"outcomes\"/></touchElements>", HttpContext.Current.Session["CurrentFoodID"]));

            string multitouchIcon = String.Empty;
            multitouchIcon = "<img src='../../../images/detail.png'>";
            detalizationIconDiv.InnerHtml = String.Format("<table><tr><td><a href='webcommand?showPinchReport=Food_0002_0003_Food={1}'>{0}</a></td>", multitouchIcon, HttpContext.Current.Session["CurrentFoodID"]);
            detalizationIconDiv.InnerHtml += String.Format("<td><a href='webcommand?showPinchReport=Food_0004_0003_Food={1}'><img src='../../../images/lock.png'></a></td></tr></table>", multitouchIcon, HttpContext.Current.Session["CurrentFoodID"]);
        ...
        private void InitializeDate()
        {
            ...
            string query = DataProvider.GetQueryText("Food_0001_0003_incomes_date");
		...
		{
			string query = DataProvider.GetQueryText("Food_0001_0003_text");
            ...
            string result = String.Format(
                "На&nbsp;<span class='DigitsValue'>{1:dd.MM.yyyy} г.</span>&nbsp;средняя розничная цена по ХМАО-Югре на продукт питания «{0}» за {3}:&nbsp;<span class='DigitsValueXLarge'>{2:N2}</span>&nbsp;руб.",
		...
		private void UltraChart1_DataBinding(object sender, EventArgs e)
		{
			...
			string query = DataProvider.GetQueryText("Food_0001_0003_chart1");
		...
		protected void UltraChart2_DataBinding(object sender, EventArgs e)
		{
			...
			string query = DataProvider.GetQueryText("Food_0001_0003_chart2");
            DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "МО", dt1);

			DataTable dt2 = new DataTable();
			query = DataProvider.GetQueryText("Food_0001_0003_chart2_data");
			DataProvidersFactory.SpareMASDataProvider.GetDataTableForChart(query, "МО", dt2);

            DataTable dtChart = dt1.Clone();
            object beloyarValue = DBNull.Value;
            foreach (DataRow moRow in dt1.Rows)
            {
                DataRow newRow = dtChart.NewRow();
                if (moRow["МО"].ToString() == "гп Белоярский")
                {
                    beloyarValue = moRow["Текущая цена"];
                    continue;
                }
                newRow["МО"] = moRow["МО"];
                newRow["Текущая цена"] = MathHelper.GeoMean(moRow, 1, 1, DBNull.Value);
                dtChart.Rows.Add(newRow);
            }
            foreach (DataRow moRow in dt2.Rows)
			{
				DataRow newRow = dtChart.NewRow();
				newRow["МО"] = moRow["МО"];
                if (moRow["МО"].ToString() == "Белоярский муниципальный район")
                {
                    newRow["Текущая цена"] = MathHelper.GeoMean(moRow, 1, 1, DBNull.Value, beloyarValue);
                }
                else
                {
                    newRow["Текущая цена"] = MathHelper.GeoMean(moRow, 1, 1, DBNull.Value);
			...
			foreach (DataRow row in dtChart.Rows)
			{
				row["МО"] = row["МО"].ToString().Replace("Город ", "").Replace("город ", "").Replace("муниципальный район", "МР");
			}

            SortTable(ref dtChart, dtChart.Columns["МО"], dtChart.Columns["Текущая цена"]);

            UltraChart2.Axis.Y.RangeMin = Convert.ToDouble(dtChart.Rows[0]["Текущая цена"]) * 0.95;
            UltraChart2.Axis.Y.RangeMax = Convert.ToDouble(dtChart.Rows[dtChart.Rows.Count - 1]["Текущая цена"]) * 1.05;
		...
		protected void UltraChart2_FillSceneGraph(object sender, FillSceneGraphEventArgs e)
		{
            ...
            UltraChart2_AddLineWithTitle(Convert.ToDouble(currCostHMAO), "ХМАО - Югра", Color.Yellow, e);
		}

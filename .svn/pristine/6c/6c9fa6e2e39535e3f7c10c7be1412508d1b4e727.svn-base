...
Ext.ux.Wizard = Ext.extend(Ext.Window, {
    layout: 'fit',
    loadMaskConfig: {
        'default': '',
        'loading': 'Загрузка...',
    ...
    */
    initComponent: function () {
        ...
        Ext.applyIf(this.cardPanelConfig, { region: 'center', items: (this.cards || [{}]), layout: new Ext.ux.wizard.CardLayout({layoutOnCardChange: true}), border: false, activeItem: 0, baseCls: 'ux-wizard-cardpanel' });
        ...
        */
        ...
        Ext.ux.Wizard.superclass.initComponent.call(this);
    },
    ...
    */
    initEvents: function () {
        Ext.ux.Wizard.superclass.initEvents.call(this);
    ...
    */
    initPanels: function () {
        ...
        this.headPanel = new Ext.ux.wizard.Header(this.headConfig);

        this.sidePanel = new Ext.ux.wizard.Header(this.sideConfig);

        Ext.apply(cardPanelConfig, {
            layout: new Ext.ux.wizard.CardLayout({layoutOnCardChange: true}),
            items: cards
        });
    ...
    */
    onCardShow: function (card) {
        ...
        if (card.isValid() && card.canNavigate(1)) {
            this.nextButton.setDisabled(false);
        } else {
            this.nextButton.setDisabled(true);
        }
    ...
    */
    onPreviousClick: function (btn) {
        if (this.currentCard > 0) {
            var card = this.cardPanel.items.items[this.currentCard];
            if (card) {
                if (card.beforeStep(card, -1)) {
                    this.navigate(card, 'prev', 1);
                }
            }
        }
    },
    ...
    onNextClick: function (btn) {
        if (this.currentCard == this.cardCount - 1) {
            ...
            if (card && card.isValid()) {
                if (card.beforeStep(card, 1)) {
                    this.navigate(card, 'next', 1);
                }
            }
        }
    },
    
    navigate: function (card, direction, step) {
        ...
        layout[direction](step);
        Ext.getCmp('wizard-move-prev').setDisabled(!layout.getPrev());
    },
    
    afterRender: function () {
        Ext.ux.Wizard.superclass.afterRender.call(this);

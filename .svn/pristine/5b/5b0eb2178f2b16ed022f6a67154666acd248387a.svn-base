...
namespace Krista.FM.RIA.Extensions.DebtBook.Presentation.ViewBuilders.Capital
{
    public class FormView : BebtBookFormView
    {
        ...
        private void BuildStavropol(List<Component> components)
        {
            ...
            var fieldSet = formPanel.Items.OfType<FieldSet>().FirstOrDefault(x => x.Title.Equals("Информация по размещению и погашению"));
            if (fieldSet == null)
            {
                throw new Exception("Не найдена группа с наименованием Информация по размещению и погашению");
            }

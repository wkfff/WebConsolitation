...
namespace Krista.FM.Server.Dashboards.ORG_0001_0007
{
    public partial class _default : CustomReportPage
    {
        ...
        CustomParam AllDate { get { return (UserParams.CustomParam("AllDate")); } }
        ...
        class CustomizerSize_800x600 : ICustomizerSize
        {
            ...
            protected override void ContfigurationGridIE(UltraWebGrid Grid)
            {
                ...
                foreach (UltraGridColumn col in Grid.Columns)
                {
                    ...
                    {
                        col.Width = onePercent * (100 - 21 - 6) / (Grid.Columns.Count - 2);
                    }
                }
        ...
        private string FillAllDate(IDataSetCombo SetCurDay)
        {

            string All = "";

            foreach (Node n in ComboCurDay.GetLastNode(0).Nodes)
            {
                All += SetCurDay.DataUniqeName[n.Text] + ",";
            }

            return All.ToString().Remove(All.ToString().Length - 1);
        }
        ...
        private void FillComboTypeField()
        {
            ...
            if (!Page.IsPostBack)
            {
                ...
                AllDate.Value = FillAllDate(SetCurDay);
                ...
                ChosenCurPeriodSU.Value = new Day(SetCurDay.DataUniqeName[SelectCurDate()]).BaseCaptionSU;

                DataTable TabeleDefaultValue = DataBaseHelper.ExecQueryByID("GSM_USER_PARAM_Default");
                TypeField.SetСheckedState(TabeleDefaultValue.Rows[0][0].ToString(),true);
        ...
        string SelectFyDate()
        {
            ...
            return ComboCurDay.SelectedNode.Parent.Nodes[0].Text;
        }
        ...
        private void DataBindGrid()
        {
            ...
            RowHMAO["Field"] = "Ханты-Мансийский автономный округ";
        ...
        private void SetHeader()
        {
            ...
            Grid.Bands[0].HeaderLayout.Add(GenHeader("Динамика к предыдущему отчётному периоду", 5, 0, 2, 1));
            Grid.Bands[0].HeaderLayout.Add(GenHeader("Динамика за период с начала года", 7, 0, 2, 1));
        ...
        private void FormatGridValueRow(UltraGridRow Row)
        {
            ...
            string m = PrevDay.GridHeader().Split(' ')[0];
            string v = PrevDay.GridHeader().Replace(m, "");

            Row.Cells.FromKey("PREVDeviation").Title = "Изменение в руб. к " + CRHelper.RusMonthDat(CRHelper.MonthNum(m)) + v;

            Row.Cells.FromKey("PREVSpeedDeviation").Title = "Изменение в % к " + CRHelper.RusMonthDat(CRHelper.MonthNum(m)) + v;

            m = FyDay.GridHeader().Split(' ')[0];
            v = FyDay.GridHeader().Replace(m, "");

            Row.Cells.FromKey("FYDeviation").Title = "Изменение в руб. к " + CRHelper.RusMonthDat(CRHelper.MonthNum(m)) + v;
            Row.Cells.FromKey("FYSpeedDeviation").Title = "Изменение в % к " + CRHelper.RusMonthDat(CRHelper.MonthNum(m)) + v;
        }
        ...
        private void DataBindColChart()
        {
            ...
            for (int i = 1; i < ReportTable.Rows.Count; i++)
            {
                try
                {
                    decimal value = (decimal)ReportTable.Rows[i]["CUR"];
                    DataRow Row = CreateRow(TableChart);
                    Row["Region"] = GenShortNameRegion(ReportTable.Rows[i]["Field"].ToString());
                    Row["Value"] = value;
                    max = (decimal)Row["Value"] > max ? (decimal)Row["Value"] : max;
                    min = (decimal)Row["Value"] < min ? (decimal)Row["Value"] : min;
                }
                catch { }
        ...
        protected void AddLegend()
        {
            ...
            if (CustomizerSize is CustomizerSize_800x600)
                legend1.Title = string.Format("Средняя розничная\nцена на «{0}»,\n рублей\n(по состоянию на {1}).", Fields.Value, ComboCurDay.SelectedValue);
            else
                legend1.Title = string.Format("Средняя розничная\nцена на «{0}»,\n рублей\n(по состоянию на {1}).", Fields.Value, ComboCurDay.SelectedValue);
        ...
        DataRow GetValueFromNameRegion(DataTable Table, string Region)
        {
            ...
            foreach (DataRow row in Table.Rows)
            {
                if (!FirstRow)
                {
                    if (GenShortNameRegion(row[0].ToString()) == Region)
                    {
                        if (row["CUR"] != DBNull.Value)
                            return row;
                    }
    ...
    HttpResponseSubstitutionCallback callback
        ...
        {
            Label3.Text = string.Format("Распределение территорий по средней розничной цене на «{0}», рубль, по состоянию на {1}", Fields.Value, CurDay.GridHeader());

            PageSubTitle.Text = string.Format("Ежемесячный мониторинг средних розничных цен на лекарственные препараты  в разрезе муниципальных образований, {1}, по состоянию на {0}.", CurDay.GridHeader(),RegionSettingsHelper.Instance.Name);

            Hederglobal.Text = "Анализ средних розничных цен на лекарственные препараты в разрезе муниципальных образований";

            Page.Title = Hederglobal.Text;
                
            Label1.Text = string.Format("Средняя розничная цена на «{0}», рубль", Fields.Value);
            Page.Title = Hederglobal.Text;

            GridHeader.Text = string.Format("Средние розничные цены на товар «{0}», рубль", Fields.Value);
        }

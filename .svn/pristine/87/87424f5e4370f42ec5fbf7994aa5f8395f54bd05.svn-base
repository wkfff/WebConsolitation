...
{
    public partial class FK_0001_0003_h : CustomReportPage
    {
        ...
        protected override void Page_Load(object sender, EventArgs e)
        {
            ...
            TextBox3.Text = string.Format("за {0} {1} {2} года по субъектам {3}.",
                monthNum,
                monthStr,
                yearNum,
                UserParams.ShortRegion.Value);
            TextBox4.Text = string.Format("Ранг субъекта в {0}.", UserParams.ShortRegion.Value);
                                    
            TextBox5.Text = string.Format("Расходы млдр.руб. = расходы консолидированных бюджетов");
            TextBox6.Text = string.Format("субъектов РФ за {0} {1} {2} года",
                monthNum,
                monthStr,
                yearNum);

            TextBox7.Text = string.Format("Население тыс.чел. = численность пост.населения {0}", populationDate);
        ...
        protected void UltraWebGrid_InitializeLayout(object sender, LayoutEventArgs e)
        {
            ...
            if (e.Layout.Bands[0].Columns.Count > 8)
            {
                ...
                AddColumnHeader(e.Layout, "Расходы", 0, 2, 1);
                AddColumnHeader(e.Layout, "Исполнено", 0, 3, 2);
                AddColumnHeader(e.Layout, "Население", 0, 5, 1);
                AddColumnHeader(e.Layout, "Бюдж.расх. на душу населения", 0, 6, 2);
        ...
        protected void UltraWebGrid_InitializeRow(object sender, RowEventArgs e)
        {
            for (int i = 0; i < e.Row.Cells.Count; i++)
            {
                ...
                {
                    e.Row.Cells[i].Style.Padding.Right = 1;

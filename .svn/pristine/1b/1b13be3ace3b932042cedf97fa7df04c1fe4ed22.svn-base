namespace MDXParser
{
    ...
    public class DependencyGraph
    {
        ...
        internal void FindLoops(Analyzer analyzer)
        {
            foreach (Vertex vertex in this.m_Vertices.Values)
            {
                ...
                if (flag)
                {
                    ...
                    m.Severity = 3;
                    m.Text = string.Format("Calculation {0} recursively refers to itself. Recursion disables block optimization mode", vertex.Name);
                    m.Id = 0x2d;
                    analyzer.Add(m);
                }
            }
        ...
        public void Visualize(NetMapControl netMapControl, ICollection<Vertex> showVertices, Vertex root)
        {
            ...
            manager.Layout = LayoutType.FruchtermanReingold;
            manager.ApplyLayoutToNetMapControl(netMapControl, 10);

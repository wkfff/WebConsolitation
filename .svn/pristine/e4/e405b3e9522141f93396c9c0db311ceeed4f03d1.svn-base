 ...
 */
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0004_YANAO
{
	public partial class Default : CustomReportPage
	{
        ...
        private const string PageSubTitleCaption = "Данные еженедельного мониторинга задолженности по заработной плате в Ямало-Ненецком автономном округе по состоянию на {0}.";
		...
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			#region Ссылки
            CrossLink1.Text = "Задолженность&nbspпо&nbspоплате&nbspтруда(по&nbspмуниципальным&nbspобразованиям&nbspЯНАО)";
            CrossLink1.NavigateUrl = "~/reports/STAT_0003_0003_YANAO/Default.aspx";
		...
		public void FillMap1Data()
		{
			...
			string shapeHint = "{0} \nКоличество граждан, перед которыми имеется задолженность: {1:N0} чел. \nСумма задолженности по выплате заработной платы: {2:N0} тысяч рублей";
                ...
                try
                {
                    ...
                    foreach (Shape shape in shapeList)
                    {
                        ...
                        if (IsCalloutTownShape(shape))
                        {
                            ...
                            shape.TextVisibility = TextVisibility.Shown;
                            shape.Text = String.Format("{0}\n{1:N0} чел.\n{2:N0} т.р.", row[1].ToString(), Convert.ToDouble(row[3].ToString()), Convert.ToDouble(row[4].ToString()));
                            shape.TextVisibility = TextVisibility.Shown;
		...
		protected void UltraWebGrid_DataBinding(object sender, EventArgs e)
		{
			...
			if (dtGrid.Rows.Count > 0)
			{
                ...
                {
                    if (Row[0].ToString().Contains("Бюджет субъекта ЯНАО"))
                    {
                        delRow.Add(Row);       
                    }
                }
		...
		private void ExcelExportButton_Click(object sender, EventArgs e)
		{
            ...
            sheet1.MergedCellsRegions.Remove(sheet1.MergedCellsRegions[1]);
            sheet1.MergedCellsRegions.Add(1, 0, 1, 13);

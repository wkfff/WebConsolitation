 ...
 */
namespace Krista.FM.Server.Dashboards.reports.STAT_0003_0004_Novosib
{
	public partial class Default : CustomReportPage
	{
        ...
        private const string mapFolderName = "Субъекты\\Новособл";//"ХМАО";
        ...
        private const string PageSubTitleCaption = "Данные ежемесячного мониторинга задолженности по заработной плате в Новосибирской области по состоянию на {0}";
        private const string Map1TitleCaption = "Задолженность по оплате труда по муниципальным образованиям на {0}, тысяч рублей";
		// --------------------------------------------------------------------
		protected override void Page_PreLoad(object sender, EventArgs e)
		{
			...
			#region Ссылки
            CrossLink1.Text = "Задолженность по оплате труда (по муниципальным<br>образованиям Новосибирской области)".Replace(" ", "&nbsp;");
            CrossLink1.NavigateUrl = "~/reports/STAT_0003_0004__Novosib/Default.aspx";
            ...
            legend.Title = "Суммарный объем просроченной\nзадолженности по выплате\nзаработной платы, тысяч рублей";
            ...
            legend2.Title = "Численность работников, перед которыми\n присутствует просроченная задолженность\n по заработной плате, человек";
		...
		public static ArrayList FindMapShape(MapControl map, string patternValue, out bool hasCallout)
		{
			...
			foreach (Shape shape in map.Shapes)
			{
				...
				if (GetShapeName(shape).ToLower().Replace(" ","").Contains(patternValue.ToLower().Replace(" ","")))
				{
					shapeList.Add(shape);
		...
		protected void UltraWebGrid_InitializeLayout(object sender, Infragistics.WebUI.UltraWebGrid.LayoutEventArgs e)
		{
            ...
            for(int i = 1;i<e.Layout.Bands[0].Columns.Count;i++)
            {
                UltraGridColumn col = e.Layout.Bands[0].Columns[i];
		...
		private void ExcelExportButton_Click(object sender, EventArgs e)
		{
			...
			DundasMap1.Height = Unit.Pixel((int)(CustomReportConst.minScreenHeight * 1));
			ReportExcelExporter.MapExcelExport(sheet2.Rows[1].Cells[0], DundasMap1, 0.75);

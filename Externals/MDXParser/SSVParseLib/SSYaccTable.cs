namespace SSVParseLib
{
    ...
    public class SSYaccTable
    {
        ...
        public SSYaccTable(string q_file)
        {
            ...
            header.type = this.convertInt(buffer, 0);
            header.prodOrLar = this.convertInt(buffer, 4);
            header.numRows = this.convertInt(buffer, 8);
            header.rowOffset = this.convertInt(buffer, 12);
            header.prodOffset = this.convertInt(buffer, 0x10);

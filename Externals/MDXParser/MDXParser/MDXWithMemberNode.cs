    ...
    public class MDXWithMemberNode : MDXWithNode
    {
        ...
        internal override void Analyze(Analyzer analyzer)
        {
            if (base.Exp.GetMDXType() == MDXDataType.String)
            {
                ...
                m.Id = 5;
                m.Text = string.Format("Calculated member '{0}' evaluates to a string. Strings are difficult to cache - consider using FORMAT_STRING property for value formatting", base.Name.GetLabel());
                m.Severity = 2;
                analyzer.Add(m);
            }
            try

        ...
        internal override void Analyze(Analyzer analyzer)
        {
            ...
            if (this.m_Prop != null)
            {
                if (this.m_Prop.Equals("NON_EMPTY_BEHAVIOR", StringComparison.InvariantCultureIgnoreCase))
                {
                    ...
                    m.Id = 7;
                    m.Text = string.Format("AS2008 only: Consider removing NON_EMPTY_BEHAVIOR property", new object[0]);
                    m.Severity = 2;
                    analyzer.Add(m);
                }
                else
                {
                    ...
                    if (!MDXNode.IsConstantNode(t) && (t != typeof(MDXIDNode)))
                    {
                        ...
                        message2.Id = 8;
                        message2.Text = string.Format("Calculation property '{0}' is not a constant or member reference. Therefore it will not benefit from neiter block computation mode, nor caching", this.m_Prop);
                        message2.Severity = 4;
            ...
            if ((this.m_Prop == null) && (this.m_RValue.GetMDXType() == MDXDataType.String))
            {
                ...
                message3.Id = 9;
                message3.Text = string.Format("RHS of the assignment evaluates to a string. Strings are difficult to cache - consider using FORMAT_STRING property for value formatting", new object[0]);
                message3.Severity = 2;

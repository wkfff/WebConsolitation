    ...
    public abstract class MDXWithNode : MDXNode
    {
        ...
        internal override void Analyze(Analyzer analyzer)
        {
            ...
            if (this.m_ExpressionUsedSingleQuotes)
            {
                ...
                m.Id = 0x2c;
                m.Id = 1;
                m.Text = string.Format("Don't use single quotes for definitions of calculation {0}", this.m_Name.GetLabel());
                m.Severity = 0;
                m.URL = "http://sqlblog.com/blogs/mosha/archive/2005/04/02/to-quote-or-not-to-quote-in-expressions-of-mdx-calculations.aspx";
                analyzer.Add(m);
            }
            if (this.m_CalcProps != null)
            {
                foreach (MDXCalcPropNode node in this.m_CalcProps)
                {
                    if (node.PropertyName.Equals("NON_EMPTY_BEHAVIOR", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        message2.Id = 2;
                        message2.Text = string.Format("AS2008 only: Consider removing NON_EMPTY_BEHAVIOR property for '{0}'", this.m_Name.GetLabel());
                        message2.Severity = 2;
                    ...
                    else
                    {
                        ...
                        if ((!node.PropertyName.Equals("SOLVE_ORDER", StringComparison.InvariantCultureIgnoreCase) && !MDXNode.IsConstantNode(t)) && (t != typeof(MDXIDNode)))
                        {
                            ...
                            message3.Id = 3;
                            message3.Text = string.Format("Calculation property '{0}' of '{1}' is not a constant or member reference. Therefore it will not benefit from neiter block computation mode, nor caching", node.PropertyName, this.m_Name.GetLabel());
                            message3.Severity = 4;
                    ...
                    if (node.PropertyName.Equals("VISIBLE", StringComparison.InvariantCultureIgnoreCase))
                    {
                        ...
                        message4.Id = 4;
                        message4.Severity = 0;
                        message4.URL = "http://technet.microsoft.com/en-us/library/ms144787(SQL.100).aspx";

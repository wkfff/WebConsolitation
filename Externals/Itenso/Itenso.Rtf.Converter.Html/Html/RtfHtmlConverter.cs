...
namespace Itenso.Rtf.Converter.Html
{
	...
	public class RtfHtmlConverter : RtfVisualVisitorBase
	{
		...
		public string Convert()
		{
			...
			using ( StringWriter stringWriter = new StringWriter(new CultureInfo("ru")) )
			{
            ...
            if (settings.ConvertVisualHyperlinks)
                return FindHyperKinkInText(html);

			return html;
		} // Convert

        /// <summary>
        /// Получение гиперссылок в документе
        /// </summary>
        /// <param name="html"></param>
        /// <returns></returns>
        private string FindHyperKinkInText(string html)
        {
            string text = html;

            Regex regex = new Regex(RtfHtmlConvertSettings.DefaultVisualHyperlinkPattern,
            ...
            {
                if (match.Value != "http://www.w3.org/TR/html4/loose.dtd")
                    text = text.Replace(match.Value, string.Format("<a href = \"{0}\">{1}</a>", match.Value, match.Value));
            }
...
#region HtmlFormat
		...
		{
			return HttpUtility.HtmlEncode( rtfText );
		} // FormatHtmlText

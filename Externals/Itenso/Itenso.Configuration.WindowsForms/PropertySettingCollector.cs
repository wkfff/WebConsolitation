...
namespace Itenso.Configuration
{
	...
	public class PropertySettingCollector : SettingCollector
	{
		...
		{
			Collect( this.owner.Controls, true );
		} // Collect
		...
		private void Collect( Control.ControlCollection children, bool recursive )
		{
			...
			{
				bool add = control.GetType().IsAssignableFrom( this.elementType );
				...
				if ( add )
				{
					string settingName = string.Concat( controlId, ".", this.propertyName );
					PropertySetting propertySetting = new PropertySetting( settingName, control, this.propertyName );
		...
		private string GetControlId( Control control )
		{
			...
			while ( control != null && control != this.owner )
			{
				if ( sb.Length > 0 )
				{
					sb.Insert( 0, "." );
